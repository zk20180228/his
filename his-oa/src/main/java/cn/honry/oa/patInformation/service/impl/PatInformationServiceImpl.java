...
@Service("patInformationService")
@Transactional
@SuppressWarnings({ "all" })
public class PatInformationServiceImpl implements PatInformationService {
	@Autowired
	@Qualifier(value = "patInformationDao")
	private PatInformationDao patInformationDao;
	@Autowired
	@Qualifier(value = "patInformationAttachDownAuthDao")
	...
	public Information get(String arg0) {
		return patInformationDao.get(arg0);
	}
	...
	public void saveOrUpdate(Information arg0) {
		...
		if(StringUtils.isBlank(arg0.getId())){
			...
			patInformationDao.save(arg0);
		}else{
			Information information = patInformationDao.get(arg0.getId());
			...
			patInformationDao.update(information);
		}
	}

	@Override
	public MenuVO queryMenuByid(String id) {
		return patInformationDao.findMenuByid(id);
	}
	...
	public List<MenuCkeckedVO> queryMenuCheckByid(String menuid,String type) {
		return patInformationDao.queryMenuCheckByid(menuid,type);
	}

	@Override
	public List<SysEmployee> queryAuthEmp(String menuId, String post,
			String title,String page,String rows) {
		return patInformationDao.queryAuthEmp(menuId, post, title, page, rows);
	}

	@Override
	public int queryAuthEmpTotal(String menuId, String post, String title) {
		return patInformationDao.queryAuthEmpTotal(menuId, post, title);
	}

	@Override
	public List<MenuCkeckedVO> queryRoleAuth(String menuId) {
		return patInformationDao.queryRoleAuth(menuId);
	}

	@Override
	public List<MenuCkeckedVO> queryDeptAuth(String menuId) {
		return patInformationDao.queryDeptAuth(menuId);
	}

	@Override
	public List<MenuCkeckedVO> queryDutyAuth(String menuId) {
		return patInformationDao.queryDutyAuth(menuId);
	}
	@Override
	public void saveInformationCheck(String menuId,String title,String infoid) {
		List<MenuCkeckedVO> list = patInformationDao.queryMenuCheckByid(menuId, "2");//审核
		MenuVO menuVO = patInformationDao.getparentCodeAndName(menuId);
		InformationCheck check = null;
		patInformationDao.deleteCkeck(infoid);
			...
			for (MenuCkeckedVO vo : list) {//审核信息改为存个人 2017年8月4日18:12:22
				if(vo.getType()!=null&&!"1".equals(vo.getType())){//不是个人
					if("0".equals(vo.getType())){//所有人
						...
						List<SubscripeVO> findSub = patInformationDao.findSub(vo.getType(), vo.getCode());
	...
	public void saveInformationSubscripe(String menuId,String title,String infoid) {
		List<String> list = patInformationDao.getPatientAppUser();
		if(list!=null &&list.size()>0){
			//发送推送消息
			
		}
//		List<MenuCkeckedVO> list = patInformationDao.queryMenuCheckByid(menuId, "3");//订阅
//		MenuVO menuVO = patInformationDao.getparentCodeAndName(menuId);
...
//		}
	...
	public List<MenuVO> findMenuVo(String deptCode, String dutyCode,
		...
		return patInformationDao.findMenuVo(deptCode, dutyCode, roleCode, acount);
	}
	...
	public Map<String, String> checkAuth(String menuid, String deptCode,
		...
		Map<String,String> map = new HashMap<String, String>();
		List<MenuCkeckedVO> list = patInformationDao.checkAuth(menuid, deptCode, dutyCode, roleCode, acount);
		if(list!=null&&list.size()>0){
	...
	public List<Information> findinformationList(String name, String menuid,
		...
		return patInformationDao.queryInformationList(name, menuid, page, rows,checkflag,pubflag);
	}

	@Override
	public int findinformationTotal(String name, String menuid, String type,String checkflag,String pubflag) {
		return patInformationDao.queryInformationTotal(name, menuid,checkflag,pubflag);
	}
	...
	public Information getInformationMsg(String menuid) {
		...
		Information info = patInformationDao.get(menuid);
		List<InformationAttachDownAuth> list = patInformationAttachDownAuthDao.getAuthByMenuID(info.getId(),false);
	...
	public void delInformation(String menuId) {
		...
		for (String menuid : menuId.split(",")) {
			Information information = patInformationDao.get(menuid);
	...
	public void deleteFile(String fileurl) {
		int i = patInformationAttachDownAuthDao.deleteFile(fileurl);
	}

	@Override
	public void updateSubscripe(String menuId, String acount) {
		patInformationDao.updateSubscripe(menuId, acount);
	}

	@Override
	public Map<String, String> informationAudit(String infoid,String type) {
		Map<String,String> map = new HashMap<String, String>();
		Information info = patInformationDao.get(infoid);
		if(info.getInfoCheckFlag()!=null&&info.getInfoCheckFlag()!=0){//已审核
			...
			Map<String, String> nameMap = employeeInInterService.queryEmpCodeAndNameMap();
			User user = ShiroSessionUtils.getCurrentUserFromShiroSession();
			MenuVO menuVO = patInformationDao.getparentCodeAndName(info.getInfoMenuid());
			String acount = "";
			if(user!=null){
				acount = user.getAccount();
			}
	...
	public Map<String, Object> getInfomationView(String menuId,String page,String rows) {
		Map<String,Object> map = new HashMap<String, Object>();
		int total = patInformationDao.getInfomationViewTotal(menuId);
		List<Information> list = patInformationDao.getInfomationView(menuId, page, rows);
		Map<String, String> nameMap = employeeInInterService.queryEmpCodeAndNameMap();
	...
	public Map<String, Object> getInformationCheck(String menuId,String page,String rows) {
		Map<String,Object> map = new HashMap<String, Object>();
		int total = patInformationDao.getInformationCheckTotal(menuId);
		List<Information> list = patInformationDao.getInformationCheck(menuId, page, rows);
		Map<String, String> nameMap = employeeInInterService.queryEmpCodeAndNameMap();
		for (Information information : list) {
			information.setPubuserName(nameMap.get(information.getInfoPubuser()));
			information.setWriterName(nameMap.get(information.getInfoWirteuser()));
	...
	public List<MenuCkeckedVO> judgeAuthBymenuCode(String menuid, String type) {
		return patInformationDao.judgeAuthBymenuCode(menuid, type);
	}
...
//				String titleImgurl = uploadFileService.fileUpload(imgFile, imgfilename,"1",acount);
			...
			if(file!=null){//附件上传
				...
				for (File file2 : file) {
					...
					FastDFSClient client = new FastDFSClient();
					String fileurl = client.uploadFile(file2, flname);
									...
									if(typeauth.length>=2){
										...
										if(StringUtils.isNotBlank(code)){
											for (String cd : code.split(",")) {
												...
												patInformationAttachDownAuthService.saveOrUpdate(author);
											}
										}
									}
								}else{
...
//				String titleImgurl = uploadFileService.fileUpload(imgFile, imgfilename,"1",acount);
			...
			patInformationAttachDownAuthService.delAuth(info.getId());
			if(file!=null){//附件上传
				info.setInfoAttach(fileUploadURL);
				String[] attachName = attachname.split("#");
				int i = 0;
...
//					fileUploadURL = uploadFileService.fileUpload(file2,flname,"1",acount);
	...
	public InformationSubscripe querySubscripe(String infoMenuid, String account) {
		return patInformationDao.querySubscripe(infoMenuid,account);
	}

	@Override
	public void updViews(String infoid) {
		patInformationDao.updViews(infoid);
	}
