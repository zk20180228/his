...
public class DrugApplyoutNow extends Entity implements java.io.Serializable {
	/**申请流水号*/
	...
	/**是否需配液 ‘1’ 是 0 否*/
	private Integer compoundFlag = 0;
	/**是否配液已执行 1 是 0 否*/
	private Integer compoundExec = 0;
	/**配液执行人*/
	...
	*/ 
	...
	/** 
	* @Fields drugNum : 申请药品种类数 
	*/ 
	int drugNum;
	/** 
	* @Fields sameRetailPrice : 零售价是否相同（1相同0不同） 
	*/ 
	Integer sameRetailPrice;
	/**申请日期---用于显示*/
	...
	/**内部入库退库审核  审批数量   做传值用 数据库无此字段**/
	private Double examNum;
	
	/** 
	* @Fields storeCost : 外部入库退库导出用字段 
	*/ 
	...
	/** 零售价是否相同（1相同0不同）
	* @Title: getSameRetailPrice 
	* @Description: 零售价是否相同（1相同0不同）
	* @author dtl 
	* @date 2017年1月12日
	*/
	public Integer getSameRetailPrice() {
		return sameRetailPrice;
	}

	/** 零售价是否相同（1相同0不同）
	* @Title: setSameRetailPrice 
	* @Description: 零售价是否相同（1相同0不同）
	* @author dtl 
	* @date 2017年1月12日
	*/
	public void setSameRetailPrice(Integer sameRetailPrice) {
		this.sameRetailPrice = sameRetailPrice;
	}
