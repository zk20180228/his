package cn.honry.finance.refund.service.impl;
...
import cn.honry.finance.refund.vo.RefundDrugList;
...
public class RefundServiceImpl implements RefundService{
	...
	private OutStoreInInterService outstoreInInterService;
	...
	private ApplyoutInInterDAO applyoutInInterDAO;//出库申请表
	...
	public List<FinanceInvoiceInfoNow> findInvoiceInfoByInvoiceNo(String invoiceNo) {
		return refundDAO.findInvoiceInfoByInvoiceNo(invoiceNo);
	}

	@Override
	public List<FinanceInvoiceInfoNow> findInvoiceInfoByInvoiceSeqs(String invoiceSeqs) {
		return refundDAO.findInvoiceInfoByInvoiceSeqs(invoiceSeqs);
	}

	@Override
	public HospitalParameter queryParameter() {
		return refundDAO.queryParameter();
	}

	@Override
	public List<OutpatientFeedetailNow> findFeedetailByInvoiceNos(String invoiceNos,String value,String clinicCode) {
		 return refundDAO.findFeedetailByInvoiceNos(invoiceNos,value,clinicCode);
	}

	@Override
	public BusinessContractunit queryContractunit(String paykindCode) {
		return refundDAO.queryContractunit(paykindCode);
	}

	@Override
	public List<SysEmployee> queryEmpFunction() {
		return refundDAO.queryEmpFunction();
	}

	@Override
	public HospitalParameter queryParameterByRj() {
		return refundDAO.queryParameterByRj();
	}

	@Override
	public HospitalParameter queryParameterByPayType() {
		return refundDAO.queryParameterByPayType();
	}

	@Override
	public Map<String, String> saveRefund(String payType, String drugList,String undDrugList, String refundDrugList, String refundUnDrugList,String patientNo, Double maney, Double price, String invoiceNo, String newInvoiceNo) {
		Map<String, String> map = new HashMap<String, String>();
		if("2".equals(payType)){
			//判断患者是否存在院内账户
			OutpatientAccount account = refundDAO.getAccountByMedicalrecord(patientNo);
...
//				if(!"".equals(drugIds)&&"1".equals(fee.getDrugFlag())){
//					drugIds = drugIds + "','";
//				}
//				if("1".equals(fee.getDrugFlag())){
//					drugIds = drugIds + fee.getItemCode();
//				}
//			}
//		}
		if(feedetailList.size()>0){
...
//		}
		...
		List<MedicinelDrugList> undDrugListVo = gson.fromJson(undDrugList, new TypeToken<List<MedicinelDrugList>>(){}.getType());
		
		String billCode = refundDAO.getSeqByNameorNum("SEQ_INPATIENT_CAN_BILLCODE", 14);
		
		
		if(undDrugListVo.size()>0){
			for(MedicinelDrugList undDrugListVos :undDrugListVo){
				drugListVo.add(undDrugListVos);
			}
		}
		...
		if(drugListVo.size()>0){
			for(MedicinelDrugList drugVo:drugListVo){
				...
				if(drugVo.getNobackNum()>0){
					...
					feedetailNew.setFeeCpcd(ShiroSessionUtils.getCurrentUserFromShiroSession().getId());//收费人
			...
			if(drugListVo.size()>0){
				for(MedicinelDrugList modls : drugListVo){
					...
					cancelitem.setName(registerInfo.getPatientName());//患者姓名
					cancelitem.setDeptCode(registerInfo.getDeptCode());//患者所在科室
					cancelitem.setDeptName(registerInfo.getDeptName());//患者所在科室名称
					...
					cancelitem.setQty(modls.getNobackNums()-modls.getNobackNum());//数量[22]
					cancelitem.setCardNo(registerInfo.getMidicalrecordId());//病历卡号
			...
			if(undDrugListVo.size()>0){
				for(MedicinelDrugList modls : undDrugListVo){
					InpatientCancelitemNow cancelitem = new InpatientCancelitemNow();
					cancelitem.setId(null);
					cancelitem.setBillCode(billCode); //申请单据号
					cancelitem.setApplyFlag(1);//门诊
			...
			invoiceInfo.setPaykindCode("tf");//结算类别
			...
			if(drugListVo.size()>0){
				for(MedicinelDrugList modls : drugListVo){
					if("1".equals(modls.getDrugFlag()) && modls.getNobackNum()>0){
						...
						int applyNumber = Integer.parseInt(applyoutInInterDAO.getSequece("SEQ_DRUG_APPLYOUT").toString());//根据sequence 获取applyNumber
						String yearLast = new SimpleDateFormat("yyMM").format(new Date());
						int value = keyvalueInInterDAO.getVal(ShiroSessionUtils.getCurrentUserLoginDepartmentFromShiroSession().getDeptCode(),"内部入库申请单号",yearLast);
		...
		if(drugListVo.size()>0){
			for(MedicinelDrugList modls : drugListVo){
				if("1".equals(modls.getDrugFlag())&&modls.getNobackNum()>0){
					outstoreInInterService.withholdStock(modls.getExecDpcd(),modls.getItemCode(),-modls.getNobackNum(),modls.getExtFlag3());
				}
			}
		}
		
		String invoiceType = "02";
		medicineListDAO.saveInvoiceFinance(ShiroSessionUtils.getCurrentUserFromShiroSession().getAccount(),newInvoiceNo,invoiceType);
		//添加发票使用记录
	...
	@Override
	public Map<String, String> saveRefundNow(String payType, String drugList,
			String undDrugList, String refundDrugList, String refundUnDrugList,
			String patientNo, Double maney, Double price, String invoiceNos) {
		Map<String, String> map = new HashMap<String, String>();
		String payKindCode = "01";//初始化定义值，后面会赋值
		String payKindName = "自费";//初始化定义值，后面会赋值
		try{
			...
			String invoiceSeq = "";
			//根据发票号查询所有发票明细表中信息
			List<FinanceInvoicedetailNow> invoicedetailList = refundDAO.findInvoiceTailByInvoiceNos(invoiceNos);
			//根据发票号查询所有发票结算表中信息
			List<FinanceInvoiceInfoNow> invoiceInfoLists = refundDAO.findInvoiceInfoByInvoiceNos(invoiceNos);
			//支付方式情况表
			List<BusinessPayModeNow> payModeList = refundDAO.findPayModeListByNos(invoiceNos);
			//收费明细表
			List<OutpatientFeedetailNow> feedetailList = refundDAO.findFeedetailListByNos(invoiceNos);
			RegistrationNow registration = new RegistrationNow();
			if(feedetailList.size()>0){
				registration =refundDAO.queryRegisterInfo(feedetailList.get(0).getClinicCode());
			}else{
				map.put("resMsg", "error");
				map.put("resCode", "该发票号未查询到收费信息，请核对后重试！");
				return map;
			}
			//日结和财务审核后是否可以退费
			...
			if("0".equals(back)){//日结后不可以退费
				if(invoicedetailList.get(0).getBalanceFlag()==1){
					...
					if(daybalance.getCheckFlag()!=null&&daybalance.getCheckFlag()==2){
						...
						daybalance.setTotCost(daybalance.getTotCost()-maney);
			...
			if(feedetailList.size()>0){
				for (OutpatientFeedetailNow fee : feedetailList) {
					...
					if(StringUtils.isNotBlank(fee.getSampleId())){
						if(StringUtils.isNotBlank(bookid)){
							bookid += "','";
						}
						bookid += fee.getSampleId();
					}
				}
			}
			//初始化处方号
			...
			List<StoRecipeNow> stoRecipeList = refundDAO.findStoRecipeByRecipNo(recipeNo);
			//医技相关义务处理
			List<TecApply> tecapplyList = null;
			List<TecTerminalApply> terapplyList = null;
			if(StringUtils.isNotBlank(bookid)){
				tecapplyList = refundDAO.getTecByapplyNumList(bookid);
				terapplyList = refundDAO.getTerByapplyNumList(bookid);
			}
			if(tecapplyList!=null&&tecapplyList.size()>0){
				for (TecApply tec : tecapplyList) {
//					TecApply tc = new TecApply();
//					BeanUtils.copyProperties(tec, tc);
//					tc.setId(null);
//					tc.setTransType(1);
//					tc.setStatus(3);
//					tc.setCreateDept(ShiroSessionUtils.getCurrentUserDepartmentFromShiroSession().getDeptCode());
//					tc.setCreateTime(new Date());
//					tc.setCreateUser(ShiroSessionUtils.getCurrentUserFromShiroSession().getAccount());
//					tc.setUpdateTime(new Date());
//					tc.setUpdateUser(ShiroSessionUtils.getCurrentUserFromShiroSession().getAccount());
//					refundDAO.save(tc);
					
					tec.setStatus(3);
					tec.setUpdateTime(new Date());
					tec.setUpdateUser(ShiroSessionUtils.getCurrentUserFromShiroSession().getAccount());
					refundDAO.update(tec);
					
				}
			}
			if(terapplyList!=null&&terapplyList.size()>0){
				for (TecTerminalApply ter : terapplyList) {
					if(ter.getConfirmNum()!=null&&ter.getConfirmNum()>0){
						TecTerminalApply tr = new TecTerminalApply();
						BeanUtils.copyProperties(ter, tr);
						tr.setId(null);
						tr.setQty(ter.getConfirmNum().doubleValue());
						tr.setSendFlag("2");
						tr.setExtFlag1("1");
						tr.setCreateUser(ShiroSessionUtils.getCurrentUserFromShiroSession().getAccount());
						tr.setCreateTime(new Date());
						tr.setCreateDept(ShiroSessionUtils.getCurrentUserDepartmentFromShiroSession().getDeptCode());
						tr.setUpdateTime(new Date());
						tr.setUpdateUser(ShiroSessionUtils.getCurrentUserFromShiroSession().getAccount());
						refundDAO.save(tr);
					}
					ter.setExtFlag1("0");
					ter.setUpdateTime(new Date());
					ter.setUpdateUser(ShiroSessionUtils.getCurrentUserFromShiroSession().getAccount());
					refundDAO.update(ter);
			...
			if(invoiceInfoLists.size()>0){
				payKindCode = invoiceInfoLists.get(0).getPaykindCode();
				payKindName = invoiceInfoLists.get(0).getPaykindName();
				for(FinanceInvoiceInfoNow info : invoiceInfoLists){
					FinanceInvoiceInfoNow invoiceInfo = new FinanceInvoiceInfoNow();
					BeanUtils.copyProperties(info,invoiceInfo);
					//修改原发票结算表
			...
			if(stoRecipeList.size()>0){
				for(StoRecipeNow sto : stoRecipeList){
					...
					sto.setModifyFlag(1);
					sto.setValidState(0);
					sto.setCancelDate(new Date());
					sto.setCancelOper(ShiroSessionUtils.getCurrentUserFromShiroSession().getAccount());
					sto.setCancelOperName(ShiroSessionUtils.getCurrentUserFromShiroSession().getName());
			...
			List<OutpatientFeedetailNow> hadSendDrugList = refundDAO.findFeedetailByInvoiceNos(invoiceNos,parameter.getParameterValue(),feedetailList.get(0).getClinicCode());
			
			Gson gson = new Gson();
			List<RefundDrugList> drugListVo = new ArrayList<RefundDrugList>();;
			List<RefundDrugList> undDrugListVo = new ArrayList<RefundDrugList>();
			if(refundDrugList.length()>2){
				drugListVo = gson.fromJson(refundDrugList, new TypeToken<List<RefundDrugList>>(){}.getType());
			}
			if(refundUnDrugList.length()>2){
				undDrugListVo = gson.fromJson(refundUnDrugList, new TypeToken<List<RefundDrugList>>(){}.getType());
			}
			String billCode = refundDAO.getSeqByNameorNum("SEQ_INPATIENT_CAN_BILLCODE", 14);
			
			if(undDrugListVo!=null&&undDrugListVo.size()>0){
				for(RefundDrugList undDrugListVos :undDrugListVo){
					drugListVo.add(undDrugListVos);
				}
			}
			//未退药的集合 包含已发药药品
			List<OutpatientFeedetailNow> feedetailListBT = new ArrayList<OutpatientFeedetailNow>();
			//待退药合集
			List<OutpatientFeedetailNow> returnfeedetailList = new ArrayList<OutpatientFeedetailNow>();
			...
			for (OutpatientFeedetailNow f : feedetailList) {
				...
				if(drugListVo.size()>0){
					for (RefundDrugList drugVo : drugListVo) {
						if(f.getId().equals(drugVo.getId())){//判断是否是待退项目
							flag = false;
							OutpatientFeedetailNow feedetail = refundDAO.queryFeedetailById(f.getId());
							OutpatientFeedetailNow fee = new OutpatientFeedetailNow();
							BeanUtils.copyProperties(feedetail,fee);
							fee.setNobackNum(drugVo.getNobackNum());//这里代表待退数量，生成退费申请信息的时候使用，切记是待退数量
							returnfeedetailList.add(fee);
							if(fee.getConfirmNum()>0){//医技已确认项目生成新纪录
								OutpatientFeedetailNow cfmfee = new OutpatientFeedetailNow();
								BeanUtils.copyProperties(feedetail,cfmfee);
								cfmfee.setId(null);
								cfmfee.setCancelFlag(1);
								cfmfee.setQty(feedetail.getConfirmNum());
								cfmfee.setNobackNum(0D);
								cfmfee.setConfirmFlag(1);
								cfmfee.setConfirmNum(feedetail.getConfirmNum());
								cfmfee.setTotCost(feedetail.getConfirmNum()*feedetail.getUnitPrice());
								cfmfee.setPayFlag(1);
								refundDAO.save(cfmfee);
							}
							clinicCode = feedetail.getClinicCode();//门诊号获取
							if(f.getQty()-drugVo.getNobackNum()-fee.getConfirmNum()>0){
								OutpatientFeedetailNow feedetailNew = new OutpatientFeedetailNow();
								BeanUtils.copyProperties(feedetail,feedetailNew);
								feedetailNew.setId(null);
								feedetailNew.setQty(feedetail.getQty()-drugVo.getNobackNum()-feedetail.getConfirmNum());//数量
								feedetailNew.setNobackNum(feedetail.getNobackNum()-drugVo.getNobackNum()-feedetail.getConfirmNum());//可退数量
								feedetailNew.setConfirmNum(0D);//确认数量
								feedetailNew.setTotCost((feedetail.getQty()-drugVo.getNobackNum()-feedetail.getConfirmNum())*f.getUnitPrice());//费用金额
								feedetailNew.setCancelFlag(1);
//								 * feedetailNew.setFlay(feedetail.getId())
//								 * 给该字段赋值并没有是什么实际意义，只是为了判断是否发药
//								 * 当为空时表示已发药，不为空表示位发药，只是为了一个状态，数据库没有相应的字段
								feedetailNew.setFlay(feedetail.getId());
					...
					if(flag){//未退药品包括已发药药品
						...
						for (OutpatientFeedetailNow has : hadSendDrugList) {
							if(StringUtils.isNotBlank(has.getFlay())&&feedetail.getId().equals(has.getId())){
			...
			for(OutpatientFeedetailNow fee:returnfeedetailList){
				//退费记录
				if("1".equals(fee.getDrugFlag())){//药品退费
					this.drugCancelItemEntity(fee, fee.getInvoiceNo(), fee.getClinicCode(), registration, billCode);
				}else{
					//非药品退费
					this.undrugCancelItemEntity(fee, fee.getInvoiceNo(), fee.getClinicCode(), registration, billCode);
					//物资退库
					OutputInInterVO out = new OutputInInterVO();;
					out.setApplyNum(fee.getQty());
					out.setFlag("I");
					out.setInpatientNo(fee.getClinicCode());
					out.setOutListCode(fee.getSampleId());
					out.setRecipeNo(fee.getRecipeNo());
					out.setSequenceNo(fee.getSequenceNo());
					out.setStorageCode(fee.getExecDpcd());
					out.setTransType(2);
					out.setUndrugItemCode(fee.getItemName());
					out.setUseNum(fee.getConfirmNum());
					matOutPutInInterService.addRecord(out);
				}
			}
...
//					String wondowT = this.stoRecipe(feedetail, recipeN, clicnicCode, invoiceNo, registration, employee, department, user2);
					String execDeptCode = "";
					...
					if(StringUtils.isNotBlank(stoRecipe.getSendTerminal())){
...
//						//门诊收费处理
	...
	public void drugCancelItemEntity(OutpatientFeedetailNow fee,String invoiceNo,String clinicCode,RegistrationNow registration,String billCode){
		...
		cancelitem.setHospitalId(HisParameters.CURRENTHOSPITALID);//当前医院id
		//获取当前科室信息
		SysDepartment dept = ShiroSessionUtils.getCurrentUserLoginDepartmentFromShiroSession();
		String deptCode=dept.getDeptCode();//
		cancelitem.setAreaCode(refundDAO.getDeptArea(deptCode));//保存当前科室所在院区
		//保存院区和
		refundDAO.save(cancelitem);
...
//					OutpatientFeedetailNow  feedetailLists = medicineListInInterDAO.queryDrugInfoList(fee.getId());
	...
	public List<InpatientCancelitemNow> getCancelItemByInvoiceNo(String invoiceNo) {
		return refundDAO.getCancelItemByInvoiceNo(invoiceNo);
	}

	@Override
	public String checkISsend(String moorder) {
		return refundDAO.checkISsend(moorder);
	}
