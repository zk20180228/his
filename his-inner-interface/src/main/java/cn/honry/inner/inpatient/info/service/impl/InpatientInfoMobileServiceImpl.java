...
@Service("inpatientInfoMobileterService")
@Transactional
@SuppressWarnings({"all"})
public class InpatientInfoMobileServiceImpl implements InpatientInfoMobileterService{
	...
	**/
	@Override
	public Map<String, Object> saveInpatientFeeInfo(List<FeeInInterVo> newfeeVoList,String empJobNo,String deptCode){
		...
		String account = empJobNo;//获取当前的登录人	
		...
		for(Entry<String, List<FeeInInterVo>> entry: feeMap.entrySet()){
			...
			for(Entry<String, List<FeeInInterVo>> ent: feeCodeMap.entrySet()){
				...
				for (FeeInInterVo feeInVo : list) {
					...
					if("1".equals(feeInVo.getTy())){//药品
						InpatientMedicineListNow inpatientMedicineList = getMedicineList(feeInVo,empJobNo,deptCode);//药品明细
						...
						if(feeInVo.getFeeFlag()==1){//收费
							...
							inpatientMedicineList.setFeeoperDeptcode(deptCode);// 收费员科室
						}
						inpatientMedicineList.setCreateUser(account);//创建人
						inpatientMedicineList.setCreateTime(date);//创建时间
						inpatientMedicineList.setCreateDept(deptCode);//创建科室
						inpatientMedicineList.setHospitalId(HisParameters.CURRENTHOSPITALID);
						inpatientMedicineList.setAreaCode(inpatientInfoDAO.getDeptArea(deptCode));
						inpatientInfoDAO.save(inpatientMedicineList);
					}
					if("2".equals(feeInVo.getTy())){//非药品
						
						InpatientItemListNow inpatientItemList = getItemList(feeInVo,empJobNo);//非药品明细
						...
						if(feeInVo.getFeeFlag()==1){//收费
							...
							inpatientItemList.setFeeoperDeptcode(deptCode);// 收费员科室
						}
						inpatientItemList.setCreateUser(account);//创建人员
						inpatientItemList.setCreateTime(date);//创建时间
						inpatientItemList.setCreateDept(deptCode);//创建科室
						inpatientItemList.setHospitalId(HisParameters.CURRENTHOSPITALID);
						inpatientItemList.setAreaCode(inpatientInfoDAO.getDeptArea(deptCode));
...
//		
		...
		for (FeeInInterVo  feeInVo: feeVoList) {
			...
			if(inpatientFeeInfoList1!=null&&inpatientFeeInfoList1.size()>0){//汇总表中存在该处方号
				...
				if(feeInVo.getFeeFlag()==1){//收费
					...
					inpatientFeeInfo.setFeeoperDeptcode(deptCode);//收费员科室 
				}
				inpatientFeeInfo.setCreateUser(account);
				inpatientFeeInfo.setCreateDept(deptCode);
			...
			if(StringUtils.isNotBlank(inpatientFeeInfo.getId())){//id不为空即update操作
				...
				inpatientFeeInfo.setAreaCode(inpatientInfoDAO.getDeptArea(deptCode));
	...
	* @date 2016年5月16日下午5:48:02
	 */
	public Map<String,Object> reverseTran(List<FeeInInterVo> feeVoList,String empJobNo,String deptCode){
				...
				for (FeeInInterVo vo : list){			
					...
					if("1".equals(vo.getTy())){	//药品
						...
						if(inpatientMedicineLists!=null&&inpatientMedicineLists.size()>0){
							...
							inpatientMedicine.setCreateDept(deptCode);
...
//								medicineList.setRecipeNo(newRecipeNo);//新的处方号
								...
								medicineList.setFeeoperDeptcode(deptCode);// 收费员科室
								medicineList.setCreateUser(account);//创建人
								medicineList.setCreateDept(deptCode);//创建科室
					...
					if("2".equals(vo.getTy())){//非药品
						...
						if(inpatientItemLists!=null&&inpatientItemLists.size()>0){
							...
							inpatientItem.setCreateDept(deptCode);
...
//								m++;
								...
								itemList.setFeeoperDeptcode(deptCode);// 收费员科室
								itemList.setCreateUser(account);//创建人
								itemList.setCreateDept(deptCode);//创建科室
				...
				if(inpatientFeeInfoList!=null&&inpatientFeeInfoList.size()>0){
					...
					if(sum>0){
						...
						feeInfo.setCreateDept(deptCode);
...
//					}
	 ...
	 * @return
	 */
 	private InpatientMedicineListNow getMedicineList(FeeInInterVo feeInVo,String empJobNo,String deptCode){
		...
		inpatientMedicineList.setRecipeDocCode(empJobNo);//开立医师代码 
		
		inpatientMedicineList.setRecipeDocname(user.getName());//开立医师名称
...
//		inpatientMedicineList.setExtDate(null);// 扩展日期 
	 ...
	 * @return
	 */
	private InpatientItemListNow getItemList(FeeInInterVo vo,String empJobNo){
		...
		inpatientItemList.setRecipeDoccode(empJobNo);//开立医师代码 
		
		inpatientItemList.setRecipeDocname(user.getName());//开立医师名称 
...
//		inpatientFeeInfo.setExtFlag2(null);//扩展标志2
	...
	@Override
	public void saveOrUpdate(InpatientInfoNow arg0) {
