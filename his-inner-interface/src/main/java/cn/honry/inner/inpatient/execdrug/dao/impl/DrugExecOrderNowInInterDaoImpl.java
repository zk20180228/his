...
import cn.honry.inner.inpatient.execdrug.dao.DrugExecOrderNowInInterDao;
import cn.honry.utils.ShiroSessionUtils;


@Repository("drugExecOrderNowInInterDao")
@SuppressWarnings({"all"})
public class DrugExecOrderNowInInterDaoImpl extends HibernateEntityDao<InpatientExecdrugNow> implements DrugExecOrderNowInInterDao {
	 ...
	 */
	@Override
	public List<InpatientExecdrugNow> getPageInfo(String hql, String page,String rows) {
	 ...
	 */
	@Override
	public List<InpatientExecdrugNow> getDrugListByIdsAndPatNo(String orderIds,String patNoData) {
		if(StringUtils.isBlank(orderIds)&&StringUtils.isBlank(patNoData)){
			return null;
		}
		StringBuffer buffer = new StringBuffer();
		buffer.append("from InpatientExecdrugNow e ");
		buffer.append("WHERE ");
		...
		List<InpatientExecdrugNow> list = this.find(buffer.toString(), null);
