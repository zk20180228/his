...
@Service("docManageServiceApp")
@Transactional
@SuppressWarnings({ "all" })
public class DocManageServiceAppImpl implements DocManageServiceApp{
	/**
     * 缓存类
     */
    @Resource
	...
	@Override
	public Map<String,Object> saveOrUpdateInpatientOrder(InpatientOrderNow entity,String str, String empJobNo, String deptCode) throws Exception {
...
//		User user = (User)SessionUtils.getCurrentUserFromShiroSession();//获取登录人
		EmployeeExtend emp = employeeInInterService.getEmpExtend(empJobNo);
		Date d = new Date();
		...
		try{
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				if("".equals(jsonObj.getString("id"))){
					...
					if("1".equals(itemType)){
						inpatientOrder.setPharmacyCode(deptCode);//取药药房、扣库科室
					}			
					...
					inpatientOrder.setDcDoccd(emp.getEmployeeJobNo());//开立医师
					inpatientOrder.setDcDocnm(emp.getEmployeeName());//开立医师名称
					inpatientOrder.setPackageCode(null);//组套编码
					inpatientOrder.setPackageName(null);//组套名称
					inpatientOrder.setDocCode(empJobNo);	//开立医师代码 -hedong 20160829 改存工号
					inpatientOrder.setDocName(emp.getEmployeeName());//开立医师名字
					...
					inpatientOrder.setRecUsercd(empJobNo);//录入人员代码-hedong 20160829 改存工号
					inpatientOrder.setRecUsernm(emp.getEmployeeName());//录入人员名称
					inpatientOrder.setExecTimes(execTimes);//执行时间点[特殊频次]
					inpatientOrder.setExecDose(execDose);//执行剂量[特殊频次]
					inpatientOrder.setId(null);			
					inpatientOrder.setCreateUser(emp.getEmployeeJobNo());
					inpatientOrder.setCreateDept(deptCode);//创建科室
					inpatientOrder.setCreateTime(DateUtils.getCurrentTime());//创建时间
					inpatientOrder.setUpdateUser(emp.getEmployeeJobNo());
					...
					if("1".equals(itemType)){
						inpatientOrder.setPharmacyCode(deptCode);//取药药房
					}
					Map<String,String> implDepartment = docAdvManageDAO.queryImplDepartment(entity.getDeptCode());
					String deptName =implDepartment.get(entity.getDeptCode());
					if(StringUtils.isNotBlank(execDpcd)){
				...
				if(odditionalitem!=null && odditionalitem.size()>0){//保存附材医嘱
					for(int h=0;h<odditionalitem.size();h++){
						...
						oddInpatientOrder.setCreateUser(emp.getEmployeeJobNo());//创建人
						oddInpatientOrder.setCreateDept(deptCode);//创建科室
