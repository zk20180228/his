...
UE.plugins['text'] = function () {
	...
	popup.render();
	 me.addListener( 'mouseover', function( t, evt ) {
	        ...
	        if(el.innerHTML.substring(0,1)=="{"){
		        ...
		        if ( /span/ig.test( hidEl.tagName ) && leipiPlugins==thePlugins) {
		            var html = popup.formatHtml(
		                '<nobr>设置文本框控件: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
		            if ( html ) {
		                var rEl = el;
	...
	UE.plugins['number'] = function () {
		...
		me.commands[thePlugins] = {
			execCommand:function () {
				var dialog = new UE.ui.Dialog({
					iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/number.html',
					name:thePlugins,
					editor:this,
					title: '数字',
					cssRules:"width:600px;height:310px;",
		...
		popup.render();
		 me.addListener( 'mouseover', function( t, evt ) {
		        ...
		        if(el.innerHTML.substring(0,1)=="{"){
			        ...
			        if ( /span/ig.test( hidEl.tagName ) && leipiPlugins==thePlugins) {
			            var html = popup.formatHtml(
			                '<nobr>设置数字控件: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
			            if ( html ) {
			                var rEl = el;
			                popup.getDom( 'content' ).innerHTML = html;
			                popup.anchorEl = el;
		...
		UE.plugins['date'] = function () {
			...
			me.addListener( 'mouseover', function( t, evt ) {
				...
				if(el.innerHTML.substring(0,1)=="{"){
					...
					if ( /span/ig.test( hidEl.tagName ) && leipiPlugins==thePlugins) {
						var html = popup.formatHtml(
						'<nobr>设置时间控件: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
						if ( html ) {
							var rEl = el;
							popup.getDom( 'content' ).innerHTML = html;
							popup.anchorEl = el;
...
UE.plugins['radios'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        if(el.innerHTML.substring(0,1)=="{"){
	        ...
	        if ( /span/ig.test( hidEl.tagName ) && leipiPlugins==thePlugins) {
	            var html = popup.formatHtml(
	                '<nobr>设置单选控件: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
	            if ( html ) {
	                var rEl = el;
	                popup.getDom( 'content' ).innerHTML = html;
	                popup.anchorEl = el;
...
UE.plugins['checkboxs'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        if(el.innerHTML.substring(0,1)=="{"){
	        ...
	        if ( /span/ig.test( hidEl.tagName ) && leipiPlugins==thePlugins) {
	            var html = popup.formatHtml(
	                '<nobr>设置多选控件: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
	            if ( html ) {
	                var rEl = el;
	                popup.getDom( 'content' ).innerHTML = html;
	                popup.anchorEl = el;
...
UE.plugins['yesornoselect'] = function () {
    ...
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/yesornoselect.html',
                name:thePlugins,
                editor:this,
                title: '有无选择组',
                cssRules:"width:600px;height:400px;",
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        if(el.innerHTML.substring(0,1)=="{"){
	        ...
	        if ( /span/ig.test( hidEl.tagName ) && leipiPlugins==thePlugins) {
	            var html = popup.formatHtml(
	                '<nobr>设置有无选控件: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
	            if ( html ) {
	                var rEl = el;
	                popup.getDom( 'content' ).innerHTML = html;
	                popup.anchorEl = el;
...
UE.plugins['textarea'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        if(el.innerHTML.substring(0,1)=="{"){
	        ...
	        if ( /span/ig.test( hidEl.tagName ) && leipiPlugins==thePlugins) {
	            var html = popup.formatHtml(
	                '<nobr>设置多行文本框控件: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
	            if ( html ) {
	                var rEl = el;
	                popup.getDom( 'content' ).innerHTML = html;
	                popup.anchorEl = el;
