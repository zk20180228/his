...
<div data-options="region:'north',border:false" style="width: 100%;height:50px;padding-top: 10px;">
		<form id="orderForm" style="padding: 5px 20px;">
          操作类型：<input class="easyui-combobox" id="queryName" value="核准入库" style="width:150px">&nbsp;&nbsp;&nbsp;&nbsp;
</form>	
	</div>
...
$(function(){
	...
	$('#grid1').datagrid({
		url:'<%=basePath%>material/orderInput/approvalList.action?t2.inState=1'+listParam,
		pageSize:20,
		...
		onDblClickRow:function(rowIndex, rowData){//双击赋值
			...
			if (row) {
				...
				if(typeof(row.companyCode)=='undefined'){ 
					$.messager.alert("提示", '"'+row.itemName+'"的供货公司为空不能核准,请选择其他物资', "info");
					return;
				}
				//判断记录是否已经添加到右侧列表中
