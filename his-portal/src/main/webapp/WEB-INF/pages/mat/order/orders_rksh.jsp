...
<div data-options="region:'north'" style="width: 38%; height:50px;padding-top: 10px;border-top:0">
		<form id="orderForm" style="padding: 5px 20px;">
          操作类型：<input class="easyui-combobox" id="queryName" value="入库审核" style="width:150px">&nbsp;&nbsp;
    ...
    <div data-options="region:'north',border:false" style="width:100%;height: 88px">
    	...
    	物资编号: <input class="easyui-textbox" name="t1.itemCode" style="width:120px"/>
		物资名称: <input class="easyui-textbox" name="t1.itemName" style="width:120px">
		发票号码: <input class="easyui-textbox" name="t1.invoiceNo" style="width:120px"><br/><br/>
		发票日期: <input class="Wdate" onClick="WdatePicker()" name="t1.invoiceDate" style="width:120px" editable="false">&nbsp;&nbsp;至&nbsp;
		<input class="Wdate" onClick="WdatePicker()" name="t2.invoiceDate" style="width:125px" editable="false">&nbsp;&nbsp;
		<button type="button" id="btnSearch" class="easyui-linkbutton" iconCls="icon-search"> 查询</button>
    	</form>
    </div>
	<div data-options="region:'center'" style="width:100%;height: 79%;border-right:0">
...
$(function(){
	...
	$('#grid1').datagrid({
		url:'<%=basePath%>material/orderInput/checkList.action?t1.inState=0'+listParam,
		pageSize:20,
		...
		columns:[[
		          ...
		          {field:'invoiceDate',title:'发票日期',width:'11%'},
		...
		onDblClickRow:function(rowIndex, rowData){//双击赋值
			...
			if (row) {
				 //发票号为空时提示
				if(typeof(row.invoiceNo)=='undefined'){ 
					$.messager.alert("提示", '"'+row.itemName+'"的发票号为空无法审核,请选择其他物资', "info");
					return;
				}
				//判断记录是否已经添加到审批列表中
	...
	$('#grid2').datagrid({
		columns:[[
		          ...
		          {field:'inListCode',title:'入库单号',width:'8%'},
		          {field:'inSerialNo',title:'单内序号',width:'8%'},
		          ...
		          {field:'inNum',title:'审批数量',width:'10%',
		        	  editor:{
		        		  type:'numberbox',
		        		  options:{
		        			  required:true,
 ...
 */
 function deleteRow(){
	...
	function bindGridEvent() {
		var numEdt = $('#grid2').datagrid('getEditor', {
	...
	function cal() {
		...
		//获取价格的编辑框
		var priceEdt = $('#grid2').datagrid('getEditor', {
			index : isEditingRowIndex,
			field : 'inPrice'
		});
		//得到价格编辑框里的值
		...
		//获取金额的编辑框
		var moneyEdt = $('#grid2').datagrid('getEditor', {
			index : isEditingRowIndex,
			field : 'inCost'
		});
		//对金额编辑框赋值
