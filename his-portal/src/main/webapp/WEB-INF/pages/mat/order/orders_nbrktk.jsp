...
<div class="easyui-layout" style="width: 100%; height: 100%;"data-options="fit:true">
	<div data-options="region:'north',border:false" style="width: 100%; height:50px;padding-top: 10px;">
		<form id="orderForm" style="padding: 5px 20px;">
          操作类型：<input class="easyui-combobox" id="queryName" value="内部退库申请" style="width:150px">&nbsp;&nbsp;
    ...
    <div data-options="region:'center',title:'退库申请'" style="background:#eee;">
    <div id="tb">
...
$(function(){
	...
	$('#queryStorage').combobox({
		...
		onSelect: function(rec){
			var url='<%=basePath%>material/orderInput/inReturn.action?t1.stop_flg=0&deptId='+rec.objectDeptCode;
		...
		onDblClickRow:function(rowIndex, rowData){//双击赋值
			...
			if (row) {
				...
				$('#grid2').datagrid('appendRow', {
					...
					inPrice:row.inPrice,
					packQty:row.packQty,
					packInNum:row.storeNum,//该字段仅用于判断申请退回数量与当前库存数量的大小
					packPrice:row.inPrice,
	...
	$('#grid2').datagrid({
		columns:[[
		          ...
		          {field:'inNum',title:'退回数量(包装数)',width:'12%',align:'right',
		        	  editor:{
		        		  type:'numberbox',
		        		  options:{
		        			  required:true,
		        			  validType:'minValue',
		        			  missingMessage:'退回数量不能为空',
		        			  invalidMessage:'退回数量须大于0'
		        			  }
		          }},
		          {field:'packUnit',title:'大包装单位',width:'10%'},
		          {field:'inPrice',title:'入库价格',width:'15%',align:'right'},
		          ...
		          {field:'packInNum',title:'库存数量',hidden:true},
		          {field:'id',title:'ID',hidden:true}
		          ]],
		 toolbar:'#tb',
...
function saveData(){
	...
	for (var i = 0; i < rows.length; i++) {
		...
		if(!f){
			...
			if(rows[i].applyNum>rows[i].packInNum){
				$.messager.alert("提示", "申请退回数量不能大于库存数量！", "error");
				$('#grid2').datagrid('uncheckAll');
				$('#grid2').datagrid('checkRow',i);
				return;
			}
	...
	$.messager.confirm('提示','确定要提交数据吗?',function(res){
		...
		$.ajax({
			url:'<%=basePath%>material/orderInput/returnInput.action',
			data:formdata,
			type:'post',
			dataType:'json',
