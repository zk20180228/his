...
<div data-options="region:'north',border:false" style="width: 100%; height:50px;padding-top: 10px;">
	<form id="orderForm" style="padding: 5px 20px;">
	          操作类型：<input class="easyui-combobox" id="queryName" value="审批出库" style="width:150px">&nbsp;&nbsp;
    ...
    <div  data-options="region:'west',title:'申请列表',iconCls:'icon-book',split:true" style="width:600px ;">
			<div class="easyui-layout" style="width: 100%; height: 100%;" data-options="fit:true">
				<div data-options="region:'north',border:false" style="width: 100%; height: 60px">
					<form id="searchForm" style="width: 100%; height: 100%" data-options="fit:true"><br>
...
<!-- 					<input type="text" name="t1.outClass3mean" value="1"> -->
				</div>
...
$(function(){
	...
	$('#queryStorage').combobox({
		...
		onSelect:function(rec){
			var url='<%=basePath%>material/orderOutput/checkList.action?t1.outState=0&t1.outClass3mean=1&deptId='+rec.deptCode;
	...
	$('#grid1').datagrid({
		url:'<%=basePath%>material/orderOutput/checkList.action?t1.outState=0&t1.outClass3mean=1'+listParam,
		pageSize:20,
		...
		columns:[[
		          ...
		          {field:'itemName',title:'物品名称',width:'14%'},
		          ...
		          {field:'storageCode',title:'仓库编码',width:'28%'},
		...
		onDblClickRow:function(rowIndex, rowData){//双击赋值
			...
			if (row) {
				...
				if(rs.length>0){
					...
					if(i!=-1){
						$.messager.alert("提示", "审批列表已有此条记录！","warning");
				...
				$('#grid2').datagrid('appendRow', {
					...
					specs:row.specs,
					outNum:row.outNum,
					packQty:row.outNum,//此字段仅用于接收申请数量以判断申请数量与审批数量的大小
	...
	$('#grid2').datagrid({
		columns:[[
		          ...
		          {field:'outPrice',title:'出库价格',width:'10%',editor:{type:'numberbox',options:{precision:2,disabled:true}}},
		          {field:'packQty',title:'申请数量',width:'10%'},
		          {field:'outNum',title:'审批数量',width:'10%',
		        	  editor:{
		        		  type:'numberbox',
		        		  options:{
		        			  required:true,
		          ...
		          {field:'specs',title:'规格',width:'10%'},
		          {field:'outCost',title:'金额',width:'10%',editor:{type:'numberbox',options:{precision:2,disabled:true}}},
		          {field:'applyNo',title:'申请单号',hidden:true},
		          {field:'id',title:'ID',hidden:true},
		          ]],
		toolbar:'#tb',
...
function saveData(){
	...
	$.messager.confirm('提示','确定要审批吗?',function(res){
		if(res){
			...
			$.ajax({
				url:'<%=basePath%>material/orderOutput/outputCheck.action',
				data:formdata,
				type:'post',
				dataType:'json',
