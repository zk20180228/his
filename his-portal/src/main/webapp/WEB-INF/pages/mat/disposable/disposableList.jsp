...
<body>
	...
	$(function() {
		...
		$('#list').datagrid({
			...
			onDblClickRow : function(rowIndex, rowData) {//双击查看
                ...
                if(row){
                	AddOrShowEast("查看",'<%=basePath %>mat/disposable/viewMatInput.action?id='+rowData.id);
			   	}
			}
		});
	...
	function add(){
		addAndEdit = 0;
		closeLayout();
		AddOrShowEast('EditForm', '<%=basePath %>mat/disposable/saveDisposableUrl.action');
	}
	//修改
	function edit(){
		...
		if (rows != null && rows.length != 0) {
			addAndEdit = 1;
			closeLayout();
			AddOrShowEast('EditForm', '<%=basePath %>mat/disposable/updateDisposableUrl.action?id=' + getIdUtil('#list'));
		}else{
            ...
            if (rows.length > 0) {//选中几行的话触发事件	                        
			 	$.messager.confirm('确认', '确定要删除选中信息吗?', function(res){//提示是否删除
					if (res){
						...
						$.ajax({
							url: '<%=basePath %>mat/disposable/deleteDisposable.action?id='+ids,
							type:'post',
...
<body>
	<div id="divLayout" class="easyui-layout" fit=true>
			...
			<div data-options="region:'center',split:false,border:false" style=" width: 100%;">
				...
				<table id="list" style="height: 557px;" data-options="fit:true,url:'${pageContext.request.contextPath}/mat/disposable/queryMatInput.action?menuAlias=${menuAlias}',method:'post',rownumbers:true,idField: 'id',striped:true,border:true,checkOnSelect:true,selectOnCheck:false,singleSelect:true,fitColumns:false,toolbar:'#toolbarId'">
					<thead>
						<tr>
							<th field="ck" checkbox="true"></th>
							<th data-options="field:'inListCode', width : 150">
								入库单号
							</th>
							<th data-options="field:'inSerialNo', width : 150">
								单内序号
							</th>
							<th data-options="field:'storageCode', width : 150">
								入库科室
							</th>
							<th data-options="field:'kindCode', width : 150">
								物品科目
							</th>
							<th data-options="field:'itemName', width : 150">
								物品名称
							</th>
							<th data-options="field:'inNum', width : 100">
								入库数量
							</th>
							<th data-options="field:'inDate', width : 200">
								入库时间
							</th>
						</tr>
					</thead>
