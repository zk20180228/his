		...
		<div>
			<table cellspacing="0" cellpadding="0" border="0">
				<tr>
					<td style="padding: 5px 15px;" nowrap="nowrap" >
						...
						<input class="easyui-combobox" id="queryName" data-options="valueField: 'id', textField: 'value', 
							data: [{ id: '01', value: '一般入库' },{ id: '02', value: '发票入库' },{id: '03', value: '核准入库' },{id: '04', value: '入库退货' },
									   {id: '05', value: '内部入库申请',selected:true},{id: '06', value: '内部入库核准'},{id: '07', value: '内部入库退库申请'},
					...
					<td>
						&nbsp;&nbsp;库房：
						<input class="easyui-combobox" id="queryTree" style="width:180px"/>
					</td>
<!-- 					<td>&nbsp;&nbsp;&nbsp;&nbsp; -->
		...
		<div id="layoutId" class="easyui-layout" data-options="fit:true"> 
			<div data-options="region:'west',title:'物资列表',collapsible:false" style="padding:5px; width:35% ;">
				...
				<div style="padding:5px,5px,5px,5px;">
					<table id="list" data-options="url:'${pageContext.request.contextPath}/material/instore/queryMatInsideStorageApply.action',method:'post',rownumbers:true,idField: 'id',striped:true,border:true,checkOnSelect:true,selectOnCheck:false,singleSelect:true,fitColumns:false">
						<thead>
							<tr>
								<th data-options="field:'ck',checkbox:true"></th>
								<th data-options="field:'itemName',width:100">物品名称</th>
								<th data-options="field:'specs',width:150">规格</th>
								<th data-options="field:'inNo',width:100">入库单价</th>
								<th data-options="field:'salePrice',width:100">零售价格</th>
								<th data-options="field:'storeNum',width:100">库存数量</th>
								<th data-options="field:'minUnit',width:100">最小单位</th>
								<th data-options="field:'packUnit',width:100">大包装单位</th>
								<th data-options="field:'packQty',width:100">大包装数量</th>
								<th data-options="field:'highvalueBarcode',width:200">高值耗材条形码 </th>
							</tr>
						</thead>
					</table>
				</div>
				...
				<table id="infolist" data-options="method:'post',rownumbers:true,striped:true,border:true,checkOnSelect:true,selectOnCheck:false,singleSelect:true,fitColumns:false,toolbar:'#toolbarId'">
					<thead>
							<tr>
								...
								<th data-options="field:'applyNum',width:100,editor:{type:'numberbox',options:{required:true}}">申请数量</th>
								<th data-options="field:'minUnit',width:100,editor:{type:'combobox',options:{required:true}}">单位</th>
								<th data-options="field:'minUnitH',width:100">最小单位</th>
								<th data-options="field:'packUnitH',width:100">包装单位</th>
								<th data-options="field:'applyCost',width:100">申请金额</th>
								<th data-options="field:'saleCost',width:100">申请零售金额</th>
								<th data-options="field:'packQty',width:100">包装数量</th>
								<th data-options="field:'applyPrice',width:100">购入价</th>
								<th data-options="field:'salePrice',width:100">零售价 </th>
								<th data-options="field:'storeSum',width:100">当前科室库存</th>
		...
		$(function(){
			$('#list').datagrid({
				...
				onDblClickRow: function (rowIndex, rowData) {//双击赋值
					$.ajax({
						url: "<c:url value='/material/instore/queryMatInsideStorageApplyIsValid.action'/>?id="+rowData.itemCode,
						type:'post',
						success: function(date) {
							var dateMap = eval("("+date+")");
							if(dateMap.resMsg=="success"){
								...
								var index = $('#infolist').datagrid('appendRow', {
									...
									applyNum : 1,//申请数量
									minUnit : rowData.minUnit,//单位
									minUnitH : rowData.minUnit,//最小单位
									packUnitH : rowData.packUnit,//包装单位
									applyCost : rowData.inPrice,//申请金额
									saleCost : rowData.salePrice,//申请零售金额
									packQty : rowData.packQty,//包装数量
									applyPrice : rowData.inPrice,//购入价
									salePrice : rowData.salePrice,//零售价 
									storeSum : rowData.storeNum,//当前科室库存
									memo : null//备注
								}).edatagrid('getRows').length - 1;
								$('#infolist').datagrid('selectRow', index);
								$('#infolist').datagrid('beginEdit', index);
							}else{
								$.messager.alert('提示',dateMap.resCode);	
							}
						}
					});
				}
			...
			$('#queryTree').combobox({    
			    url:"<c:url value='/material/instore/queryMatInsideStorageApplyTree.action'/>",    
			    valueField:'id',    
			    textField:'text'   
			}); 
		...
		function save(){
			...
			if(rows!=null&&rows.length>0){
				...
				jQuery.messager.confirm("提示","确定保存所选申请信息？",function(event){
					if(event){
						$.post("<c:url value='/material/instore/queryMatInsideStorageApplySave.action'/>",
				        		...
				        		jQuery.messager.confirm("提示","保存成功，是否打印申请单？",function(event){
				        			if(event){
				        				$.messager.alert('提示','该功能正在建设中！','',function(){
				        					window.location.href="<c:url value='/material/instore/listMatInsideStorageApply.action?menuAlias=WZRK'/>";
				        				});
				        			}else{
				        				window.location.href="<c:url value='/material/instore/listMatInsideStorageApply.action?menuAlias=WZRK'/>";
				        			}
