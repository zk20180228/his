...
$(function(){
	...
	//加载list列表
	$('#list').datagrid({
		url: '<%=basePath %>material/applyLibrary/findMatStockdetail.action',
		pagination: true,
...
function save(){
	...
	$.ajax({
		url: '<%=basePath %>material/applyLibrary/verificationMatStockdetailSave.action?jsonString='+jsonString+'&deptC='+deptC,
		type:'post',
		success: function(dataMap) {
			...
			if(dataMapObj.resMsg=="error"){
				...
				$.ajax({
					url: '<%=basePath %>material/applyLibrary/saveMatApply.action?jsonString='+jsonString+'&deptC='+deptC+'&deptR='+deptR,
					type:'post',
					success: function(data) {
						if(data=="ok"){
							$.messager.alert("操作提示", "出库成功");
							window.location.href="<c:url value='/material/applyLibrary/listApplyLibrary.action'/>?menuAlias=${menuAlias}";
						}else{
