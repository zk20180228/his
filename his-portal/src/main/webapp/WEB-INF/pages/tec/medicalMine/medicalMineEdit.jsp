		...
		<div class="easyui-panel" id="panelEast" style="border:0">
			<div style="padding:10px">
	    		<form id="editForm" method="post">
					...
					<input  name="model.modelClass"  type="hidden" value="${model.modelClass }"/>
					<table class="honry-table" cellpadding="0" cellspacing="0" border="0">
						<tr>
							<td class="honry-lable">排班分类：</td>
			    			<td class="honry-info">
								<c:if test="${model.modelClass==1}">
									<input id="modelClass" name="modelClass" value="项目排版模板" class="easyui-textbox" readonly="true" style="width: 200"/>
								</c:if>
		    					<c:if test="${model.modelClass==2}">
									<input id="modelClass" name="modelClass" value="设备排版模板" class="easyui-textbox" readonly="true" style="width: 200"/>
								</c:if>
			    			</td>
		    			</tr>
		    			<tr id="deptTrId" >
		    			...
		    			<tr id="doctorTrId" >
			    			...
			    			<input  id="modelDoctorHidden" name="model.modelItemName" class="easyui-textbox"  value="${model.modelItemName }" style="width: 200"/>
			    			</td>
		    			</tr  id="dateTrId">
							<td class="honry-lable" style="font-size: 14">日期：</td>
			    			<td class="honry-info">
			    			<input id="date" class="Wdate" type="text" name="model.date" value="${model.date }" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'2099-12-31'})" required = "true" style="width:150px;border: 1px solid #95b8e7;border-radius: 5px;"/>
			    			</td>
			    			<input type="hidden" id="tmpdate" value="${model.date }" />
		    			</tr>
						<tr id="middayTrId" >
						...
						<tr id="prelimitTrId" >
			    			...
			    			<td class="honry-info"><a href="javascript:void(0)" title="预约限额和特诊限额，只能是一个大于0，一个等于0！<br>最大值：999" data-options="position:'bottom'" class="easyui-tooltip">
			    			<input type="text" class="easyui-numberbox" id="modelPrelimit" name="model.modelPrelimit" value="${model.modelPrelimit }" data-options="required:true,min:0,max:999,precision:0,validType:['modelPrelimit','modelPreUpperlimit','maxValue[999]']" style="width: 200"/></a></td>
		    			</tr>
						<tr id="netlimitTrId"  >
							<td class="honry-lable">网络限额：</td>
			    			<td class="honry-info"><a href="javascript:void(0)" title="网络限额不能大于预约限额！<br>最大值：999" data-options="position:'bottom'" class="easyui-tooltip">
			    			<input type="text"  class="easyui-numberbox" id="modelNetlimit" name="model.modelNetlimit" value="${model.modelNetlimit }" data-options="required:true,min:0,max:999,precision:0,validType:['modelNetlimit','maxValue[999]']" style="width: 200"/></a></td>
		    			</tr>
						<tr id="speciallimitTrId"  >
							<td class="honry-lable">特诊限额：</td>
			    			<td class="honry-info"><a href="javascript:void(0)" title="预约限额和特诊限额，只能是一个大于0，一个等于0！<br>最大值：999" data-options="position:'bottom'" class="easyui-tooltip">
			    			<input type="text"  class="easyui-numberbox" id="modelSpeciallimit" name="model.modelSpeciallimit" value="${model.modelSpeciallimit }" data-options="required:true,min:0,precision:0,validType:['modelSpeciallimit','maxValue[999]']" style="width: 200"/></a></td>
		    			</tr>
						<tr id="appflagTrId"  >
...
<script type="text/javascript">
		...
		$(function(){
			...
			if(idVal!=null&&idVal!=''){
				setTimeout(function(){
// 					var modelClass = $('#modelss').val().trim();
// 					$('#modelClass').combobox('select',$.trim($('#modelss').val()));
					/*
					初始化工作科室框
					*/
// 					$("#modelWorkdeptText").combobox({
...
// 					});
					bindEnterEvent('modelWorkdeptText',popWinToDept,'easyui');//绑定回车事件 
	 				...
	 				$('#clinic').combobox({//查询诊室    
		    			url: "<c:url value='/technical/medicalMineplate/getClinic.action'/>?deptId="+$('#modelDeptid').val(),
					    valueField:'id',    
					    textField:'clinicName',
					    editable:false,
					    required:(modelClass==1)?true:false,
					    onLoadSuccess:function(none){
					    	$('#clinic').combobox('select',$('#modelClinicid').val());
					    }
					});
				},50);
			}
		...
		function submit(){ 
			...
			var id = $('#date').val();//获取日期选择框的值
			var date = $('#date').val();
			var pp = $('#tt').tabs('getSelected');
			var tab = pp.panel('options').tab; 
			var name = tab[0].innerText;//获取选项卡title
			var Starttime=$("#modelStarttime").timespinner("getValue");//开始时间
			var Endtime=$("#modelEndtime").timespinner("getValue");//结束时间
			var modelMidday=$("#modelMidday").combobox("getText");
			if(modelMidday=="晚上"){
// 				if(Endtime>="24:00"&&Starttime<Endtime){
// 					$.messager.alert("提示","结束时间必须大于开始时间");
// 					return ;
// 				}else if(Starttime<'24:00'&&Starttime>Endtime){
// 					$.messager.alert("提示","结束时间必须大于开始时间");
// 					return ;
...
// 					return ;
// 				}
				if(!(Starttime<='24:00'&&Endtime>"00:00")&&Starttime>Endtime){
					$.messager.alert("提示","结束时间必须大于开始时间");
					setTimeout(function(){
			...
			if(modelMidday=="上午"){
				...
				if(Starttime>"17:30"){
					$.messager.alert("提示","请选择下午的时间[<span  style='color:blue'>13:30-15:30</span>]");
					setTimeout(function(){
						$(".messager-body").window('close');
					},3500);
					return;
		    ...
		    $('#editForm').form('submit',{ 
		    	url: "<%=basePath%>/technical/medicalMine/savemedicalMine.action",
		    	date:{"date":date},
		        onSubmit:function(){
		        ...
		        success:function(data){ 
		        	...
		        	if(data.resMsg=="error"){
		        	...
		        	}else if(data.resMsg=="exist"){
		        		$.messager.alert("提示","该排班已存在,请重新填写!");
		        		setTimeout(function(){
		        			$(".messager-body").window('close');
		        		},3500);
		        		...
		        		$('#tt').tabs('select',name);
                    	var ids = id.split('-');
                    	var newid = ids[0]+ids[1]+ids[2];
                    	$('#'+newid).datagrid('reload');
		        	}
		        },
				error : function(data) {
					$.messager.progress('close');	
		...
		function addContinue(){ 
			var id = $('#date').datebox('getValue');//获取日期选择框的值
			var pp = $('#tt').tabs('getSelected');
			var tab = pp.panel('options').tab; 
			var name = tab[0].innerText;//获取选项卡title
			var Starttime=$("#modelStarttime").timespinner("getValue");//开始时间
...
// })
		 ...
		 */
		 var premessage = "网络限额大于预约限额！";
			...
			if($('#tmpdate').val()){
				var tmpVal = $('#tmpdate').val()
				var date = new Date(tmpVal);
				var mm = date.getMonth()+1<10?"0"+(date.getMonth()+1):(date.getMonth()+1);
				var dd = date.getDate()<10?"0"+date.getDate():date.getDate();
				var trueDate = date.getFullYear()+"-"+mm+"-"+dd;
				$('#date').val(trueDate);
			}
	</script>
	</body>
