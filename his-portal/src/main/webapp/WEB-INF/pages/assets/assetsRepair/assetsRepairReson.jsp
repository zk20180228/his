    	...
    	<div data-options="region:'center',border:false" ">
			<form id="editForm" method="post" enctype="multipart/form-data">
				<table class="honry-table" cellpadding="0" cellspacing="0" border="0" style="margin:10px auto 0;">
					...
					<input type="hidden" id="createUser" name="deviceDossier.createUser" value="${deviceDossier.createUser }">
					<input type="hidden" id="createDept" name="deviceDossier.createDept" value="${deviceDossier.createDept }">
					<input type="hidden" id="createTime" name="deviceDossier.createTime" value="${deviceDossier.createTime} }"/>
					<tr>
						<td class="honry-lable">维修内容：</td>
		    			<td class="honry-info"><input class="easyui-textbox" id="repairReson"
...
<script type="text/javascript">
	...
	function submit(){
		var repairReson = $('#repairReson').textbox('getValue');
		$.messager.progress({text:'保存中，请稍后...',modal:true});
		$('#editForm').form('submit', {
			url : "<c:url value='/assets/deviceDossier/saveRepairReson.action'/>",
			queryParams:{repairReson:repairReson},
			onSubmit : function() {
