	...
	$(function(){
		...
		$('#list').datagrid({
			...
			onDblClickRow: function (rowIndex, rowData) {
					...
					var tempWinPath = "<c:url value='/publics/infectious/viewInfectiousView.action'/>?id="+getIdUtil("#list")+"&save=true";
		...
		function add(){
			//添加datagrid事件及分页
			var tempWinPath = "<c:url value='/publics/infectious/addInfectious.action'/>?"+"&save=false";
			window.open (tempWinPath,'newwindow',' left=50,top=50,width='+ (screen.availWidth -150) +',height='+ (screen.availHeight-180) +',scrollbars,resizable=yes,toolbar=yes')
		}
		
		function edit(){
			if(getIdUtil("#list").length!=0){
				var tempWinPath = "<c:url value='/publics/infectious/viewInfectious.action'/>?id="+getIdUtil("#list")+"&save=false";
					...
					if (idss.length > 0) {//选中几行的话触发事件	                        
						$.messager.confirm('确认', '确定要删除选中信息吗?', function(res) {//提示是否删除
							if (res) {
								...
								$.ajax({
									url : "<c:url value='/publics/infectious/delInfectious.action'/>?ids="+ids,
									type : 'post',
									success : function() {
...
<body>
	<div id="divLayout" class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'center',split:false,title:'传染病患者列表',border:false">
			<div id="p" style="width:90%;height:30px;margin-top:auto;margin-bottom:auto;padding:5px;">
				<form id="editForm" >
			...
			<div id="q" style="width:100%;height:95%;">
				<table id="list" data-options="url:'${pageContext.request.contextPath}/publics/infectious/querylistinfectious.action?menuAlias=${menuAlias}',method:'post',rownumbers:true,idField: 'id',striped:true,border:true,checkOnSelect:true,selectOnCheck:false,singleSelect:true,toolbar:'#toolbarId',fit:true">
					<thead>
						<tr>
							<th  field="ck" checkbox="true" ></th>
							<th data-options="field:'reportType',formatter: formatreportNo" style="width: 20%">
								报卡类别
							</th>
							<th data-options="field:'patientName'" style="width: 16%">
								姓名
							</th>
							<th data-options="field:'workPlace'" style="width: 20%">
								工作单位
							</th>
							<th data-options="field:'reportSex',formatter: sexRend" style="width: 13%">
								性别
							</th>
							<th data-options="field:'approveOper'" style="width: 20%">
								审核人
							</th>
						</tr>
					</thead>
