...
<body>
	<div class="easyui-panel"  id="panelEast"  data-options="title:'科室',iconCls:'icon-form',border:false,fit:true" style="width:100%">
		<div style="padding:10px">
			<form id="editForm" method="post" >
				<input type="hidden" id="userId" name="id" value="${userid }">
				<table id="divLayout" class="honry-table" cellpadding="1" cellspacing="1" style="width:100%;">
		    		<c:forEach var="list" items="${uDeptList }" varStatus="status">
	...
	//弹框  科室增加
		...
		/**
			...
			function addContinue(){
					AddDeptdilogs("科室信息", "<c:url value='/sys/queryDeptUser.action'/>?uid="+$('#userId').val()+"&param="+new Date().getTime());
			}
					 ...
					 for(var i=0;i<getD.length;i++){  	 	
					  	whichObj=getD[i];
							...
							$.messager.confirm('确认', '确定要删除选中信息吗?', function(res){//提示是否删除
									if (res){
										$.ajax({
											url: "<c:url value='/sys/delUserDept.action'/>?deptid="+ids+"&userid="+$('#userId').val(),
											type:'post',
											success: function() {
												$.messager.alert('提示','删除成功');
												AddOrShowEast('EditForm',"<c:url value='/sys/editUserDept.action'/>?id="+getIdUtil("#list"),'35%');
												}
											});
										}
					        		});
