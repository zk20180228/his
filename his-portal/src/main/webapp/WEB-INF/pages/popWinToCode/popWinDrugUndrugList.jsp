					...
					<table id="list" style="width:100%" data-options="fit:true">
						<thead>
							<tr>
			    				...
			    				<th data-options="field:'undrugPinyin'" style="width:15%">拼音码</th>
			    				<th data-options="field:'undrugWb'" style="width:15%">五笔码</th>
			    				<th data-options="field:'undrugInputcode'" style="width:15%">自定义码</th>
			    				<th data-options="field:'undrugGbcode'" style="width:15%">国家编码</th>
			    				<th data-options="field:'undrugGjcode'" style="width:15%">国际编码</th>
	...
	//加载页面
		$(function(){
			...
			$('#list').datagrid({
				...
				singleSelect:true,
				pagination:true,
				url:'queryDrugUndrugPopWin.action',
				method:'post',
				rownumbers:true,
				idField: 'id',
				striped:true,
				...
				$.each(rowData, function (index, value) {
	             ...
	             onDblClickRow: function (rowIndex, rowData) {//双击查看
	            	 if('function' === typeof window.opener.popWinDrugCallBackFn){
							window.opener.popWinDrugCallBackFn(rowData);
						}else{
							window.opener.$(tmpId).val(rowData.encode);
							window.opener.$(tmpName).combobox("setValue",rowData.name);
		...
		*/
		function searchFrom(){ 
	   		...
	   		$('#list').datagrid({
				url:'<%=basePath%>/popWin/popWinDrugUndrug/queryDrugUndrugPopWin.action',
				method:'post',
				checkOnSelect:true,
				selectOnCheck:false,
				singleSelect:true,
