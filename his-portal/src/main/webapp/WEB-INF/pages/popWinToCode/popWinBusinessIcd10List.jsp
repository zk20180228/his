		...
		<div data-options="region:'center',fit:true"  id="content" style="width: 100%">
				...
				<div style="padding: 0px 5px 5px 5px;height: 90%">
					<table id="list" style="width:100%" data-options="fit:true">
						<thead>
							<tr>
			    				<th data-options="field:'code'" style="width:15%">代码</th>
	...
	//加载页面
		$(function(){
			...
			$('#list').datagrid({
				...
				singleSelect:true,
				pagination:true,
				url:'<%=basePath %>popWin/popWinBusinessIcd10/queryCodeBusinessIcd10.action',
				method:'post',
				rownumbers:true,
				idField: 'id',
				striped:true,
				...
				$.each(rowData, function (index, value) {
	             ...
	             onDblClickRow: function (rowIndex, rowData) {//双击查看
						if(getIdUtil("#list").length!=0){
							var tmptext = textId.split(',');
				    		var tmpId ="#"+tmptext[0];
							window.opener.$(tmpId).textbox('setValue',rowData.name);//可触发原有js的onSelect事件 
				    		tmpId ="#"+tmptext[1];
							window.opener.$(tmpId).val(rowData.code); 
				    		window.close();
						}
					}    
			});	
