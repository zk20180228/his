					...
					<table id="list" style="width:100%" data-options="fit:true">
						<thead>
							<tr>
			    				...
			    				<th data-options="field:'operCode'" style="width:25%">操作员</th>
			    				<th data-options="field:'operDate'" style="width:25%">操作日期</th>
	...
	//加载页面
		$(function(){
			...
			$('#list').datagrid({
				...
				singleSelect:true,
				pagination:true,
				url:'queryTerminalPopWin.action',
				method:'post',
				rownumbers:true,
				idField: 'id',
				striped:true,
				...
				$.each(rowData, function (index, value) {
	             ...
	             onDblClickRow: function (rowIndex, rowData) {//双击查看
						if(getIdUtil("#list").length!=0){
				    		var tmpId ="#"+textId;
							window.opener.$(tmpId).combobox('select',rowData.code);//可触发原有js的onSelect事件 
				    		window.close();
						}
					}    
			});	
		...
		*/
		function searchFrom(){ 
	   		...
	   		$('#list').datagrid({
				url:'<%=basePath%>/popWin/popWinTerminal/queryTerminalPopWin.action',
				method:'post',
				checkOnSelect:true,
				selectOnCheck:false,
				singleSelect:true,
