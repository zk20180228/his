	...
	//加载页面
		$(function(){
			...
			$('#list').datagrid({
				...
				singleSelect:true,
				pagination:true,
				url:'queryBusinessFrequencyPopWin.action',
				method:'post',
				rownumbers:true,
				idField: 'id',
				striped:true,
				...
				$.each(rowData, function (index, value) {
	             ...
	             onDblClickRow: function (rowIndex, rowData) {//双击查看
						if(getIdUtil("#list").length!=0){
				    		var tmpId ="#"+textId;
							window.opener.$(tmpId).combobox('select',rowData.encode);//可触发原有js的onSelect事件 
				    		window.close();
						}
					}    
			});	
		...
		*/
		function searchFrom(){ 
	   		...
	   		$('#list').datagrid({
				url:'<%=basePath%>/popWin/popWinBusinessFrequency/queryBusinessFrequencyPopWin.action',
				method:'post',
				checkOnSelect:true,
				selectOnCheck:false,
				singleSelect:true,
