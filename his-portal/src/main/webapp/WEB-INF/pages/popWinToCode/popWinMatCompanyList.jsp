			...
			<div data-options="region:'center',fit:true"  id="content" style="width: 100%">
				...
				<div style="padding: 0px 5px 5px 5px;height: 90%">
					<table id="list" style="width:100%" data-options="fit:true">
						<thead>
							<tr>
			    				<th data-options="field:'companyCode'" style="width:15%">代码</th>
			    				<th data-options="field:'companyName'" style="width:25%">名称</th>
			    				<th data-options="field:'spellCode'" style="width:25%">拼音码</th>
			    				<th data-options="field:'wbCode'" style="width:20%">五笔码</th>
			    				<th data-options="field:'customCode'" style="width:15%">自定义码</th>
	...
	//加载页面
		$(function(){
			...
			$('#list').datagrid({
				...
				selectOnCheck:false,
				singleSelect:true,
				url:'queryMatCompanyPopWin.action',
				method:'post',
				rownumbers:true,
				idField: 'id',
				striped:true,
				...
				$.each(rowData, function (index, value) {
	             ...
	             onDblClickRow: function (rowIndex, rowData) {//双击查看
						if(getIdUtil("#list").length!=0){
				    		var tmpId ="#"+textId;
							window.opener.$(tmpId).combobox('select',rowData.companyCode);//可触发原有js的onSelect事件 
				    		window.close();
						}
					}    
			});	
		...
		*/
		function searchFrom(){ 
	   		...
	   		$('#list').datagrid({
				url:'<%=basePath%>/popWin/popWinFactory/queryMatCompanyPopWin.action',
				method:'post',
				checkOnSelect:true,
				selectOnCheck:false,
				singleSelect:true,
