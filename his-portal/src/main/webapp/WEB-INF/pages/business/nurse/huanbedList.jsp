	 ...
	 <div class="easyui-layout" data-options="fit:true">
		...
		<div data-options="region:'center',fit:'true'" title="病床信息" style="height:100%;width: 80%">
			<div id="cc" class="easyui-layout" data-options="fit:true">
				...
				<div data-options="region:'center'" style="height: 7%;padding-top: 10px;padding-left: 5px;">
				...
				<div data-options="region:'south',border:false" style="height:85%;width: 100%">
	...
	$(function(){
		...
		$('#list1').datagrid({
			...
			onClickRow: function (rowIndex, rowData) {
				$.messager.confirm("确认","是否将"+$('#patientName').text()+"转移到"+rowData.bedName+"床?",function(r){
			    	if(r){
			    		$.ajax({
		    				url: '<%=basePath %>nursestation/nurse/saveHuanbed.action',
		    				data:{id:rowData.id,inpatientNo:inpatientNo},
		    				type:'post',
		    				success: function(data) {
		    					if(data=="success"){
		    						$.messager.alert('提示','换床成功！');
		    						setTimeout(function(){
										$(".messager-body").window('close');
										//实现刷新栏目中的数据
										window.opener.$("#treePatientInfo").tree("reload");
			    						window.close();
									},3500);
		    					}else{
		    						$.messager.alert('提示','换床成功！');
