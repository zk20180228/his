...
<script type="text/javascript">
	var addAndEdit;
	var bedLevelMap=null;//床位等级map
	var bedWardMap=null;//病房号map
	var bedStateMap=new Map();//病床状态
	...
	$(function() {
		...
		$('#list').datagrid({
			url:'<%=basePath %>baseinfo/hospitalbed/queryHospitalbed.action?menuAlias=${menuAlias}',
		...
		$.ajax({
			...
			success: function(data) {
				...
				for(var i=0;i<bstype.length;i++){
					bedStateMap.put(bstype[i].encode,bstype[i].name);
				}
			}
		});
	}
		...
		$.ajax({
			...
			success: function(data) {
				...
				for(var i=0;i<btype.length;i++){
					bedOrganMap.put(btype[i].encode,btype[i].name);
				}
			}
		});
	}
	...
	function funWard(){
		$.ajax({
			url: '<%=basePath %>baseinfo/hospitalbed/querybedWard.action',
			type:'post',
	...
	//获取渲染后的床位等级
	function funBedLevel(value,row,index){
		if(value!=null&&value!=''){
	...
	function trr(){
	$('#hospitalTree').tree({
			url : '<%=basePath %>baseinfo/hospitalbed/nurseStation.action',
			method:'get',
			lines : true,
			cache : false,
			animate : true,
			formatter : function(node) {//统计节点总数
				...
				if(node.id!="1"){
					var nodes = $('#hospitalTree').tree('getChildren',node.target);
					$('#node').val(node.id);
					$('#isdept').val(node.attributes.isdept);
					closeLayout();
					$('#list').datagrid('unselectAll');
					$('#list').datagrid('uncheckAll');
					$('#list').datagrid('clearChecked');
					
					if(node.attributes.deptCode){
						ListGrid(node.attributes.deptCode,node.attributes.isdept);
					}else{
						ListGrid(node.id,node.attributes.isdept,node.attributes.pid);
					}
				}else{
					ListGrid(node.attributes.deptCode,node.attributes.isdept);
				}
			},onLoadSuccess : function(node, data) {
				$('#hospitalTree').tree('select',$('#hospitalTree').tree('find', 1).target);
	...
	//list数据初始化
	function ListGrid(node,isdept,deptcode){
		//添加datagrid事件及分页
		$('#list').datagrid({
			url:'<%=basePath %>baseinfo/hospitalbed/queryHospitalbed.action?menuAlias=${menuAlias}',
			queryParams: {
	...
	function add(){
		a=$('#node').val();
		if(a!=null&&a!=''){
			$('#hospitalTree').tree('select',$('#hospitalTree').tree('find',a).target);
		}
		if(a!=null&&a!=''&a!='1'){
			addAndEdit = 0;
			closeLayout();
			AddOrShowEast('EditForm', '<%=basePath %>baseinfo/hospitalbed/addHospitalbedURl.action');
		}else{
			$.messager.alert('提示','请在左侧选择要添加的具体科室或病房');
			setTimeout(function(){
	...
	function edit(){
	    ...
	    if(row){
	    	AddOrShowEast('EditForm', '<%=basePath %>baseinfo/hospitalbed/editHospitalbedUrl.action?id='+row.id);
		}else{
	...
	function del1(){
		...
		if(obj.length>0){
			...
			$.messager.confirm('确认对话框', '您想要删除'+j+'条记录吗？', function(r){
				if (r){
					$.ajax({
						url:'<%=basePath %>baseinfo/hospitalbed/deleteHospitalbed.action',
						type:'post',
	...
	function refresh(){//刷新树 
		$('#hospitalTree').tree('options').url =  "<c:url value='/register/treeDeptSchedule.action'/>";
	...
	function searchTree(){
// 		var text = $('#searchTree').textbox('getValue');
// 		$("#hospitalTree").tree("search", [text,true]);
		...
		$.ajax({
			url : "<%=basePath %>baseinfo/hospitalbed/queryDeptLsit.action",
...
<body style="margin: 0px;padding: 0px">
	<div id="divLayout" class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'west',split:true,tools:'#toolSMId',title:'护士站病房信息',border:true" style="width:320px; min-width: 310px;">
			<div id="cc" class="easyui-layout" data-options="fit:true">   
			    <div data-options="region:'north',border:false" style="height:40px;padding-top: 7px;">
			    	<span class="hospitalbedListSearchSize">
			    		<input  class="easyui-textbox" ID="searchTree" style="width:182px" name=""  data-options="prompt:'输入护士站信息回车查询'" />
				...
				<table id="list" 
					...
					<thead>
						<tr>
							...
							<th data-options="field:'nurseCellName'" style="width: 12% ">护士站</th>
							<th data-options="field:'BedwardID'" style="width: 12% ">病房号</th>
							<th data-options="field:'bedName'" style="width: 12% ">床号</th>
							<th data-options="field:'bedLevel',formatter:funBedLevel" style="width: 14%">床位等级</th>
							<th data-options="field:'bedState',formatter:funBedState" style="width: 8%">床位状态</th>
							<th data-options="field:'bedOrgan',formatter:funBedOrgan" style="width: 8%">床位编制</th>
							<th data-options="field:'bedPhone'" style="width: 15%">床位电话</th>
							<th data-options="field:'bedBelong'"style="width: 10%" >归属</th>
							<th data-options="field:'bedFee'" align="right" halign="left" style="width: 6%">费用</th>
						</tr>
					</thead>
				</table>
			</div>
