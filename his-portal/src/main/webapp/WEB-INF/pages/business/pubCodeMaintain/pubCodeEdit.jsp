			...
			<input id="id" type="hidden" name="dictionary.id" value="${dictionary.id }">
				<table class="honry-table" cellpadding="1" cellspacing="1" border="false" style="margin-left:auto;margin-right:auto;">					
	    			...
	    			<tr>
		    			...
		    			<td class="honry-info">&nbsp;<input id="yesId" type="checkbox" style="width:18px;" onclick="onClickIsStop('yesId')"/><input type="hidden" id="stopId" name="dictionary.validState" value="0"/></td>
	    			</tr>		    		 
				</table>
...
function submit(flg) {
	...
	$('#editForm').form('submit', {
		url:"<%=basePath %>baseinfo/pubCodeMaintain/saveOrUpdatePubCode.action?flag="+flag,
		onSubmit: function(){
			var node = $('#tDt').tree('getSelected');
			if(node){
				if(node.id=='root' && $('#type').val()==""){
					$.messager.alert('操作提示', '编码类型不能为空！','error');  
	                return false;  
				}
				if(node.id=='root' && $('#typeName').val()==""){
