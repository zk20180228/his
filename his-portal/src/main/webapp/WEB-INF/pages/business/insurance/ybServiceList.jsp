...
<script type="text/javascript"> 
				...
				$('#list').datagrid({
					...
					onLoadSuccess: function (data) {//默认选中
			          ...
			          //分页工具栏作用提示
			        	...
			        	listData=rowData
			        	$('#id').val(rowData.id);
			        	searchHisInfo(rowData.serviceCode)
		      ...
		      function append() {
		          if (endEditing()) {
		              $('#list1').datagrid('appendRow', {
							ybItemCode:listData.serviceCode,
							ybItemName:listData.serviceName,
							costType:listData.costType,
							itemType:"2"
		              });
		  ...
		  function formatzz(val,row,index){
			...
			function formatbedLevel(val,row,index){
				if(val=='1'){
					return "普通病床"
				}
				if(val=='2'){
...
<div data-options="region:'center',title:'服务信息',border:true">
		<table id="list" style="width:100%;" data-options="url:'${pageContext.request.contextPath}/baseinfo/insurance/queryServiceList.action?menuAlias=${menuAlias}',method:'post',rownumbers:true,idField: 'id',striped:true,border:false,checkOnSelect:true,selectOnCheck:false,singleSelect:true,fitColumns:false,fit:true">
			<thead>
				<tr>				
<!-- 				<th data-options="field:'ck',checkbox:true" ></th> -->
				    <th data-options="field:'id'"width="6%"hidden=true>ID</th>
					<th data-options="field:'serialNo'"width="3%">序号</th>
					<th data-options="field:'serviceCode'"width="9%">中心编码</th>
					<th data-options="field:'serviceName'"width="9%">中心名称</th>
					<th data-options="field:'costType',formatter:formatHisCostype"width="6%" >费用类别</th>
					<th data-options="field:'bedLevel',formatter:formatbedLevel"width="6%">病床等级</th>
					<th data-options="field:'payStandard'"width="6%">支付标准</th>
					<th data-options="field:'maxPrice'"width="6%">最高价格</th>
					<th data-options="field:'cityMaxPrice'"width="6%">市级最高价格</th>
					<th data-options="field:'countyMaxPrice'"width="6%">县级最高价格</th>
					<th data-options="field:'otherPrice'"width="6%">县级以下价格</th>
					<th data-options="field:'lxPrice'"width="6%">离休价格</th>	
					<th data-options="field:'eyPrice'"width="6%">二乙价格</th>
					<th data-options="field:'operator'"width="6%">经办人</th>            
				</tr>
			</thead>
		</table>
	</div>
	<div data-options="region:'south',title:'服务对照信息',split:true" style="height:40%;">
...
<!-- 				    <th data-options="field:'ck',checkbox:true" ></th> -->
					...
					<th data-options="field:'insuranceNo'"width="6%" hidden=true>医疗机构编号</th>
					...
					width="6%" >院内收费类别</th>
					<th data-options="field:'itemDosageform'"width="6%" >院内项目剂型</th>
					...
					<th data-options="field:'inSelfProportion'"width="6%" hidden=true>住院自付比例</th>
					<th data-options="field:'operator'"width="6%">经办人</th>            
				</tr>
			</thead>
		</table>
