...
<script type="text/javascript"> 
				...
				$('#list').datagrid({
					...
					onLoadSuccess: function (data) {//默认选中
			          ...
			          //分页工具栏作用提示
			        	...
			        	listData=rowData
			        	$('#id').val(rowData.id);
			        	searchHisInfo(rowData.itemCode)
		      ...
		      function append() {
		          if (endEditing()) {
		              $('#list1').datagrid('appendRow', {
							insuranceNo:listData.insuranceNo,
							ybItemCode:listData.itemCode,
							ybItemName:listData.itemName,
							costType:listData.costType,
							itemType:"1",
							inSelfProportion:listData.inSelfProportion
		              });
		  ...
		  function formatzz(val,row,index){
			...
			function itemunit(val,row,index){
				if(val=='04'){
					return "次"
				}
				if(val=='05'){
...
<div id="divLayout" class="easyui-layout" data-options="fit:true,border:false">
	...
	<div data-options="region:'center',title:'诊疗信息',border:true">
		<table id="list" style="width:100%;" data-options="url:'${pageContext.request.contextPath}/baseinfo/insurance/queryClinicList.action?menuAlias=${menuAlias}',method:'post',rownumbers:true,idField: 'id',striped:true,border:false,checkOnSelect:true,selectOnCheck:false,singleSelect:true,fitColumns:false,fit:true">
			<thead>
				<tr>
...
<!-- 					<th data-options="field:'ck',checkbox:true" ></th> -->
					...
					<th data-options="field:'itemCode'"width="6%">中心编码</th>
					<th data-options="field:'itemName'"width="9%">中心名称</th>
					<th data-options="field:'costType',formatter:formatHisCostype"width="3%">费用类别</th>
					<th data-options="field:'costItemGrade',formatter:formatlevel"width="3%">收费项目等级</th>
					<th data-options="field:'itemUnit',formatter:itemunit"width="3%">单位</th>
					<th data-options="field:'maxPrice'"width="3%">最高价格</th>
					<th data-options="field:'cityMaxPrice'"width="3%">市级最高价格</th>
					<th data-options="field:'countyMaxPrice'"width="3%">县级最高价格</th>
					<th data-options="field:'otherPrice'"width="3%">县级以下价格</th>
					...
					<th data-options="field:'eyPrice'"width="3%">二乙价格</th>
					<th data-options="field:'approveFlag',formatter:formatflag"width="3%">是否需要审批标志</th>
					<th data-options="field:'insuranceItemCode'"width="3%">非药品编码</th>
					<th data-options="field:'operator'"width="3%">经办人</th>            
				</tr>
			</thead>
		</table>
	</div>
	<div data-options="region:'south',title:'诊疗对照信息',split:true" style="height:40%;">
