				...
				<div style="width:80%;height:40%;border: 0px inset;">
					<table class="honry-table" cellpadding="1" cellspacing="1" style="width:80%;">
						...
						<tr>
							...
							<td class="honry-lable">楼层：</td>
							<td>
								<input id="deptAddress" class="easyui-textbox"/>
							</td>
							<td class="honry-lable">房间：</td>
							<td>
						...
						<tr>
							...
							<td>
								<input id="bedName" class="easyui-textbox" name="bedName" readonly="readonly"/>
							</td>
...
<script type="text/javascript">
	...
	$(function(){
		...
		//加载部门树
		$('#tDt').tree({
			url : '<%=basePath%>nursestation/doctorAdvice/treeDoctorAdvice.action',
			method : 'get',
			animate : true,
			lines : true,
			formatter : function(node) {//统计节点总数
				...
				if(node.attributes.pid!="3"){
					$.messager.alert('提示',"请选择会诊状态下的患者");
					return false;
				}
				$.ajax({
					url:"<%=basePath%>outpatient/auditing/queryDoctorId.action?medicalrecordId="+medicalrecordId,
					success:function(data){
						var doctorId= eval("("+data+")");
						if(doctorId!=empId){
							$.messager.alert('提示',"该患者不在当前登录医生名下，无法进行申请");
							return false;
						}
						 $('#patientNo').val(medicalrecordId);
						  //查询会诊申请记录列表展示
							$('#listlist').datagrid({
								url:"<%=basePath %>outpatient/auditing/queryAuditingList.action?menuAlias=${menuAlias}&medicalrecordId="+medicalrecordId
							});
						    $.ajax({
								url: "<%=basePath%>outpatient/auditing/queryAuditing.action?medicalrecordId="+medicalrecordId,
								type:'post',
								success: function(data) {		
									var  plist=eval("("+data+")");
									$('#patientName').textbox('setValue',plist.patientName);
									$('#reportSex').textbox('setValue',sexMap.get(plist.reportSex));
									$('#reportAge').textbox('setValue',plist.reportAge);
	 ...
	 /**
	   ...
	   */
	   function popWinToDeptCnsl(){
		   $('#cnslDoccd').combobox('setValue',"");
			var tempWinPath = "<%=basePath%>popWin/popWinDepartment/toDepartmentPopWin.action?deptType=C,I,T&textId=cnslDeptcd";
   ...
   /**
	   ...
	   */
	   function popWinToDept(){
		   $('#docCode').combobox('setValue',"");
			var tempWinPath = "<%=basePath%>popWin/popWinDepartment/toDepartmentPopWin.action?deptType=C,I,T&textId=deptCode";
			window.open (tempWinPath,'newwindow',' left=150,top=80,width='+ (screen.availWidth -300) +',height='+ (screen.availHeight-170)+',scrollbars,resizable=yes,toolbar=yes')
		}
