		...
		<div data-options="region:'center',border:false" style="width: 100%;height: 100%;padding:5px" align="center">
			<table style="width: 100%;height: 10%;">
				...
				<tr >
					<td nowrap="nowrap">
							...
							<!--新添字段-->
							<input id="nurseCellName" name="nurseCellName" type="hidden">
							<input id="applyDocName" name="applyDocName" type="hidden">
							<input id="chargeDocName" name="chargeDocName" type="hidden">
							<input id="sexName" name="sexName" type="hidden">
							<input id="cancelName" name="cancelName" type="hidden">
							<input id="approvalOperName" name="approvalOperName" type="hidden">
			...
			<table class="honry-table"  cellpadding="1" cellspacing="1"	border="1px solid black" style="width:100%;padding:5px">
					...
					<tr>
						...
						<td>
							<input class="easyui-numberbox" id="quantity" name="quantity" style="width: 50px;" data-options="required:true,missingMessage:'只能输入数字'">　　
						...
						<td>
	       					<input id="orderTime" name="orderTime" class="Wdate" type="text" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'%y-%M-%d',maxDate:'%y-%M-{%d+7}'})" style="width:150px;border: 1px solid #95b8e7;border-radius: 5px;"/>	
						</td>
						</tr>
					<tr>
						<td class="honry-lable">
					...
					<tr>
						...
						<td>
							<input id="hematin" name="hematin" class="easyui-numberbox" data-options="required:true">g/L
						</td>
						</tr>
						<tr>
						<td class="honry-lable">
						...
						<td>
							<input id="hct" name="hct" class="easyui-numberbox" data-options="required:true">
						</td>
						<td class="honry-lable">
							血小板：
						</td>
						<td>
							<input id="platelet" name="platelet" class="easyui-numberbox" data-options="required:true">
							x10 <sup>9</sup>/L
						</td>
						</tr>
						<tr>
						...
						<td>
							<input id="alt" name="alt" class="easyui-numberbox" data-options="required:true">
							U/L
						</td>
						<td class="honry-lable">
							HbsAg：
			...
			<table style="width: 100%;">
						<tr class="inpatientApplication">
					    	<td ><input id="templateSubmit" type="checkbox" onclick="clickclick()">自定义诊断名称&nbsp;
						    		<span id="lczds" style="display: none;"><input id="lczdText" name="diagnoses"  disabled="disabled" class="easyui-textbox" data-options="required:true" value="${inpatientApplyNow.diagnoseName}" ></input></span> 
						    	</td>
					    	<td style="color:Blue;text-align: right;">申请医师：
					    		<input  id="shenqingyishi" name="applyDocCode" class="easyui-combobox" readonly="readonly" data-options="required:true">
					    		主治医师：
					    		<input  id="zhuzhiyishi" name="chargeDocCode" data-options="required:true">
					    		<a href="javascript:delSelectedData('zhuzhiyishi');"  class="easyui-linkbutton" data-options="iconCls:'icon-opera_clear',plain:true"></a>
					    		申请时间：
					    		<input id="shijian" name="applyTime" class="Wdate" type="text" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'%y-%M-%d-%H-%m-{%s-10}',maxDate:'%y-%M-%d-%H-%m-{%s+10}'})" style="width:150px;border: 1px solid #95b8e7;border-radius: 5px;"/>
					    	</td>
						</tr>
			</table>
			<table style="width: 100%;">
...
<script type="text/javascript">
	...
	/*******************************结束读身份证***********************************************/
			function query(){
				...
				$.ajax({
		   			url: "<%=basePath%>publics/inpatientInfoList/queryInpatientInfoList.action?id="+id,
						...
						if(plist.length>1){
							...
							$("#infoDatagrid").datagrid({
							    ...
							    columns:[[    
							        ...
							        {field:'patientName',title:'姓名',width:'19%',align:'center'} ,   
							        {field:'certificatesNo',title:'身份证号',width:'35%',align:'center'} 
							    ]] ,
							    onDblClickRow:function(rowIndex, rowData){
							    	$('#xingming').textbox('setValue',rowData.patientName);
									$('#sexCode').textbox('setText',sexMap.get(rowData.reportSex));
									$('#sexCodeHidden').val(rowData.reportSex);
									$('#sexName').val($('#sexCode').textbox('getText'));
									$('#clinicNo').val(rowData.inpatientNo);
									$('#patientNo').val(plist[0].medicalrecordId);
									$('#deptCode').val(rowData.deptCode);
									$('#bedNo').val(rowData.bedName);
									...
									$('#keshi').textbox('setValue',deptMap[rowData.deptCode]);
									$('#deptName').val($('#keshi').textbox('getValue'));
									$('#zhuzhiyishi').combobox('setValue',rowData.chargeDocCode);
							...
							$('#sexCode').textbox('setText',sexMap.get(plist[0].reportSex));
							$('#sexCodeHidden').val(plist[0].reportSex);
							$('#sexName').val($('#sexCode').textbox('getText'));
							$('#clinicNo').val(plist[0].inpatientNo);
							$('#patientNo').val(plist[0].medicalrecordId);
							$('#deptCode').val(plist[0].deptCode);
							$('#bedNo').val(plist[0].bedName);
							$('#nurseCellCode').val(plist[0].nurseCellCode);
							var ages=DateOfBirth(plist[0].reportBirthday);
							$('#age').textbox('setValue',ages.get("nianling"));
							$('#reportAgeunit').text(ages.get('ageUnits'));
							$('#keshi').textbox('setValue',deptMap[plist[0].deptCode]);
							$('#deptName').val($('#keshi').textbox('getValue'));
							$('#zhuzhiyishi').combobox('setValue',plist[0].chargeDocCode);
							$('#chargeDocName').val($('#zhuzhiyishi').combobox('getText'));

						}else{
							$.ajax({
								url:"<%=basePath%>publics/inpatientInfoList/queryPatInfo.action?medId="+id,
								success:function(data){
									if(data=='1'){
										$.messager.alert('提示','患者没有进行住院登记或住院接诊');
										setTimeout(function(){
											$(".messager-body").window('close');
										},3500);
									}else{
										$.messager.alert('提示','病历号有误，请核对后重新输入');
										setTimeout(function(){
											$(".messager-body").window('close');
										},3500);
									}
			...
			function submit() {
				...
				if($('#quantity').numberbox('getValue')==0){
					...
					('#quantity').numberbox('setValue',200);
					return;
				}
				if($('#orderTime').val()==null||$('#orderTime').val()==""){
					$.messager.alert('提示','请填写预定输血日期！');
					setTimeout(function(){
						$(".messager-body").window('close');
					},3500);
