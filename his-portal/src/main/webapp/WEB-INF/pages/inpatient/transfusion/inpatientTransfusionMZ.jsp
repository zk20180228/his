...
<%@ include file="/common/metas.jsp"%>
	...
	$(function(){
		...
		$('#list').datagrid({
			url:"<%=basePath%>publics/InpatientApply/queryInpatientApplyList.action?menuAlias=${menuAlias}&state="+1,
			onBeforeLoad:function(){
				$('#list').datagrid('clearChecked');
				$('#list').datagrid('clearSelections');
			},
			onDblClickRow:function(rowIndex, rowData){
				window.open('<%=basePath %>publics/transfusion/transfusionApplicationViewMZ.action?id='+rowData.id,'信息查看','width='+(window.screen.availWidth-150)+',height='+(window.screen.availHeight-220)+
	...
	function del() {
		...
		if (rows.length > 0) {
			...
			$.messager.alert("操作提示", "请选择删除对象");
		}
	}
		function add(){
			window.open('<%=basePath %>publics/transfusion/transfusionApplicationListMZ.action?menuAlias='+ss,'输血申请','width='+(window.screen.availWidth-150)+',height='+(window.screen.availHeight-90)+
			',top=20,left=60,resizable=yes,status=yes,menubar=no,scrollbars=yes');
		}
		...
		function edit(){
			...
			if(data==""||data==null){
				...
				window.open('<%=basePath %>publics/transfusion/inpatientApplicationEditMZ.action?id='+data.id+'&menuAlias='+ss,'输血申请','width='+(window.screen.availWidth-150)+',height='+(window.screen.availHeight-90)+
				',top=20,left=60,resizable=yes,status=yes,menubar=no,scrollbars=yes');
			}
...
<body style="margin: 0px;padding: 0px;">
	<div id="divLayout" class="easyui-layout" fit="true">
		...
		<div data-options="region:'north',split:false,border:false" style="width: 100%;height: 50px;padding-top: 10px;">
			病历号: 
		...
		<div id="divLayout" region="center" style="width: 100%;height: 95%" title="输血申请">
			<table id="list" border="false" 
				class="easyui-datagrid" 
				data-options="method:'post',rownumbers:true,idField:'id',border:false,fit:true,singleSelect:true,checkOnSelect:false,selectOnCheck:false,pagination:true,pageSize:20,pageList:[20,40,60,80,100],toolbar:'#toolbarId'">
				<thead>
					<tr>
						<th data-options="field:'name'" width="12.5%" style="font-size: 14">
							申请人
						</th>
						<th data-options="field:'orderTime'" width="12.5%" style="font-size: 14">
							申请时间
						</th>
						<th data-options="field:'bloodKind',formatter:functionxuexing" width="12.5%" style="font-size: 14">
							申请血型
						</th>
						<th data-options="field:'bloodTypeCode',formatter:functionchengfen" width="12.5%" style="font-size: 14">
							血液成分
						</th>
						<th data-options="field:'rh',formatter:functionRH" width="12.5%" style="font-size: 14">
							RH
						</th>
						<th data-options="field:'quantity'" width="12.5%" style="font-size: 14">
							申请输血量
						</th>
						<th
							data-options="field:'stockUnit'" width="12.5%" style="font-size: 14">
							申请数量单位
						</th>
						<th
							data-options="field:'applyState',formatter:functionzhuangtai" width="12.5%" style="font-size: 14">
							状态
						</th>
					</tr>
				</thead>
