		...
		<div class="easyui-layout" data-options="fit:true" > 
			<div data-options="region:'north'" style="height:42px;width:100%;border-top:0;border-left:0;border-right:0;">
				<table style="width:100%;padding: 0px 5px 0px 5px;height: 40px;">
					<tr>
						<td nowrap="nowrap">
							<a href="javascript:void(0)"  onclick="apply(true)"  class="easyui-linkbutton" data-options="iconCls:'icon-ok'"  >通过</a>
							<a href="javascript:void(0)"  onclick="apply(false)" class="easyui-linkbutton" data-options="iconCls:'icon-no'"  >不通过</a>
	...
	function apply(flag){
	    ...
	    if(ids!=null&&ids!=undefined&&ids.length>0){
	    	...
	    	for(var a=0;a<ids.length;a++){
	    		if(ids[a].apply_status=="0"){//只能审批申请状态的数据
	    			cPWAppId+=ids[a].id+",";
	    		}
	    	}
	    	if(cPWAppId==''){
	    		$.messager.alert('提示','所选项中无申请状态的数据,无法进行审批！'); 
	    		return;
	    	}else{
	    		var apply_status="";
	    		if(flag){//通过
	    		...
	    		$.ajax({
					type:"post",
					data:"cPWAppId="+cPWAppId+"&apply_status="+apply_status,
					url:"${pageContext.request.contextPath}/outpatient/CPWay/approveApply.action",
					success:function(backData){
