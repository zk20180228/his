	...
	<div class="easyui-layout" style="width:100%;height:100%;" id="treeLayOut">
		<div data-options="region:'center'"  id="content">
				...
				<div style="padding: 0px 5px 5px 5px;">
					<table id="list" style="width:100%" data-options="url:'queryCodeCheckpoint.action',method:'post',rownumbers:true,idField: 'id',striped:true,border:true,checkOnSelect:true,selectOnCheck:false,singleSelect:true,fitColumns:false">
						<thead>
							<tr>
			...
			$('#list').datagrid({
				...
				$.each(rowData, function (index, value) {
				  if(value.id == id){
	               ...
	               iconCls: 'icon-add',
	               handler: function () {
	                   AddOrShowEast('EditForm', 'addCodeCheckpoint.action');
	          	   }
	            },'-',{
		            id:'btnEdit',
					text:'修改',	
					...
					handler:function(){
					  if(getIdUtil("#list").length!=0){
						 AddOrShowEast('EditForm', 'updateCodeCheckpoint.action?id='+getIdUtil("#list"));
					  }
						   ...
						   if (res){
							 var ids = '';
							   for(var i=0; i<rows.length; i++){
								  ...
								  $.ajax({
								  url: 'delCodeCheckpoint.action?id='+ids,
								  type:'post',
								  success: function() {
	             ...
	             onDblClickRow: function (rowIndex, rowData) {//双击查看
					if(getIdUtil("#list").length!=0){
						 AddOrShowEast('EditForm','viewCodeCheckpoint.action?id='+getIdUtil("#list"));
					}
				}    
			});	
		});
