...
<body>
	<div id="divLayout" class="easyui-layout" fit="true">
		<div data-options="region:'north',split:false" style="padding:10px;min-height:80px;height:auto;">
  		...
  		<div  data-options="region:'center',split:false,title:'医嘱类别列表',iconCls:'icon-book'" style="padding:10px;">
  			<table id="list"
			data-options="rownumbers:true,singleSelect:false,url:'queryCodeOrdercategory.action',method:'post',fitColumns:'true'">
			<thead>
				<tr>
			...
			$('#list').datagrid({
				...
				$.each(rowData, function (index, value) {
				  if(value.id == id){
	               ...
	               iconCls: 'icon-add',
	               handler: function () {
	                   AddOrShowEast('EditForm', 'addCodeOrdercategory.action');
	          	   }
	            },'-',{
		            id:'btnEdit',
					text:'修改',	
					...
					handler:function(){
					  if(getIdUtil("#list").length!=0){
						 AddOrShowEast('EditForm', 'updateCodeOrdercategory.action?id='+getIdUtil("#list"));
					  }
						   ...
						   if (res){
							 var ids = '';
							   for(var i=0; i<rows.length; i++){
								  ...
								  $.ajax({
								  url: 'delCodeOrdercategory.action?id='+ids,
								  type:'post',
								  success: function() {
	             ...
	             onDblClickRow: function (rowIndex, rowData) {//双击查看
					if(getIdUtil("#list").length!=0){
						 AddOrShowEast('EditForm','viewCodeOrdercategory.action?id='+getIdUtil("#list"));
					}
				}    
			});	
		});
