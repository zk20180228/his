...
<body>
	<div id="divLayout" class="easyui-layout" data-options="fit:true" id="treeLayOut">
				...
				<div data-options="region:'center',border:false" style="width:100%;">
					<table id="list" style="width:100%" data-options="url:'querylistTitle.action',method:'post',rownumbers:true,idField: 'id',striped:true,border:true,checkOnSelect:true,selectOnCheck:false,singleSelect:true,fitColumns:false,fit:true">
						<thead>
							<tr>
	...
	$(function() {
		...
		$('#list').datagrid({
			...
			toolbar : [ {
				...
				iconCls : 'icon-add',
				handler : function() {
					AddOrShowEast('EditForm', 'addTitle.action');
				}
			},'-', {
				id : 'btnEdit',
				text : '修改',
				...
				handler : function() {
					if (getIdUtil("#list").length != 0) {
						AddOrShowEast('EditForm', 'editTitle.action?id=' + getIdUtil("#list"));
					}
				}
			}, '-', {
				id : 'btnDelete',
					...
					if (idss.length > 0) {//选中几行的话触发事件	                        
						$.messager.confirm('确认', '确定要删除选中信息吗?', function(res) {//提示是否删除
							if (res) {
								...
								$.ajax({
									url : 'delTitleToWin.action?id=' + ids,
									type : 'post',
									success : function() {
			...
			onDblClickRow : function(rowIndex, rowData) {//双击查看
				if(getIdUtil("#list").length!=0){
					AddOrShowEast('EditForm', 'viewTitle.action?id='+getIdUtil("#list"));
