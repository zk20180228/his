...
<body>
	...
	$(function() {
		...
		$('#list').datagrid({
			...
			onDblClickRow : function(rowIndex, rowData) {//双击查看
                ...
                if(row){
                	AddOrShowEast("查看厂商",'<%=basePath %>drug/manufacturer/viewDrugManufacturer.action?id='+rowData.id);
			   	}
			}
		});
	...
	function add(){
		addAndEdit = 0;
		closeLayout();
		AddOrShowEast('EditForm', '<%=basePath %>drug/manufacturer/saveManufacturerUrl.action');
	}
	//修改
	function edit(){
		...
		if (rows != null && rows.length != 0) {
			addAndEdit = 1;
			closeLayout();
			AddOrShowEast('EditForm', '<%=basePath %>drug/manufacturer/updateManufacturerUrl.action?id=' + getIdUtil('#list'));
		}else{
            ...
            if (rows.length > 0) {//选中几行的话触发事件	                        
			 	$.messager.confirm('确认', '确定要删除选中信息吗?', function(res){//提示是否删除
					if (res){
						...
						$.ajax({
							url: '<%=basePath %>drug/manufacturer/deleteManufacturer.action?id='+ids,
							type:'post',
...
<body>
	<div id="divLayout" class="easyui-layout" fit=true>
			...
			<div data-options="region:'center',split:false,border:false" style=" width: 100%;">
				...
				<table id="list" style="height: 557px;" data-options="fit:true,url:'${pageContext.request.contextPath}/drug/manufacturer/queryDrugManufacturer.action?menuAlias=${menuAlias}',method:'post',rownumbers:true,idField: 'id',striped:true,border:true,checkOnSelect:true,selectOnCheck:false,singleSelect:true,fitColumns:false,toolbar:'#toolbarId'">
					<thead>
						<tr>
							<th field="ck" checkbox="true"></th>
							<th data-options="field:'manufacturerName', width : 200">
								名称
							</th>
							<th data-options="field:'manufacturerPinyin', width : 100">
								拼音码
							</th>
							<th data-options="field:'manufacturerWb', width : 100">
								五笔码
							</th>
							<th data-options="field:'manufacturerInputcode', width : 100">
								自定义码
							</th>
							<th data-options="field:'manufacturerLink', width : 100">
								联系方式
							</th>
							<th data-options="field:'manufacturerBank', width : 100">
								开户银行
							</th>
							<th data-options="field:'manufacturerAccount', width : 100">
								开户账号
							</th>
							<th data-options="field:'manufacturerAddress', width : 100">
								地址
							</th>
							<th data-options="field:'manufacturerGmp', width : 100">
								GMP
							</th>
							<th data-options="field:'manufacturerRemark', width : 300">
								备注
							</th>
						</tr>
					</thead>
