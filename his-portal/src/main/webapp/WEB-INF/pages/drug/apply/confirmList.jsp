...
// 							$('#medicalrecordId').textbox('setValue',rowData.medicalrecordId);  
							...
							setTimeout(function(){
								 if(listDrugBack==0&&listNotDrugBack==0){
										$.messager.alert('友情提示','该患者没有退费申请');
									} 
							},3500);
					    }
					}); 
...
function funApproval(){
	...
	if(flg>0){
		...
		$.ajax({
			url:'<%=basePath%>inpatient/applycon/applyState.action',
			type:'post',
			traditional:true, 
			data:{'ids':arr},
			success:function(data){
				if(data.resCode == -1){
					...
					$.messager.confirm('确认','此次将退还现金'+data.resMes+'元。',function(r){    
					    if (r){  
					    	$.ajax({
								url:'<%=basePath%>inpatient/applycon/applyConfirm.action',
								type:'post',
								traditional:true, 
								data:{'ids':arr},
								success:function(data){
									if(data.resCode == "success"){
	    						    	...
	    						    	$.each(brr,function(i,n){
	    						    		 $('#listDrugBack').datagrid('deleteRow',$('#listDrugBack').datagrid('getRowIndex',n));
	    								});
	    						    	$.each(brr1,function(i,n){
	    						    		 $('#listNotDrugBack').datagrid('deleteRow',$('#listNotDrugBack').datagrid('getRowIndex',n));
	    								});
									}
								}
							});
		...
		<div class="easyui-layout" data-options="fit:true,border:false">
			<div  data-options="region:'north',border:false" style="height:45px;padding-top: 10px;" >
				...
				<shiro:hasPermission name="${menuAlias}:function:returns">
					<a href="javascript:funApproval()" class="easyui-linkbutton" data-options="iconCls:'icon-backfee2'">退费</a>
				</shiro:hasPermission>
			</div>
			<div  data-options="region:'center',border:false" style="height: 60px;border-bottom-width: 0px;"> 
				<table id="tp" class="honry-table" style=" border: 0px; width: 100%;height: 35px;">
					<tr>
		...
		<div id="ttdiv" class="easyui-tabs" title="患者明细" data-options="border:false,fit:true,tabPosition:'bottom'">
			<div title="药品退费申请区" style=" height: 100%; width: 100%;">
				<table id="listDrugBack" class="easyui-datagrid" 
					...
					<thead>
 						<tr> 
							<th data-options="field:'applyNo',checkbox:true" ></th> 
