				...
				var pd =  true;
	   			...
	   			for(var i=0;i<obj.length;i++){
	   			 ...
	   			 if(pd){
	   				...
	   				if(numtext > maxSum){
	   					$.messager.alert('友情提示','该药品可退的数量最大为'+(maxSum/row.packQty));
	   					setTimeout(function(){
							$(".messager-body").window('close');
						},3500);
	   	    			$('#num').numberbox('setValue','');
 	    ...
 	    onSelect: function(node){
 	   		if(node.id != 1){
					...
					$('#inpatientNo').val(node.attributes.inpatientNo);
	 	   			...
	 	   			loadTabs(node.attributes.medicalrecordId);
	...
	if(flg>0){
		    	...
		    	if(a){
			    	...
			    	$.each(brr,function(i,n){
			    		...
			    		for(var i=0;i<obj.length;i++){
			    		 ...
			    		 var drugRow = $('#listDrug').datagrid('getSelected');
			    		 var rows = $('#listDrugBack').datagrid('getRows');
			    		 var inde = $('#listDrugBack').edatagrid('getRowIndex',n)
			    		 var backrows = rows[inde];
			    		 $('#listDrug').datagrid('updateRow',{
			    			 index: indexd,
			    			 row: {
			    				 nobackNum:drugRow.nobackNum+n.quantity,
			    				 moneySum:backrows.ck+drugRow.moneySum
			    			 }
			    		 });
			    		 $('#listDrugBack').datagrid('deleteRow',$('#listDrugBack').edatagrid('getRowIndex',n));
			    		 $('#listDrugBack').datagrid('unselectRow',indexd);
		...
		if(index==0){
			...
			if(obj.length){
				    	...
				    	if(a){
					    	...
					    	$.each(brr,function(i,n){
					    		...
					    		for(var i=0;i<obj.length;i++){
					    		 ...
					    		 var inde = $('#listDrugBack').edatagrid('getRowIndex',n);
					    		 var backrows = rows[inde];
					    		 var cost = parseFloat(backrows.ck)+parseFloat(drugRow.moneySum);
					    		 $('#listDrug').datagrid('updateRow',{
					    			 index: indexd,
					    			 row: {
					    				 nobackNum:drugRow.nobackNum+n.quantity,
				...
				$.messager.alert('友情提示','请药品申请区，要取消的药品');
				setTimeout(function(){
					$(".messager-body").window('close');
				},3500);
			}
...
function save(){
	...
	$('#saveform').form('submit',{
        url:'<%=basePath %>inpatient/applydrug/saveConsultation.action',
        onSubmit:function(){ 
	    ...
	    <div data-options="region:'north',border:false" style="height:50px;width: 100%;padding-top: 10px;">
        	...
        	<shiro:hasPermission name="${menuAlias}:function:readIdCard">
  			...
  			<shiro:hasPermission name="${menuAlias}:function:save">
  				<a href="javascript:save()"  class="easyui-linkbutton" data-options="iconCls:'icon-save'">保存申请</a>
  			</shiro:hasPermission>
  			</span>
	    </div>   
			    	...
			    	<form id="saveform"  method="post">
				    	<table id="tp" class="tableCss" style="width: 100%;">
				    		<tr>
								...
								<td><input class="easyui-textbox" id="medicalrecordId" name='medicalrecordId1' data-options="prompt:'请输入病历号'"/>
					    ...
					    <div title="患者药品区" style="width: 100%;height: 100%">   
					        <div id="cc2" class="easyui-layout" data-options="fit:true,border:false">   
							    <div data-options="region:'north',border:false" style="height:47%;width: 100%">
							         ...
							         checkOnSelect:false,selectOnCheck:false,singleSelect:true,fitColumns:false,fit:true">   
							        	<thead>
							        		<tr>
									            ...
									            <th data-options="field:'nobackNum',formatter:funNum" style="width: 9%">可退数量</th>   
									            ...
									            <th data-options="field:'isUnit',hidden:true" style="width: 9%">是否包装单位</th>
									            ...
									            <th data-options="field:'recipeDeptName'"  style="width: 9%">开方科室</th>
									             ...
									             <th data-options="field:'extFlag3',hidden:true">显示单位</th>
								        	</tr>
							        	</thead>
							        </table>
							    </div>   
							        	...
							        	<thead>
							        		<tr>
									            ...
									            <th data-options="field:'sequenceNo'"  style="width: 9%">处方内流水号</th> 
									             <th data-options="field:'operationId',hidden:true">手术序号</th>
									             <th data-options="field:'extFlag3',hidden:true">显示单位</th>
									             <th data-options="field:'packQty',hidden:true" style="width: 9%">包装数</th>   
								        	</tr>
							        	</thead>
							        </table> 
							    </div>   
					    	...
					    	<div id="cc3" class="easyui-layout" data-options="fit:true">   
							    <div data-options="region:'north',border:false" style="height:47%;width: 100%">
							    	<table id="listNotDrug" class="easyui-datagrid"  style="width: 100%;height: 90%;"
							         data-options="rownumbers:true,idField: 'id',
							         striped:true,border:false,fitColumns:false,
							    ...
							    <div data-options="region:'south'" style="height:45%;width: 100%;border-left:0;">
							    	<table id="listNotDrugBack" class="easyui-datagrid" title='非药品退费申请区' style="width: 100%;height:90%;"
								         data-options="rownumbers:true,striped:true,border:false,
