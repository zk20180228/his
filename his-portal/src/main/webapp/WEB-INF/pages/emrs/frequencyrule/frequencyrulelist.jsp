...
<html>
<head>
<title>频率质控项目设置</title>
<script type="text/javascript">
var medicTypeMap = new Map();
$(function(){
	setTimeout(function(){
		$('#list').datagrid({
			url:'<%= basePath%>emrs/frequencyRule/getAllRule.action',
		});
	},800);
	$.ajax({
...
function submit(){
	...
	var url = "<%=basePath%>emrs/frequencyRule/addrule.action";
	if(submitType==1){
		url = "<%=basePath%>emrs/frequencyRule/updateRule.action";
	}
...
function updatevalue(){
	...
	$('#ruleunit').combobox('setValue',row.unit);
...
function delvalue(){
	var row = $('#list').datagrid('getSelected');
	$.ajax({
		url:"<%=basePath%>emrs/frequencyRule/delRule.action",
		data:{id:row.id}
		,success: function(data){
			$.messager.progress('close');
        	$.messager.alert('提示',data.resMsg,'info',function(){
    	...
    	<table id="list" class="easyui-datagrid" style="width:100%" data-options="rownumbers:true,striped:true,border:true,checkOnSelect:true,selectOnCheck:false,fitColumns:true,singleSelect:true,fit:true">   
		    <thead>   
		        <tr>   
		            ...
		            <th data-options="field:'code',width:100">规则编码</th>   
		            <th data-options="field:'name',width:100">规则名称</th>   
		            <th data-options="field:'desc',width:100">规则描述</th>   
		            <th data-options="field:'group',width:100">规则分组</th>   
		            <th data-options="field:'responsibilityEvel',width:100">责任级别</th>   
		            <th data-options="field:'value',width:100">频率要求</th>   
		            <th data-options="field:'unit',width:100,formatter:formatedata">时间</th>   
		            <th data-options="field:'tip',width:100">提示</th>   
		...
		<div data-options="region:'center',split:true,border:false">
			...
			<table class="honry-table" cellpadding="0" cellspacing="0" border="0" style="margin: 10px auto 0;">
				...
				<tr>
					...
					<td class="honry-info">
						<input style="width: 200px;" class="easyui-combobox" id="ruleunit"
									name="rule.unit"
									value="${rule.unit }"
									data-options="required:true,valueField:'id',textField:'text',data:[{id:1,text:'周'},{id:2,text:'天'},{id:3,text:'小时',selected:true},{id:4,text:'分钟'},{id:5,text:'秒'}]"/>
					</td>
				</tr>
				<tr>
					<td class="honry-lable">
