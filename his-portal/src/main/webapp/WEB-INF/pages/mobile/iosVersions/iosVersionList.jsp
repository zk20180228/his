				...
				<form id="search" method="post">
					<table style="width:100%;border-bottom:1px solid #95b8e7;padding:6px;" class="changeskinBottom">
						<tr>
							...
							&nbsp;是否推送广播：<input class="easyui-combobox" id="sendFlg" value="7" />
			...
			<table id="list" fit="true" data-options="method:'post',rownumbers:true,striped:true,border:false,checkOnSelect:true,selectOnCheck:false,singleSelect:true,fitColumns:false,toolbar:'#toolbarId'">
				<thead>
					<tr>
						...
						<th data-options="field:'apkDownloadAddr',align:'center',width:'27%'">APPSTORE下载地址</th>
						<th data-options="field:'apkDownloadQRAddr',align:'center',width:'27%'">二维码</th>
						<th data-options="field:'sendRadio',formatter:formatFlg,align:'center',width:'5%'">是否推送广播</th>
					</tr>
				</thead>
			</table>
		</div>
...
<script type="text/javascript">
	$(function(){
		...
		$('#list').datagrid({
			...
			url: '<%=basePath %>mosys/iosUpdateVersion/findIosVersionList.action',
			onBeforeLoad:function(){
				$('#list').datagrid('clearChecked');
				$('#list').datagrid('clearSelections');
			},
			onDblClickRow: function (rowIndex, rowData) {//双击查看
				AddOrShowEast('查看',"<%=basePath %>mosys/iosUpdateVersion/toViewIosVersion.action?id="+rowData.id);
			   	},
	...
	*/
	function searchFrom() {
		...
		var sendFlg = $('#sendFlg').combobox('getValue');
		$('#list').datagrid('load',{queryNum:queryNum,queryName:queryName,queryFlg:queryFlg,sendFlg:sendFlg});
	}
	...
	*/
		...
		if(rows.length > 0){
			$.messager.confirm('提示','您确认要删除吗？',function(r){    
	  		    if (r){ 
					...
					$.ajax({
						url:"<%=basePath%>mosys/iosUpdateVersion/delIosVersions.action",
						async:false,
						cache:false,
						data:{'ids':ids},
						type:"POST",
	...
	//添加
		function add(){
		   AddOrShowEast('添加',"<%=basePath %>mosys/iosUpdateVersion/toAddIosVersion.action");
	}
	//修改	
	function edit(){
		var row = $('#list').datagrid('getSelected');
		  if(row != null){
               AddOrShowEast('编辑',"<%=basePath %>mosys/iosUpdateVersion/toEditIosVersion.action?id=" + row.id);
   		  }else{
