...
<body >
	...
	<div id="toolbarId">
		...
		<a href="javascript:void(0)" onclick="moveWhite()" class="easyui-linkbutton" data-options="iconCls:'icon-change',plain:true">移动至白名单</a>
...
var sexList = '';
	...
	$.ajax({
		...
		$('#list').datagrid({
			...
			url: '<%=basePath %>mosys/blackList/findBlackLists.action',
			onBeforeLoad:function(){
				$('#list').datagrid('clearChecked');
				$('#list').datagrid('clearSelections');
			},
			onDblClickRow: function (rowIndex, rowData) {//双击查看
				AddOrShowEast('查看',"<%=basePath %>mosys/blackList/toViewBlack.action?id="+rowData.id);
			   	},
	...
	*/
		...
		if(rows.length > 0){
			$.messager.confirm('提示','您确认要删除吗？',function(r){    
	  		    if (r){ 
					...
					$.ajax({
						url:"<%=basePath%>mosys/blackList/delBlackLists.action",
						async:false,
						cache:false,
						data:{'ids':ids},
						type:"POST",
	...
	//添加
		function add(){
		   AddOrShowEast('添加',"<%=basePath %>mosys/blackList/toAddBlack.action");
	}
	//修改	
	function edit(){
		var row = $('#list').datagrid('getSelected');
		  if(row != null){
               AddOrShowEast('编辑',"<%=basePath %>mosys/blackList/toEditBlack.action?id=" + row.id);
   		  }else{
	...
	*/
	function moveWhite() {
		...
		if(rows.length > 0){
			...
			for(var i = 0; i < rows.length; i++){
					if(userAccount != ''){
						userAccount += ','+rows[i].user_account;
					}else{
						userAccount += rows[i].user_account;
					}
			}
						...
						if(data.resCode=='0'){
							$.messager.confirm('提示','您确认要移至白名单吗？',function(r){    
					  		    if (r){  
					  				...
					  				$.ajax({
					  					url:"<%=basePath%>mosys/blackList/moveWhite.action",
					  					async:false,
					  					cache:false,
					  					data:{'ids':ids},
					  					type:"POST",
