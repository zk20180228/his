...
<body >
	...
	<div id="toolbarId">
		...
		<a href="javascript:void(0)" onclick="moveBlack()" class="easyui-linkbutton" data-options="iconCls:'icon-change',plain:true">移动至白名单</a>
...
<script type="text/javascript">
	$(function(){
		$('#list').datagrid({
			...
			url:'<%=basePath%>mosys/blackListManage/findBlackManage.action',
	...
	*/
		...
		if(rows.length > 0){
			$.messager.confirm('提示','您确认要删除吗？',function(r){    
	  		    if (r){ 
	  				...
	  				$.ajax({
	  					url:"<%=basePath%>mosys/blackListManage/delBlackManage.action",
	  					async:false,
	  					cache:false,
	  					data:{'ids':ids},
	  					type:"POST",
	...
	//添加
	function add(){
   		Adddilog("添加",'<%=basePath%>mosys/blackListManage/addBlackManage.action?');
	}
	//修改	
	function edit(){
		var row = $('#list').datagrid('getSelected');
		  if(row != null){
			  Adddilog("编辑",'<%=basePath%>mosys/blackListManage/editBlackManage.action?id='+ row.id);
   		  }else{
	...
	//修改	
	function synCach(){
		$.messager.confirm('提示','您确认要同步至缓存吗？',function(r){    
  		    if (r){ 
  				$.ajax({
  					url:"<%=basePath%>mosys/blackListManage/synCach.action",
  					async:false,
  					cache:false,
  					type:"POST",
  					success:function(data){
		...
		if(rows.length > 0){
			$.messager.confirm('提示','您确认要移至白名单吗？',function(r){    
	  		    if (r){  
	  				...
	  				$.ajax({
	  					url:"<%=basePath%>mosys/blackListManage/moveWhite.action",
	  					async:false,
	  					cache:false,
	  					data:{'ids':ids},
	  					type:"POST",
