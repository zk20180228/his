...
<script type="text/javascript">
		...
		$(function(){
			$('#list').datagrid({
				nowrap:false,    
 	            striped:true,    
 	            url:'<%=basePath%>mosys/moExceptionLog/queryMobileException.action',   
 	            sortName: 'id',    
 	            sortOrder: 'desc',    
 	            idField:'id',
 	            pagination:true,
			...
			// 告警级别  1：高，2：中，3：低
			warnLevelMap.put("1","高");
			warnLevelMap.put("2","中");
			warnLevelMap.put("3","低");
        ...
        if (iid.length > 0) {//选中几行的话触发事件	                        
		 	 $.messager.confirm('确认', '确定要处理选中异常吗?', function(res){//提示是否处理
				if (res){
					...
					$.ajax({
						url:"<%=basePath%>mosys/moExceptionLog/startDeal.action?id="+ids,
						type:'post',
						success: function() {
						$('#list').datagrid('reload');
						}
        ...
        if (iid.length > 0) {//选中几行的话触发事件	                        
		 	 $.messager.confirm('确认', '异常是否处理完成?', function(res){//提示是否删除
				if (res){
					...
					$.ajax({
						url: "<%=basePath%>mosys/moExceptionLog/endDeal.action?id="+ids,
						type:'post',
						success: function() {
						$('#list').datagrid('reload');
						}
...
<body style="margin: 0px; padding: 0px">
   <div id="divLayout"  style="width:100%;height:100%;overflow-y: auto;">
   		...
   		<div data-options="region:'center',border:false" style="height:calc(100% - 40px);width:100%;">
...
<!-- 						<th data-options="field:'MSG'" style="width: 14%">异常信息</th> -->
						<th data-options="field:'MSG_DETAIL_show'" style="width: 10%">异常详细信息</th>
<!-- 						<th data-options="field:'MSG_DETAIL'" style="width: 10%">异常详细信息</th> -->
					</tr>
				</thead>
			</table>
   		</div>
	</div>
	<div id="msgWinId" style="display: none;">
