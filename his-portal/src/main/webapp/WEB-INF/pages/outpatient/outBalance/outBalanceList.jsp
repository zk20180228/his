...
<script type="text/javascript">
	...
	function stamp(){
		$.messager.confirm('确认','要打印费用清单吗？',function(r){
			if(r){
				...
				var medicalrecordIdSearch=$('#inpatientNo').val();
				if(medicalrecordIdSearch!=null&&medicalrecordIdSearch!=''){
					window.open ("<c:url value='/inpatient/outbalanceout/printBalace.action?randomId='/>"+timerStr+"&medicalrecordIdSearch="+medicalrecordIdSearch,'newwindow','height=900,width=850,top=50,left=500,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
					clear(true);
				}else{
	...
	function printInvoBill(){
		...
		if(cardNo==null||cardNo==""){
			$.messager.alert('提示','病历号为空,请填写就病历号！');
			setTimeout(function(){
				$(".messager-body").window('close');
			},3500);
			return;
		...
		var timerStr = Math.random();
		window.open ("<c:url value='/iReport/iReportPrint/iReportToPrintInvo.action?randomId='/>"+timerStr+"&cardNo="+cardNo+"&fileName=YLSFPJ",'newwindow','height=900,width=850,top=50,left=500,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
	}
	...
	$(function(){
		...
		$('#shishou1').numberbox({
			onChange: function (newValue, oldValue) {
				if(newValue==''){
					$('#zhaoling1').textbox('setValue',$('#yingshou').val());
					$('#zhaoling2').textbox('setValue',$('#yingshou').val());
				}else{
					var ys = $('#yingshou').val();
					var sh = $('#shishou1').val();
		...
		$('#shishou2').textbox({
			onChange: function (newValue, oldValue) {
				if(newValue==''){
					$('#zhaoling2').textbox('setValue',$('#yingfan').textbox('getValue'));
				}else{
					var ys = $('#yingfan').val();
					var sh = $('#shishou2').val();
					var zl = parseFloat(sh)+parseFloat(ys);
		...
		$('#shishou3').textbox({
			onChange: function (newValue, oldValue) {
				if(newValue==''){
					$('#zhaoling3').textbox('setValue',$('#yingfan2').val());
					$('#zhaoling2').textbox('setValue',$('#yingfan2').val());
				}else{
					var ys = $('#yingfan2').val();
					var sh = $('#shishou3').val();
					if(parseInt(sh)>parseInt(ys)){
		...
		bindEnterEvent('medicalrecordId1',search,'easyui');
		//初始化无数据列表
	...
	function read_card_ic(){
		...
		$.ajax({
			...
			success: function(data) { 
				...
				$('#medicalrecordId1').textbox('setValue',data);
				search();
			}
		});
	};
	...
	function search(){
		var medicalrecordId = $('#medicalrecordId1').val();//病历号
		if(medicalrecordId == ''||medicalrecordId==null){
			...
			queryByInpatient(medicalrecordId);
		}
	}
	function queryInfo(dataObj,number){
		if(number=="1"){
			if(dataObj[0].inState=='B'||dataObj[0].inState=='P'){
				//住院流水号
				...
				//病历号
				$('#medicalrecordId1').textbox('setValue',dataObj[0].medicalrecordId);
				//根据住院流水号   获取最大的结算序号
				...
				$.messager.alert('提示','患者不是出院登记状态，不能进行出院结算！');
				setTimeout(function(){
					$(".messager-body").window('close');
				},3500);
				return;
			...
			if(dataObj.inState=='B'||dataObj.inState=='P'){
				//住院流水号
				...
				//病历号
				$('#medicalrecordId1').textbox('setValue',dataObj.medicalrecordId);
				//根据住院流水号   获取最大的结算序号
		...
		$.ajax({
			...
			success: function(Cancelitem){
				if(Cancelitem.resCode=="2"){
					$.messager.alert('提示','患者存在未确认的退费申请，不能进行出院结算！'); 
					setTimeout(function(){
						$(".messager-body").window('close');
					},3500);
				...
				if(drgs=="1"){
					$.messager.confirm('确认', '是否使用单病种优惠吗?', function(res){
						if (res){
							...
							//获取减免金额
							$.ajax({
								url : '<%=basePath%>inpatient/outbalanceout/queryInpatientDerateList.action',
								data:{inpatientNo:inpatientNo,inDate:inDate,outDate:outDate},
								type:'post',
								success:function(date){
									$("#jmMoney").textbox('setValue',date);//减免总额
									//计算费用
									feiyong(inpatientNo);
								}
							});
		...
		$.ajax({
			...
			success: function(cos){
				...
				if(sfff>0){
					...
					$('#yingshou').textbox('setValue','');
					$('#shishou2').textbox('setValue','0');//找零
					$("#showw").hide();
					$("#hidee").show();
					$("#hidee1").hide();
					$('#payWays').combobox({
		...
		$.ajax({//查询住院信息
			...
			success: function(dataObj) {
				if(dataObj.length==0){
					...
					//显示信息
					if(dataObj[0].freeCost<0){
						clear();
						$.messager.alert('提示','患者欠费应进行欠费结算！');
						setTimeout(function(){
							$(".messager-body").window('close');
						},3500);
						return;
	 ...
	 * @version 1.0
	 */
	function queryByInpatient(medicalrecordId){
		$.ajax({//查询住院信息
			url: '<%=basePath%>inpatient/outbalanceout/queryBymedicalrecordId.action',
			data:{medicalrecordIdSearch:medicalrecordId},
			type:'post',
			success: function(dataObj) {
				if(dataObj.length==0){
						...
						if(date=="success"){
							...
							if(sh=="0"||sh1=="0"||sh2=="0"){
								...
								$('#editForm').form('submit',{
									...
									url:'<%=basePath%>inpatient/outbalanceout/saveBalance.action',
		...
		//清空
		function clear(){
			$('#dayin').linkbutton('disable');
			$('#medicalrecordId1').textbox('setValue','');
			$('#patientName').text('');
			$('#deptCode').text('');
			$('#paykindCode').text('');
			$('#inpatientNo').val('');
...
<body style="margin: 0px; padding: 0px"> 
	<div id="cc" class="easyui-layout" data-options="fit:true">   
			...
			<div id="bb" class="easyui-layout"  data-options="fit:true">   
				<div data-options="region:'north',border:false" style="height:50px;padding-top:10px;">
					<shiro:hasPermission name="${menuAlias}:function:query">
						<a href="javascript:search();void(0)"   class="easyui-linkbutton" iconCls="icon-search" style="width: 65px;height: 25px;">查询</a>
					</shiro:hasPermission>
					...
					<shiro:hasPermission name="${menuAlias}:function:print">
						<a href="javascript:stamp();void(0)" class="easyui-linkbutton" id="dayin" data-options="iconCls:'icon-2012081511202',disabled:true" style="width: 65px;height: 25px;">打印</a>
					</shiro:hasPermission>
					...
					&nbsp;&nbsp;
						...
						到
						<input id="outDate" class="Wdate" type="text" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'%y-%M-%d 23:59:59'})"  style="width:150px;border: 1px solid #95b8e7;border-radius: 5px;"/>
				...
				<div data-options="region:'center',border:false" style="height: 100px" >
					<table id="colors" class="honry-table" align="center" style="width: 100%;height: 100%">
						<tr>
							<td class="honry-lable" style="width: 12%;">病历号：</td>
							<td style="width: 12%;"><input name="medicalrecordId" id="medicalrecordId1" data-options="prompt:'请输入病历号查询'" class="easyui-textbox" >
		...
		<div data-options="region:'center',border:false,iconCls:'icon-book'" style="height:28%;width: 100%">
			<div id="divLayout" class="easyui-layout outBalanceList" data-options="fit:true,border:true">
				<div data-options="region:'west',border:false,title:'费用明细列表',iconCls:'icon-book'" style="width:49%;height:100%;">
					<table class="easyui-datagrid" id="infolist" data-options="method:'post',border:false,fit:true,rownumbers:true,idField: 'id'">
						<thead>
							<tr>
				...
				<div data-options="region:'center',border:true,title:'预交金列表',iconCls:'icon-book'" style="width:50%;height:100%">
		...
		<div data-options="region:'south'" style="height:50%;width: 100%">
			<div id="aa" class="easyui-layout" data-options="fit:true">   
				<div data-options="region:'north',border:false" id="showw"	style="width:100%;height:80px;">
					<fieldset id="one" class="changeskin"><legend><font style="font-weight: bold;font-size: 12px;">补收款项</font></legend>
					<table class="honry-table" style="width: 100%;">
						<tr>
							...
							<td  style="width: 20%" >
								&nbsp;&nbsp;
								<input name="" id="shishou1" class="easyui-numberbox" >
							</td>
				...
				<div data-options="region:'center',border:false,title:'支付列表'" style="width: 100%; ">
						...
						<input type="hidden" id="costJson" name="costJson">
							<table id="infolist3" class="easyui-datagrid"  data-options="checkOnSelect:true,selectOnCheck:true,
					...
					<div id="toolbarId">
						...
						<span style='color:#C24641;font-size:12px;'>　选择支付方式后，请双击填写支付信息</span>
					</div>	
				</div>
			</div>
		</div>
