...
<script type="text/javascript">
	...
	function liandong(){
		...
		var totle;
		if(sign==''){
			totle=$('#zhaoling1').textbox('getValue');//找零
		}else{
		...
		if(len>1){
			...
			if(null==totle||''==totle){
				for(var i=0;i<len;i++){
					...
					$(editors.target).textbox('setValue','0');
				}
			}else{
				for(var i=1;i<len;i++){
					editors= $('#infolist3').datagrid('getEditor',{index:i,field:'cost'});
			...
			var editor=$('#infolist3').datagrid('getEditor',{index:0,field:'cost'});
			if(null==totle||''==totle){
				$(editor.target).textbox('setValue','0');
			}else{
				$(editor.target).textbox('setValue',totle.toFixed(2));
			}
	...
	function stamp(){
		$.messager.confirm('确认','要打印费用清单吗？',function(r){
			if(r){
				...
				var medicalrecordIdSearch=$('#medicalrecordId').val();
				if(medicalrecordIdSearch!=null&&medicalrecordIdSearch!=''){
					...
					$.messager.alert('提示','就诊卡号为空,请填写就诊卡号！');
					setTimeout(function(){
	...
	$(function(){
		...
		$('#shishou1').textbox({//支付
			onChange: function (newValue, oldValue) {
				if(newValue==''){
					...
					var sh = $('#shishou1').textbox('getValue');
					if(parseInt(sh)<0){
						$.messager.alert('提示','金额不能为负！');
						return;
					}
					var ys = $('#yingshou').textbox('getValue');
		...
		$('#payWays').combobox({
			...
			onSelect: function(param){
				...
				var num
				if(sign==''){
					num=$('#zhaoling1').textbox('getValue');//找零
				}else{
	...
	function queryInfo(dataObj,number){
		if(number=="1"){
			if(dataObj[0].inState=='B'||dataObj[0].inState=='P'){
				...
				$.messager.alert('提示','患者不是出院登记状态，不能进行欠费结算！');
				setTimeout(function(){
					$(".messager-body").window('close');
				},3500);
				return;
		...
		$.ajax({
			...
			success: function(Cancelitem){
				if(Cancelitem.resCode=="2"){
					$.messager.alert('提示','患者存在未确认的退费申请，不能进行欠费结算！');
					setTimeout(function(){
						$(".messager-body").window('close');
					},3500);
	...
	function shjs(){
		...
		if(sfff>0){
			...
			$('#zhaoling1').textbox('setValue',-sfff);
			...
			success: function(dataObj) {
				if(dataObj.length==0){
					...
					if(dataObj[0].freeCost>0){
						$.messager.alert('提示','患者并没有欠费应进行出院结算！');
						setTimeout(function(){
							$(".messager-body").window('close');
						},3500);
						return;
	...
	function baocun() {
		...
		if(medicalrecordId==''||medicalrecordId==null){
			$.messager.alert('提示','请先输入就诊卡号进行查询！');
			setTimeout(function(){
				$(".messager-body").window('close');
			},3500);
		}else{
			//封账前金额校验
			var total=$('#zhaoling1').textbox('getValue');//总金额
			if(total==null||total==''){
				$.messager.alert('提示','找零为空');
				return false;
			}
			var editor;//支付金额
			var rows=$('#infolist3').datagrid('getRows');
...
<style type="text/css">
.arrearageBalanceList .panel-header{
	border-top:0
}
</style>
</head>
<body style="margin: 0px; padding: 0px"> 
	<div id="cc" class="easyui-layout" data-options="fit:true">   
				...
				<div data-options="region:'center',border:false" style="height: 100px" >
					<table id="colors" class="honry-table" align="center" style="width: 100%;height: 100%">
						<tr>
							...
							<td style="width: 12%;" id="paykindCode" name="paykindCode"></td>
						</tr>
						<tr>
		...
		<div data-options="region:'center',border:false,iconCls:'icon-book'" style="height:28%;width: 100%">
			<div id="divLayout" class="easyui-layout arrearageBalanceList" data-options="fit:true,border:true">
