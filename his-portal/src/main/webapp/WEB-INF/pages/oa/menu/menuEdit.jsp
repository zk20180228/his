    		...
    		<form id="editForm" method="post" ">
				...
				<table class="honry-table" cellpadding="0" cellspacing="0" border="0" style="margin:5px auto 0;">
	    			...
	    			<tr>
		    			...
		    			<td class="honry-info">
		    			<input type="hidden" id="mview" name = "mview" value="${menu.mview}"/>
		    			<input type="text"  class="easyui-textbox" data-options="buttonIcon:'icon-folder'" id="mmview"  style="width: 300" value="${mmview}"  /></td>
	    			</tr>
	    			<tr>
...
//			console.log($('#mmcheck').parent().children())
		...
		$(function(){
		    ...
		    $("#mmpublish").textbox({onClickButton:function(){
		    	$("#mpublish-window .iframeid")[0].src = '<%=basePath%>oa/menuManager/addRight.action?type=1'
	    		$('#mpublish-window').window('open');
		    }})
		    
		
		    $("#mmview").textbox({onClickButton:function(){
		    	$("#mview-window .iframeid")[0].src = '<%=basePath%>oa/menuManager/addRight.action?type=3'
	    		$('#mview-window').window('open');
		    }})
		
		    $("#mmcheck").textbox({onClickButton:function(){
		    	$("#mcheck-window .iframeid")[0].src = '<%=basePath%>oa/menuManager/addRight.action?type=2'
		...
		function submitForm(){
			$('#editForm').form('submit',{  
				url:'<%=basePath%>oa/menuManager/saveMenu.action', 
	        	onSubmit:function(){
