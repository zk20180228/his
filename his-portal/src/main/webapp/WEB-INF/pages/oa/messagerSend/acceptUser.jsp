	...
	<head>
		...
		<script type="text/javascript">
			...
			$(function(){
				...
				//回显
				var hadSelect = window.opener.$('#acceptUserH').val();
				if(!(typeof hadSelect === "undefined")){
		...
		<div class="header">
			<div class="nav radio radio-theme">
				...
				<div class="lableBox">
					...
					<label for="selectPerson">科室</label>
				</div>
				<div class="lableBox">
					<input type="radio" value="2" id="selectSection" name="selectCon">
					<label for="selectSection">医学职称</label>
				</div>
				<div class="lableBox">
					<input type="radio" value="5" id="selectTeachingSection" name="selectCon">
					<label for="selectTeachingSection">教学职称</label>
				</div>
				<div class="lableBox">
					<input type="radio" value="3" id="selectPost" name="selectCon">
					<label for="selectPost">党内职务</label>
				</div>
				<div class="lableBox">
					<input type="radio" value="6" id="selectPartyPost" name="selectCon">
					<label for="selectPartyPost">党内职务</label>
				</div>
				<div class="lableBox">
					<input type="radio" value="4" id="selecRank" name="selectCon">
					<label for="selecRank">人员</label>
				</div>
			</div>
		</div>
...
<!-- 								 <a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-search'" onClick="searchTreeNodes()" style="margin-top: 2px;">搜索</a> -->
			...
			<div id="content5" class="contentnav">
				...
				<div class="topContent">
					<div class="panel-body" style="padding: 20px">
						<div style="width: 540px" class="tableMain">
							<table id="teachingPostlist" ></table>
						</div>
						<div class="slelctMain">
							<div class="btnAllBox">
								<div class="btnBox">
									<button onclick="add5()" type="button" class="btn btn-success"><span>添&nbsp;&nbsp;加 <i class="honryicon icon-angle_right"></i></span></button>
									<button onclick="del5()" type="button" class="btn btn-danger"><span><i class="honryicon icon-angle_left"></i> 移&nbsp;&nbsp;除</span></button>
									<button onclick="delAll(5)" type="button" class="btn btn-danger"><span><i class="honryicon icon-double_angle_left"></i> 移除全部</span></button>
								</div>
							</div>
							<div class="selectBox">
								<select multiple="multiple" id="selectAll5" style="width:150px;height:100%;"></select>
							</div>
						</div>
					</div>
				</div>
			...
			<div id="content6" class="contentnav">
				<!--行政职务 级别-->
				
				<div class="panel-body" style="padding: 20px">
					<div style="width: 540px" class="tableMain">
						<table id="partyGradelist"></table>
					</div>
					<div class="slelctMain">
						<div class="btnAllBox">
							<div class="btnBox">
								<button onclick="add6()" type="button" class="btn btn-success"><span>添&nbsp;&nbsp;加 <i class="honryicon icon-angle_right"></i></span></button>
								<button onclick="del6()" type="button" class="btn btn-danger"><span><i class="honryicon icon-angle_left"></i> 移&nbsp;&nbsp;除</span></button>
								<button onclick="delAll(6)" type="button" class="btn btn-danger"><span><i class="honryicon icon-double_angle_left"></i> 移除全部</span></button>
							</div>
						</div>
						<div class="selectBox">
							<select multiple="multiple" id="selectAll6" style="width:150px;height:100%;"></select>
						</div>
					</div>
				</div>
			</div>
...
<%-- 			url: "<%=basePath%>baseinfo/pubCodeMaintain/queryDictionary.action?type=title",//duties --%>
			url: "data/medicalTitles.json",//duties
			method: 'get',
			rownumbers: true,
			idField: 'id',
			striped: true,
		...
		//教学职称
		$('#teachingPostlist').datagrid({
			url: "data/teachingTitles.json",//duties
			method: 'get',
			rownumbers: true,
			idField: 'id',
			striped: true,
			...
			onDblClickRow: function(rowIndex, rowData) { //双击查看
				var tmp = true;
				$('#selectAll5 option').each(function(index, v) {
					if(v.getAttribute("value") == rowData.encode) {
						tmp = false
						return false;
					}
				...
				if(tmp){
					$("#selectAll5").append("<option value='" + rowData.encode + "'>" + rowData.name + "</option>")
				}
			}
		});
		//职位 党内职务
...
<%-- 			url: "<%=basePath%>baseinfo/pubCodeMaintain/queryDictionary.action?type=duties",//title --%>
			url: "<%=basePath%>data/partyDuties.json",//title
			method: 'get',
			rownumbers: true,
			idField: 'id',
			striped: true,
		...
		//职位  行政职务
		$('#partyGradelist').datagrid({
<%-- 			url: "<%=basePath%>oa/information/getDutyAuth.action", --%>
<%-- 			url: "<%=basePath%>baseinfo/pubCodeMaintain/queryDictionary.action?type=duties",//title --%>
			url: "data/administrativeDuties.json",//title
			method: 'get',
			rownumbers: true,
			idField: 'id',
			striped: true,
			...
			onDblClickRow: function(rowIndex, rowData) { //双击查看
				var tmp = true;
				$('#selectAll6 option').each(function(index, v) {
					if(v.getAttribute("value") == rowData.encode) {
						tmp = false
						return false;
					}
				...
				if(tmp){
					$("#selectAll6").append("<option value='" + rowData.encode + "'>" + rowData.name + "</option>")
				}
			}
		});
		//职位的
		...
		function del5() {
			$('#selectAll5 option:selected').remove()
		}
		function del6() {
			$('#selectAll6 option:selected').remove()
		}
		function add3(){
		...
		function add6(){
			var postlist = $('#partyGradelist').datagrid('getChecked');
			var tmp,str = "";
			for(var i=0;i<postlist.length;i++){
				tmp = true
				$('#selectAll6 option').each(function(index, v) {
			...
			if(str != "") {
				$('#selectAll6').append(str)
			}
		}
		function add5(){
			var postlist = $('#teachingPostlist').datagrid('getChecked');
			var tmp,str = "";
			for(var i=0;i<postlist.length;i++){
				tmp = true
				$('#selectAll5 option').each(function(index, v) {
		...
		function delAll(value) {
			...
			if(value==6){
				$('#partyGradelist').datagrid('uncheckAll');
			}
			if(value==5){
				$('#teachingPostlist').datagrid('uncheckAll');
			}
			if(value==4){
		...
		function submitSelect(){
			...
			var value2 = "";//选中的医学职称
			var value2Name = "";//选中的医学职称
			var value3 = "";//选中的级别
			var value3Name = "";//选中的级别
			var value4 = "";//选中的人员
			var value4Name = "";//选中的人员
			var value5 = "";//选中的教学职称
			var value5Name = "";//选中的教学职称
			var value6 = "";//选中的行政职务
			var value6Name = "";//选中的行政职务
			if($('#All').is(':checked')){
				value = 'all_全部人员';
				valueName = "全部人员";
				window.opener.$('#acceptUserH').val(value)
				window.opener.showbackUser(valueName);
				closeWin();
				return ;
			}
			$('#selectAll1 option').each(function(index, v){
			...
			$('#selectAll2 option').each(function(index, v){//职务
				if(value3!=""){
					value3 += ",";
					value3Name += "、";
				}
			...
			$('#selectAll5 option').each(function(index, v){
				...
				value5 += v.getAttribute("value")+":"+v.text;
				value5Name += v.text;
			});
			$('#selectAll6 option').each(function(index, v){
				if(value6!=""){
				...
				value6 += v.getAttribute("value")+":"+v.text;
				value6Name += v.text;
			});
			var showbackname = "";
			if(valueName!=""){
				...
				if(value1Name!=""){
					showbackname += "【科室】:"+value1Name+";";
				}
				if(value2Name!=""){
					showbackname += "【医学职称】:"+value2Name+";";
				}
				if(value3Name!=""){
					showbackname += "【党内职务】:"+value3Name+";";
				}
				if(value4Name!=""){
					showbackname += "【人员】:"+value4Name+";";
				}
				if(value5Name!=""){
					showbackname += "【教学职称】:"+value5Name+";";
				}
				if(value6Name!=""){
					showbackname += "【行政职务】:"+value6Name+";";
				}
			}
			window.opener.$('#acceptUserH').val("3_"+value1+"#"+"4_"+value2+"#"+"2_"+value3+"#"+"1_"+value4+"#5_"+value5+"#6_"+value6);
			window.opener.showbackUser(showbackname);
			closeWin();
		}
		function closeWin(){
			window.close();
