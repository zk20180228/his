				...
				<table id="list" data-options="method:'post',rownumbers:true,striped:true,border:true,checkOnSelect:true,selectOnCheck:false,singleSelect:true,fitColumns:false,toolbar:'#toolbarId',fit:true">
					<thead>
						<tr>
							...
							<th data-options="field:'diseaseName'" style="width:8%">分类</th>  
							<th data-options="field:'name'" style="width:10%">名称</th>	
							<th data-options="field:'code'" style="width:7%">编码</th>
...
<!--     		</div> -->
	...
	$('#list').datagrid({
		url:'<%=basePath %>/oa/repositoryCateg/getCateg.action',
		onDblClickRow : function(rowIndex, rowData) {//双击查看
			var row = $('#list').datagrid('getSelected'); //获取当前选中行    
            if(row){
           		AddOrShowEast('EditForm','<%=basePath %>oa/repositoryCateg/toCategView.action?cateid='+rowData.id);
		   	}
		},
	...
	function stopformatter(value,row,index){		
		...
		function add(){
			...
			AddOrShowEast('EditForm','<%=basePath %>oa/repositoryCateg/toCategADD.action?deptCode='+nodeid);			
		}
		//弹出修改编辑区域
		function edit(){
			var row = $('#list').datagrid('getSelected'); //获取当前选中行         
            	if(row){            		
            		AddOrShowEast('EditForm','<%=basePath %>oa/repositoryCateg/toCategEdit.action?cateid='+row.id);
				}else{
		    ...
		    if (iid.length > 0) {//选中几行的话触发事件	                        
				$.messager.confirm('确认', '确定要删除选中信息吗?', function(res){//提示是否删除
					if (res){
						...
						$.ajax({
							url: '<%=basePath %>oa/repositoryCateg/delCate.action',
							data:'cateid='+ids,
							type:'post',
							success: function() {
