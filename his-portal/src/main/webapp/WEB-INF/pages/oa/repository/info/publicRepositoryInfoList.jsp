		...
		<div data-options="region:'center',border:true" class="deleteBorder" style="border-top:0">
			<div style="width:100%;height:6%;">    		
				<table style="width:100%;height:100%;padding:0px;margin: 0px;" data-options="fit:true,border:true">
					<tr>
						<td>
							标题 ：<input class="easyui-textbox"  name="nameSerc" id="nameSerc" data-options="prompt:'请输入标题'" style="width: 200px;" />
...
<!--     		</div> -->
	...
	$(function(){
		var id='${id}'; //存储数据ID
		//添加datagrid事件及分页
		bindEnterAndBlackEvent('searchTreeInpId', searchTreeNodes,'easyui');
		$('#list').datagrid({
			pagination:true,
			pageSize:20,
	...
	$('#list').datagrid({
		...
		onDblClickRow : function(rowIndex, rowData) {//双击查看
            ...
            if(row){
            	...
            	openNav("<%=basePath%>oa/repositoryInfo/toPersonalInfoView.action?infoid="+row.id+"&isShowCollect=1&tabName="+tabName+"&tabID="+tabID+"&patName="+patName, tabName, tabID);
		   	}
		},
	...
	function stopformatter(value,row,index){		
		...
		function add(){
			...
			var tabName = '知识库添加' ;
        	var tabID = 'publicinformationaddid';
        	var patName = '公共知识库';
			openNav('<%=basePath %>oa/repositoryInfo/toPersonalInfoAdd.action?deptCode='+node.id+'&tabName='+tabName+'&tabID='+tabID+'&patName='+patName, tabName, tabID);
		}
		//弹出修改编辑区域
		function edit(){
			var row = $('#list').datagrid('getSelected'); //获取当前选中行         
			if(row != null){
				if(account != row.createUser){
					$.messager.alert('提示','不可修改别人发布的信息...','info');
					return ;
				}else{
					var tabName = '知识库修改' ;
				...
				$.messager.confirm('确认', '确定要删除选中信息吗?', function(res){//提示是否删除
					if (res){
						...
						for(var i=0; i<iid.length; i++){
							if(account == iid.createUser){
								$.messager.progress('close');
								$.messager.alert('提示','不可删除非本人发布信息...','info');
								return ;
							}
							if(ids!=''){
								ids += ',';
