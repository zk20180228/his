...
<body>
	<div id="divLayout" class="easyui-layout" data-options="fit:true">
		<div data-options="region:'west',tools:'#toolSMId',title:'知识库分类查询',split:true" style="width: 14%;">
			<div id="toolSMId">
...
<!--     		</div> -->
	...
	$('#list').datagrid({
		url:'<%=basePath %>/oa/repositoryInfo/getPublicInfo.action',
		onDblClickRow : function(rowIndex, rowData) {//双击查看
			var row = $('#list').datagrid('getSelected'); //获取当前选中行    
            if(row){
            	var tabName = '公共知识库文章浏览';
            	var tabID = 'publicinformationviewid';
            	openNav("<%=basePath%>oa/repositoryInfo/toPersonalInfoView.action?infoid="+row.id+"&isShowCollect=1&tabName="+tabName+"&tabID="+tabID, tabName, tabID);
<%--            		AddOrShowEast('EditForm','<%=basePath %>oa/repositoryInfo/toPersonalInfoView.action?cateid='+rowData.id); --%>
		   	}
		},
	...
	function stopformatter(value,row,index){		
		...
		function add(){
			...
			openNav('<%=basePath %>oa/repositoryInfo/toPersonalInfoAdd.action?deptCode='+node.id, '知识库文章添加', 'informationaddid');
<%-- 			AddOrShowEast('EditForm','<%=basePath %>oa/repositoryInfo/toPersonalInfoAdd.action?deptCode='+node.id);			 --%>
		}
		//弹出修改编辑区域
		function edit(){
			var row = $('#list').datagrid('getSelected'); //获取当前选中行         
            	if(row){            		
            		openNav('<%=basePath %>oa/repositoryInfo/toPersonalInfoEdit.action?infoid='+row.id, '知识库文章修改', 'informationeditid');
<%--             		AddOrShowEast('EditForm','<%=basePath %>oa/repositoryInfo/toPersonalInfoEdit.action?cateid='+row.id); --%>
				}else{
	 ...
	 // 列表查询重置
	    ...
	    */
		...
		function refresh() {//刷新树
			$('#tDt').tree('options').url = "<c:url value='/oa/repositoryCateg/getCategTree.action'/>?treeAll=" + false;
			$('#tDt').tree('reload');
		}
