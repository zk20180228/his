...
// 				加载节点树					
				$('#tt').tree({    
				    url:'<%=basePath%>oa/menuManager/showTreeWithCodeForTital.action',
				    method:'get',
				    animate:true,
				    lines:true,
			...
			function loadDatagrid(cid){
				...
				$('#dg').datagrid({
					url:'<%=basePath%>oa/information/getInfotmation.action',
			...
			function editContent(){
				...
				if(node.length > 0){
					AdddilogModel("addContent-window","栏目编辑",'<%=basePath%>oa/menuManager/editMenu.action?id='+node[0].id,'800px','900px');
				}else{
			...
			function del(){
				...
				$.messager.confirm('确认','确定要删除勾选的文章吗？',function(r){
					if(r){
						...
						$.ajax({
							url : '<%=basePath%>oa/information/delInformation.action',
							data: {menuId:menuid},
							success :function(r){
								reload();
				...
				$.ajax({
					...
					success : function(re){
						if(re.resCode=="success"){//进入浏览页面
							openNav('<%=basePath%>oa/information/toAdd.action?menuId='+tree.id, '文章添加', 'informationaddid');
						}else{
			...
			function edit(){
				...
				$.ajax({
					...
					success : function(re){
						if(re.resCode=="success"){//进入浏览页面
							openNav("<%=basePath%>oa/information/toEdit.action?menuId="+row.id, '文章修改', 'informationeditid');
						}else{
