...
UE.Editor.prototype.getActionUrl = function(action) {
    ...
    { 
    	...
    	var url = '<%=basePath%>sys/ueditorUpload/catcherImgUpload.action?comefrom=1&acount='+acount;
	...
	$('#fb0').filebox({    
	 ...
	 $('#infoMenuid').combotree({ 
	    ...
	    onSelect : function(record){//ispublish
	    	$.ajax({
	    		url : '<%=basePath%>oa/information/checkAuth.action?menuId='+record.id,
...
function saveForm(param){
	...
	$('#editForm').form('submit',{
	    url:'<%=basePath%>oa/information/editInformation.action?info.infoPubflag='+param+'&imgfilename='+imgfile,   
