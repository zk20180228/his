...
<body>
	<table id="searchBox" title="搜索结果" >
    <thead>
        <tr>
        	<th data-options="field:'ck',checkbox:true"></th>
        	<!-- <th data-options="field:'',width:'5%'">状态</th> -->
        	<th data-options="field:'sendName',width:'15%'">发件人</th>
            <th data-options="field:'subject',width:'50%'">主题</th>
            <th data-options="field:'folder',width:'6%',formatter:formatterFolder">所在文件夹</th>
            <th data-options="field:'sendDate',width:'13%'">时间</th>
            <th data-options="field:'emailSize',width:'5%',formatter:formatterSize">大小</th>
            <th data-options="field:'starFlg',width:'3%',align:'center',formatter:formatterStar">星标</th>
        </tr>
    </thead>
</table>
	<script type="text/javascript">
	    ...
	    function getSelectionsIds(){
	    	var sels = $("#searchBox").datagrid("getSelections");
	    	var ids = [];
	    	for(var i in sels){
	    		if(sels[i].id){
	    			ids.push(sels[i].id);
		...
		$('#searchBox').datagrid({
            fit:true,
			url:'<%=basePath%>oa/mail/searchMail.action',
            queryParams:{
                content:window.parent.$("#search").val()
            },
			method:'post',
			singleSelect:false,
			fitColumns:true,
            ...
            toolbar: [{
		        ...
		        handler:function(){
		        	...
		        	var row = $("#searchBox").datagrid("getSelections")[0];
                    var subject;
                    if(row.subject != null){
		        ...
		        handler:function(){
                    var ids = [];
                    var rows = $("#searchBox").datagrid("getSelections");
                    if(rows.length === 0){
                        $.messager.alert('提示','未选中任何邮件!');
                        return false;
                    }
...
//					    href: url,
        ...
        function formatterFolder(value) {
            switch (value){
                case 1 :
                    return "收件箱";
                case 2 :
