...
<body>
	<table id="draftBox" title="草稿箱" >
    <thead>
        <tr>
            <th data-options="field:'ck',checkbox:true"></th>
            <!-- <th data-options="field:'',width:'5%'">状态</th> -->
            <th data-options="field:'receiveName',width:'15%'">收件人</th>
	...
	<script type="text/javascript">
	    ...
	    function getSelectionsIds(){
	    	...
	    	var sels = draftBox.datagrid("getSelections");
	    	var ids = [];
	    	for(var i in sels){
	    		if(sels[i].id){
	    			ids.push(sels[i].id);
        ...
        $('#draftBox').datagrid({
            fit:true,
            url:'<%=basePath%>oa/mail/queryBox.action?folder=2&cid=${id}',
            method:'get',
            singleSelect:false,
            fitColumns:true,
            pagination:true,
            ...
            toolbar: [{
                ...
                handler:function(){
                    ...
                    $.messager.confirm('确认','删除草稿后邮件将无法恢复，您确定要删除吗？',function(r){
                        if (r){
                            markMail(ids, 5);
                        }
                    });
                ...
                handler:function(){
                    ...
                    var row = $("#draftBox").datagrid("getSelections")[0];
                    var subject;
                    if(row.subject != null){
                ...
                handler:function(){
                    var ids = [];
                    var rows = $("#draftBox").datagrid("getSelections");
                    if(rows.length === 0){
                        $.messager.alert('提示','未选中任何邮件!');
                        return false;
                    }
            ...
            onDblClickRow:function(index, row){
                ...
                if(tab){
                    ...
                    $tabs.tabs('add',{
                        title:title,
                        content : "<iframe id='editDraftIframe' src="+"<%=basePath%>oa/mail/page/editDraft.action?id="+row.id+" scrolling='no' frameborder='0' style='width:100%;height:100%;'></iframe>",
                        closable:true,
                        bodyCls:"content"
                    });
                }
