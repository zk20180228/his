...
<body>
	...
	<script type="text/javascript">
        ...
        function getSelectionsIds(){
	    	var sels = $("#receiveBox").datagrid("getSelections");
	    	var ids = [];
	    	for(var i in sels){
	    		if(sels[i].id){
	    			ids.push(sels[i].id);
		...
		$('#receiveBox').datagrid({
            fit:true,
			url:'<%=basePath%>oa/mail/queryBox.action?folder=1&cid=${id}',
			method:'get',
			singleSelect:false,
			fitColumns:true,
			pagination:true,
            ...
            toolbar: [{
		        ...
		        handler:function(){
		        	...
		        	var row = $("#receiveBox").datagrid("getSelections")[0];
                    var subject;
                    if(row.subject != null){
		        ...
		        handler:function(){
                    var ids = [];
                    var rows = $("#receiveBox").datagrid("getSelections");
                    if(rows.length === 0){
                        $.messager.alert('提示','未选中任何邮件!');
                        return false;
                    }
