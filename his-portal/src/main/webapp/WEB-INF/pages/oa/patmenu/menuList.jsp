...
// 				加载节点树					
			...
			function loadDatagrid(cid){
				$("#toolbarId").show();
				$('#dg').datagrid({  
					url:'<%=basePath%>oa/patMenuManager/queryMenuById.action?id='+cid,
					method:'get',
					toolbar:"#toolbarId",
					singleSelect:true,
					onDblClickRow : function(row,index){
						var code = index.code;
						AdddilogModel("addContent-window","栏目查看",'<%=basePath%>oa/patMenuManager/menuInfo.action?code='+code,'40%','60%');
					},
					columns:[[    
...
//					          },},    
			...
			function addContent(){
				...
				if(nodes == null){
					AdddilogModel("addContent-window","栏目添加",'<%=basePath%>oa/patMenuManager/addMenu.action','40%','60%');
				}else{
					AdddilogModel("addContent-window","栏目添加",'<%=basePath%>oa/patMenuManager/addMenu.action?id='+nodes.id,'40%','60%');
<%-- 						$("#add-window .iframeid")[0].src = '<%=basePath%>oa/patMenuManager/addMenu.action?id='+nodes.id'; --%>
// 						$('#add-window').window('open');
			...
			function editContent(){
				...
				if(node.length > 0){
					AdddilogModel("addContent-window","栏目编辑",'<%=basePath%>oa/patMenuManager/editMenu.action?code='+node[0].code,'40%','60%');
				}else{
				...
				var rows = $('#dg').datagrid('getSelected');
				 ...
				 $.messager.confirm('确认', '确定要删除选中信息吗?', function(res){//提示是否删除
						if (res){
							...
							var code = rows.code;
							$.ajax({
							 	url: "<%=basePath%>oa/patMenuManager/delMenu.action?id="+ids+"&code="+code,   
								type:'post',
								success: function() {
			...
			function mysort(index,id,morder,parentcode, type, gridname) {
				...
				if (index == 0 && 'up' == type){
					...
					setTimeout(function(){
						...
						$.ajax({
					 	url: "<%=basePath%>oa/patMenuManager/move.action",
