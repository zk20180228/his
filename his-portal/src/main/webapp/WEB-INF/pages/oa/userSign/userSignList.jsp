...
<%--   			</shiro:hasPermission> --%>
		...
		<div id="confirmPassword" style="width: 350px;height:300px"> 
			<div style="margin:28px 50px;width: 180px;height: 30px">
				<input style="width:97%;" id="passwordSerc"  class="easyui-textbox" data-options="type:'password'" />
			</div>
		...
		$(function(){
			$('#ed').datagrid({  
	            ...
	            toolbar:'#toolbarId',
	            url: "<%=basePath%>oa/userSign/queryUserSignList.action",
...
// 			//获取焦点  
// 			$('#passwordSerc').textbox('textbox').bind('focus',function(){
// 				$('#passwordSerc').textbox({
// 					prompt:'',
// 					type:'password'	
// 				});
// 			 });
		...
		function add(){
			AddOrShowEast('<%=basePath %>oa/userSign/addUserSign.action');
		}
...
// 			$(".panel.window.messager-window").remove();
			console.info($('#passwordSerc').length)
			var row = $('#ed').datagrid('getSelected');
...
// 						closePasswordLayout();
						...
						closePasswordLayout();
						$('#passwordSerc').textbox('setValue','');
				   		AddOrShowEast('<%=basePath %>oa/userSign/editUserSign.action?search='+row.id);
					}
		...
		function AddOrShowEast(url) {
			...
			if(eastpanel.length>0){
				...
				$('#divLayout').layout('add', {
					...
					closable:true
// 					onClose:function(){
// 						console.info($(".panel.window.easyui-fluid").length);
// 						$(".panel.window.easyui-fluid").remove();
// 					},
// 					onDestroy:function(){
...
// 					}
		...
		function del(){
			var rows = $('#ed').datagrid('getChecked');
			if(rows.length>0){
				$.messager.confirm('确认', '确定要删除'+rows.length+"条数据吗?", function(res) {
					if(res){
						var ids = "";
						for (var i = 0; i < rows.length; i++) {
							if(ids==""){
