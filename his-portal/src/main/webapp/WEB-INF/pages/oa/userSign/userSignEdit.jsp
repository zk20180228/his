		...
		<div style="padding:10px">
			<form id="editForm">
				<table class="honry-table" cellpadding="0" cellspacing="0" border="0" style="margin:10px auto 0;">
					...
					<tr>
						<td class="honry-lable" id="showType">用户：</td>
...
// 	var userName = "${userName}";
	...
	$('#signType').combobox({
		...
		valueField:'code',
		textField:'name',
		data:[{code:1,name:'电子签名'},{code:2,name:'电子签章'}],
		onSelect:function(record){
			$('#signCategory').combobox({
				disabled:false
			});
			$('#signCategory').combobox('setValue','');
			$('#userAccName').textbox('setValue','');
			$('#userAcc').val('');
			if(record.code==1){
				$('#signCategory').combobox('setValue','1');
				/* $('#userAccName').textbox('setValue',userName);
				$('#userAcc').val(userAccount); */
				$('#showType').html($('#signCategory').combobox('getText').substr(1,3)+"：")
			}
		}
	});
	...
	$('#signCategory').combobox({
		...
		valueField:'code',
		textField:'name',
		data:[{code:1,name:'按用户'},{code:2,name:'按角色'},{code:3,name:'按职务'},{code:4,name:'按科室'}],
		onSelect:function(record){
...
//  $('#imgId').prop('src',null);
		...
		if(signType==1){
			...
			$.ajax({
				...
				success: function(data) {
					if(data.id!=null&&data.id!=id){//电子签名唯一 如果存在不添加记录  如果查询id和更改id一致 进行更改操作
						$.messager.alert('提示','该用户已经存在电子签名，请删除后再继续添加!');
						flag=false;
					}
				}
			});
	...
	function openBedWin(){
		...
		var signCategory = $('#signCategory').combobox('getValue');
		var signType=$('#signType').combobox('getValue');
		Adddilog("请选择用户",'<%=basePath%>oa/userSign/queryOpen.action?type='+signCategory+'&signType='+signType+'&timer='+new Date().getTime(),'40%','60%');
	}
