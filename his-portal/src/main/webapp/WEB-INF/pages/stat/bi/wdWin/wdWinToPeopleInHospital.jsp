							...
							<table class="honry-table" cellpadding="1" cellspacing="1" border="1px solid black">
								...
								<tr>
									...
									<tr>
									 ...
									 <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									 &nbsp;<input id="sexTdId" class="easyui-combobox" value="" name="report_sex" style='width:200px;'></input>
									 </td>
									</tr>
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>入院来源
									 </td>
									 <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									 &nbsp;<input id="souceTdId" class="easyui-combobox" value="" name="in_source" style='width:200px;'></input>
									 </td>
									</tr>
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>病危
									 </td>
									 <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									 &nbsp;<input id="critiTdId" class="easyui-combobox" value="" name="critical_flag" style='width:200px;'></input>
									 </td>
									</tr>
							</table>
						</form>
	...
	var deptCodeMap = new Map();//20160817 hedong 用于存放后台获取的deptCode 000001=子部门 
		   ...
		   $('#monthTdId1').combobox({
			...
			$('#souceTdId').combobox({
				...
				textField:'name',
				 ...
				 onChange:function(){
					    $("#msgTd").html("&nbsp;");
	                    var placeCodes = $('#souceTdId').combotree('getValues');
	                    if(placeCodes!=""){
	                    	if(dimensionString==""){
			...
			$('#critiTdId').combobox({
			    ...
			    onSelect:function(node){
			    	 $("#msgTd").html("&nbsp;");
	                    var sexCodes = $('#critiTdId').combobox('getValues');
	                    if(sexCodes!=""){
	                    	if(dimensionString==""){
	                        	dimensionString="critical_flag,病危";
	                        }else if(dimensionString.indexOf("critical_flag")==-1){
	                        	dimensionString=dimensionString+",critical_flag,病危";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("critical_flag")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("critical_flag");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",critical_flag,病危","");
	                    			 }else{
	                    				 dimensionString = dimensionString.replace("critical_flag,病危,","");
	                    			 }
	                    		 }
	   ...
	   //20160817 hedong  将数组中数据进行','拼接 end
		...
		function confirmWd(){
			...
			var deptValue="";
			var deptVal = $("#deptTdId").combobox("getValues");//用户选择全部时 dept_code='1'
		      if(deptVal){
	        ...
	        var sexValue ="";
	        var sexVal = $("#sexTdId").combobox("getValues");//用户选择全部 city_code='1'
	        if(sexVal){
	      	   sexValue = sexVal;
	         }
	        var sourseValue = "";
	        var sourseVal = $("#souceTdId").combobox("getValues");//获取入院来源选项值，若不为空  放入sourseValue到下方拼接参数
		      if(sourseVal){
		    	  sourseValue = sourseVal;
		      }
		      var flagValue ="";
		        var flagVal = $("#critiTdId").combobox("getValues");//用户选择全部 city_code='1'
		        if(flagVal){
		        	flagValue = flagVal;
		         }
			...
			if(dimensionString.indexOf(",")!=-1){
				...
				for(var i=0;i<diArr.length;i++){
					...
					if(diArr[i]=="in_source"){
						if(dimensionValue==""){
							dimensionValue="in_source,"+sourseValue;
						}else{
							dimensionValue=dimensionValue+"?in_source,"+sourseValue;
						}
					}
					if(diArr[i]=="critical_flag"){
						if(dimensionValue==""){
							dimensionValue="critical_flag,"+flagValue;
						}else{
							dimensionValue=dimensionValue+"?critical_flag,"+flagValue;
						}
					}
				}
			}
