								...
								<tr>
									...
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>科室
									  ...
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="deptTdId" class="easyui-combotree" value="" name="doct_dept" style='width:200px;'></input>
								     </td>
									</tr>
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>患者年龄
									  ...
									  <span>
									   ...
									   &nbsp;至&nbsp;<input class="easyui-textbox"   style='width:145px;' name='patient_age' id='ageTdId2'  />&nbsp;<select id="ageComboxbox" class="easyui-combobox"  style="width:50px;">   
									...
									<tr>  
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>地域
									  ...
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="address" class="easyui-combobox" value="" name="patient_nativeplace" style='width:200px;'></input>
								     </td>
									</tr>
									<!-- 文化程度缺少字段临时注释掉<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>文化程度
									 </td>
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="degree" class="easyui-combobox" value="" name="employee_education_code" style='width:200px;'></input>
								     </td>
									</tr> -->
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>支付方式
									 </td>
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="payway" class="easyui-combobox" value="" name="mode_code" style='width:200px;'></input>
								     </td>
									</tr>
							</table>
						</form>
	...
	var deptCodeMap = new Map();//20160817 hedong 用于存放后台获取的deptCode 000001=子部门 
		...
		$(function(){
			...
			$("#ageTdId2").next("span").children().first().blur(function(){
			    if($("#ageTdId2").val()){
			    	if(!validateNumForage($("#ageTdId2").val())){
				     ...
				     if(Number($("#ageTdId2").val())<=Number($("#ageTdId1").val())){
				    	 $("#ageTdId2").textbox('setValue','');
						 $("#msgTd").html("&nbsp;");
		   ...
		   $('#monthTdId1').combobox({
				 ...
				 onChange:function(){
					 $("#msgTd").html("&nbsp;");
	                    ...
	                    if(deptCodes!=""){ 
	                        ...
	                        }else if(dimensionString.indexOf("doct_dept")==-1){
	                        	dimensionString=dimensionString+",doct_dept,科室";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("doct_dept")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("doct_dept");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",doct_dept,科室","");
	                    			 }else{
	                    				 var tmpStr = "doct_dept,科室";
			...
			//地址省
			$('#address').combobox({
				url: "<c:url value='/statistics/outpatientFeeType/queryAddress.action'/>?parId=",
                valueField:'cityCode',    
                textField:'cityName',
			    editable:false,
			    multiple:true,
			    onSelect:function(node){
			    	 $("#msgTd").html("&nbsp;");
	                    var sexCodes = $('#address').combobox('getValues');
	                    if(sexCodes!=""){
	                    	if(dimensionString==""){
	                        	dimensionString="patient_nativeplace,地域";
	                        }else if(dimensionString.indexOf("patient_nativeplace")==-1){
	                        	dimensionString=dimensionString+",patient_nativeplace,地域";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("patient_nativeplace")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("patient_nativeplace");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",patient_nativeplace,地域","");
	                    			 }else{
	                    				 dimensionString = dimensionString.replace("patient_nativeplace,地域,","");
	                    			 }
	                    		 }
			...
			/* //学历
			$('#degree').combobox({
				url: "<c:url value='/statistics/outpatientFeeType/queryDegree.action'/>",
                valueField:'encode',    
                textField:'name',
			    editable:false,
			    multiple:true,
			    onSelect:function(node){
			    	 $("#msgTd").html("&nbsp;");
	                    var sexCodes = $('#degree').combobox('getValues');
	                    if(sexCodes!=""){
	                    	if(dimensionString==""){
	                        	dimensionString="employee_education_code,学历";
	                        }else if(dimensionString.indexOf("employee_education_code")==-1){
	                        	dimensionString=dimensionString+",employee_education_code,学历";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("employee_education_code")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("employee_education_code");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",employee_education_code,学历","");
	                    			 }else{
	                    				 dimensionString = dimensionString.replace("employee_education_code,学历,","");
	                    			 }
	                    		 }
			...
			//支付方式
			$('#payway').combobox({
				url: "<c:url value='/statistics/outpatientFeeType/queryPayWay.action'/>",
                valueField:'encode',    
                textField:'name',
			    editable:false,
			    multiple:true,
			    onSelect:function(node){
			    	 $("#msgTd").html("&nbsp;");
	                    var sexCodes = $('#payway').combobox('getValues');
	                    if(sexCodes!=""){
	                    	if(dimensionString==""){
	                        	dimensionString="mode_code,支付方式";
	                        }else if(dimensionString.indexOf("mode_code")==-1){
	                        	dimensionString=dimensionString+",mode_code,支付方式";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("mode_code")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("mode_code");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",mode_code,支付方式","");
	                    			 }else{
	                    				 dimensionString = dimensionString.replace("mode_code,支付方式,","");
	                    			 }
	                    		 }
		...
		function clearSelectVal(selectArr){
		 ...
		 function cloneage(){
			...
			/*  $("#tmpageTd").append("<span><br/><br/>&nbsp;&nbsp;&nbsp;&nbsp; <input class='easyui-textbox'   style='width:145px;' name='age' id='ageTdId1-"+globalVar+"'  />&nbsp; 至&nbsp;<input class='easyui-textbox'   style='width:145px;' name='age' id='ageTdId2-"+globalVar+"' />  <a id='"+globalVar+"' href='javascript:deleteage("+globalVar+");' class='easyui-linkbutton' iconCls='icon-delete' plain='true'></a></span>"
			 ...
			 var targetObj = $("<span><br/><br/>&nbsp;<input class='easyui-textbox'   style='width:145px;' name='age' id='ageTdId1-"+globalVar+"'  />&nbsp; 至&nbsp;<input class='easyui-textbox'   style='width:145px;' name='patient_age' id='ageTdId2-"+globalVar+"' />&nbsp;<select id='ageComboxbox-"+globalVar+"' class='easyui-combobox'  style='width:50px;'><option value='岁'>岁</option><option value='月'>月</option>  <option value='天'>天</option> </select> <a id='"+globalVar+"' href='javascript:deleteage("+globalVar+");' class='easyui-linkbutton' iconCls='icon-delete' plain='true'></a></span>").appendTo("#tmpageTd");
			 //hedong 重新渲染样式		 
				...
				ageTdId2Obj.next("span").children().first().blur(function(){
				    if(ageTdId2Obj.val()){
				    	if(!validateNumForage(ageTdId2Obj.val())){
					     ...
					     if(Number(ageTdId2Obj.val())<=Number(ageTdId1Obj.val())){
					    	 ageTdId2Obj.textbox('setValue','');
							 $("#msgTd").html("&nbsp;");
							 $("#msgTd").append("<font color='red'>截止患者年龄不能小于或等于起始患者年龄！</font>"); 
					     }
	  ...
	  //20160817 hedong 将数组中数据进行','拼接 begin
	   ...
	   //20160817 hedong  将数组中数据进行','拼接 end
		function confirmWd(){
			 ...
			 //hedong 20160817 (若用户选择了科室分类如)处理科室begin....
				...
				//hedong 20160817 (若用户选择了科室分类如)处理科室end....
		          ...
		          if($("#ageTdId1").val()||$("#ageTdId2").val()){
				      ...
				      ageValue=ageValue+ $("#ageComboxbox").combobox("getValue");
		          }
	    	    ...
	    	    for(var y=3;y<=globalVar;y++){
					 ...
					 if(ageTdId1Obj.length>0&&ageTdId2Obj.length>0){
						 if(ageTdId1Obj.val()==''&& ageTdId2Obj.val()==''){ 
							 ...
							 ageValue=ageValue+ageUitObj.combobox("getValue");
						 } 
					 }
	    	    }
			...
			//地域
			 var addressValue ="";
		      var addressVal = $("#address").combobox("getValues");//用户选择全部 
		      if(addressVal){
			...
			/* //文化程度
			 var degreeValue ="";
		      var degreeVal = $("#degree").combobox("getValues");//用户选择全部 
		      if(degreeVal){
		    	  degreeValue = degreeVal;
		      }
		     */
			//支付方式
			 var paywayValue ="";
		      var paywayVal = $("#payway").combobox("getValues");//用户选择全部 
		      if(paywayVal){
		    	  paywayValue = paywayVal;
		      }
		  	if(deptValue==""&&addressValue==""&&paywayValue==""&&ageValue==""){
				 $("#msgTd").html("&nbsp;");
				 $("#msgTd").append("<font color='red'>维度不能都为空！</font>"); 
				 return;
			}
			if(dimensionString.indexOf(",")!=-1){
				var diArr = dimensionString.split(",");
				for(var i=0;i<diArr.length;i++){
					if(diArr[i]=="doct_dept"){
						if(dimensionValue==""){
							dimensionValue="doct_dept,"+deptValue
						}else{
							dimensionValue=dimensionValue+"?doct_dept,"+deptValue
						}
					}if(diArr[i]=="patient_age"){
						if(dimensionValue==""){
							dimensionValue="patient_age,"+ageValue
						}else{
							dimensionValue=dimensionValue+"?patient_age,"+ageValue
						}
					}if(diArr[i]=="patient_nativeplace"){
						if(dimensionValue==""){
							dimensionValue="patient_nativeplace,"+addressValue
						}else{
							dimensionValue=dimensionValue+"?patient_nativeplace,"+addressValue
						}
					}
					/* if(diArr[i]=="employee_education_code"){
						if(dimensionValue==""){
							dimensionValue="employee_education_code,"+degreeValue
						}else{
							dimensionValue=dimensionValue+"?employee_education_code,"+degreeValue
						}
					} */if(diArr[i]=="mode_code"){
						if(dimensionValue==""){
							dimensionValue="mode_code,"+paywayValue
						}else{
							dimensionValue=dimensionValue+"?mode_code,"+paywayValue
						}
					}
				}
			}
