...
<body class="easyui-layout" data-options="fit:true">
	...
	<script type="text/javascript">
		...
		var deptMap=null;
		var dimensionNameArray =['总住院人数','住院人次费用','药品费用','其他费用','药费占比(%)'];
		$(function(){
			//查询科室map
			$.ajax({
		...
		function queryList(dateArray,dateType,dimensionString,dimensionValue){
			...
			$('#table1').datagrid({
				...
				remoteSort:false,
	 			url:'<%=basePath%>bi/drugFeeTolFeeRatio/queryDrugFeeTolFeeRatio.action', 
		...
		*/
		function popWin(){
			...
			window.open ("<c:url value='/statistics/wdWin/wdWinToDrugFeeTolFeeRatio.action?randomId='/>"+timerStr,'newwindow','height=350,width=700,top=50,left=500,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
		}
