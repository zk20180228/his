...
<body>
	<div id="topTiltle" class="bottomLine" style="box-sizing: border-box; width:100%; height:50px;padding-left: 5px;">
	...
	<div id = "content_table" style="margin-top:30px;margin-left:0.6%;" align = "center">
		...
		<div id = "tb1"  class="linkRelativeRatioBox MYchar" style="width: 50%;height: calc(60% - 40px);margin-top:30px;"> 
		...
		<div id="tb2" class="onBasisBox MYchar" style="width: 50%;height: calc(60% - 40px);margin-top:30px;"> 
 ...
 */
    ...
    function start(type,date){
	   ...
	   onBasis =$("#tb2");
    	...
    	linkRelativeRatio.setLoading("linkRelativeRatiosed")
    	onBasis.setLoading("onBasissed") 
    	
    	$.ajax({ 
 		  url:"<%=basePath%>statistics/DrugIncome/queryDrugFee.action",
 		   type:"post",
           ...
           success: function(data) {
			  ...
			  data[0][1].length == 0 && (classify.setLoading({
        	   ...
        	   for(var i = 0 ;i<data1.length;i++ ){
        		   data1Data.push({
        			   name:data1[i].stat,
        			   value:data1[i].douValue.toFixed(2)
        		   })
        		   datatitle.push(data1[i].stat)
        	   }
        	   ...
        	   for(var j = 0 ;j<data2.length;j++){
        		   data2y.push(((data2[j].douValue)).toFixed(2))
        		   data2x.push(data2[j].dept)
        	   }
        	  	...
        	  	var data3y = []
	         	   for(var k = 0 ;k<data3.length;k++){
	         		   data3y.push(((data3[k].douValue)).toFixed(2))
	        		   data3x.push(data3[k].doctor)
	        	   }
        		...
        		var data4 = data[0][4]
        	  	...
        	  	var data4y = []
        		var databai = []
        		var data4strhead = "<tr><th></th>"
        		var data4strvalue = "<tr><td>总收入</td>"
        		for(var l = data4.length-1 ;l>=1;l--){
	        		   data4x.push(data4[l-1].name)
	        		   data4y.push((((data4[l-1].douValue-0))).toFixed(2))
	        		   if(data4[l].douValue != 0){
	         		   databai.push(((data4[l-1].douValue-data4[l].douValue)/data4[l].douValue*100).toFixed(2) +"%")
	        		   }else{
	        			   databai.push("--")
	        		   }
	         			data4strhead+="<th style=\"text-align: center;\">"+data4[l-1].name+"</th>"
	         			data4strvalue+="<td>"+(data4[l-1].douValue-0).toFixed(2)+"</td>"
	        	   }
        		...
        		data4strhead+="</tr>";
        		console.info(data4strhead);
        		linkRelativeRatioTableHead.html(data4strhead);
//         		linkRelativeRatioTableHead.prop("align","center");
        		...
        		var data5strvalue = "<tr><td>总收入</td>"
        		for(var o = data5.length-1;o >=1 ;o-- ){
        			data5x.push(data5[o-1].name)
	        		data5y.push((((data5[o-1].douValue-0))).toFixed(2))
	        		if(data5[o].douValue != 0){
	        	 	databai2.push(((data5[o-1].douValue-data5[o].douValue)/data5[o].douValue*100).toFixed(2)+"%")
	        		}else{
	        			databai2.push("--")
	        		}
	        	 	data5strhead+="<th style=\"text-align: center;\">"+data5[o-1].name+"</th>"
	        	 	data5strvalue+="<td>"+(data5[o-1].douValue-0).toFixed(2)+"</td>"
        		}
...
var charAll = {
	...
	(function(charAll) {
		var classifyoption = {
			...
			color:['#FF9190', '#FDB12A', '#79C2F5', '#A3E874', '#27727B', "#CCA6D9", "#ECE579", "#E1B070",'#7B7081','#68C2FF','#9C6DBB','#E4C0FE','#FF9190','#FFB434','#CE4855','#46EE79','#5DD251'],  
