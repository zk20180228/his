								...
								<tr>
									...
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>科室
									  ...
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="deptTdId" class="easyui-combotree" value="" name="reg_dpcd" style='width:200px;'></input>
								     </td>
									</tr>
									<tr>
									</tr>
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>费别
									 </td>
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="codeNameTdId" class="easyui-combobox" value="" name="class_code" style='width:200px;'></input>
								     </td>
									</tr>
							</table>
						</form>
	...
	<script type="text/javascript">
		...
		$(function(){
		   ...
		   $('#monthTdId1').combobox({
				 ...
				 onChange:function(){
					 $("#msgTd").html("&nbsp;");
	                    ...
	                    if(deptCodes!=""){ 
	                        ...
	                        }else if(dimensionString.indexOf("reg_dpcd")==-1){
	                        	dimensionString=dimensionString+",reg_dpcd,科室";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("reg_dpcd")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("reg_dpcd");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",reg_dpcd,科室","");
	                    			 }else{
	                    				 var tmpStr = "reg_dpcd,科室";
			...
			//费别
			$('#codeNameTdId').combobox({
				url:'<%=basePath%>bi/project/systemTypeCombobox.action',
                valueField:'codeEncode',    
                textField:'codeName',
			    editable:false,
			    multiple:true,
			    onSelect:function(node){
			    	 $("#msgTd").html("&nbsp;");
	                    var sexCodes = $('#codeNameTdId').combobox('getValues');
	                    if(sexCodes!=""){
	                    	if(dimensionString==""){
	                        	dimensionString="class_code,费别";
	                        }else if(dimensionString.indexOf("class_code")==-1){
	                        	dimensionString=dimensionString+",class_code,费别";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("class_code")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("class_code");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",class_code,费别","");
	                    			 }else{
	                    				 dimensionString = dimensionString.replace("class_code,费别,","");
	                    			 }
	                    		 }
		...
		function confirmWd(){
    	    ...
    	    var sexVaue ="";
		      var sexVal = $("#codeNameTdId").combobox("getValues");//用户选择全部 
		      if(sexVal){
		    	  sexValue = sexVal;
		      }
			...
			if(dimensionString.indexOf(",")!=-1){
				...
				for(var i=0;i<diArr.length;i++){
					if(diArr[i]=="reg_dpcd"){
						if(dimensionValue==""){
							dimensionValue="reg_dpcd,"+deptValue
						}else{
							dimensionValue=dimensionValue+"?reg_dpcd,"+deptValue
						}
					}if(diArr[i]=="class_code"){
						if(dimensionValue==""){
							dimensionValue="class_code,"+sexValue
						}else{
							dimensionValue=dimensionValue+"?class_code,"+sexValue
						}
					}
				}
			}
