						...
						<form id="search" method="post">
							<table class="honry-table" cellpadding="1" cellspacing="1" border="1px solid black">
								...
								<tr>
									...
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>手术类型
									 </td>
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="kind" class="easyui-combobox" value="" name="ops_kind" style='width:150px;'></input>
								     </td>
									</tr>
	...
	var dimensionString =""; //除时间以外的其他维度  其他维度(目前暂时有 科室 年龄 地域 )按照选择的先后顺序来排放
		...
		$(function(){
		   ...
		   $('#monthTdId1').combobox({
			...
			$('#kind').combobox({ 
				url:"<%=basePath%>statistics/operationCost/treeOperationCost.action", 
				valueField:'encode',
				textField:'name',
				 multiple: true,
				 editable:false,
				 panelHeight:120,
		           onSelect:function(){
					 $("#msgTd").html("&nbsp;");
	                   var deptCodes = $('#kind').combogrid('getValues');
	                    if(deptCodes!=""){ 
	                        if(dimensionString==""){
	                        	dimensionString="ops_kind,手术类型";
	                        }else if(dimensionString.indexOf("ops_kind")==-1){
	                        	dimensionString=dimensionString+",ops_kind,手术类型";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("ops_kind")!=-1){
	                    			 ...
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",ops_kind,手术类型","");
	                    			 }else{
	                    				 var tmpStr = "ops_kind,手术类型";
...
// 			 dayEnd=$("#dayTdId2").combobox("getValue");
			...
			var kind="";
			var kindVal = $("#kind").combogrid("getValues");//用户选择全部时 dept_code='1'
		      if(kindVal){
		    	  kindValue = kindVal;
		      }
			...
			if(!kindValue){
				 $("#msgTd").html("&nbsp;");
				 $("#msgTd").append("<font color='red'>手术类别不能为空！</font>"); 
				 return;
			}
			...
			if(dimensionString.indexOf(",")!=-1){
				...
				for(var i=0;i<diArr.length;i++){
					if(diArr[i]=="ops_kind"){
						if(dimensionValue==""){
							dimensionValue="ops_kind,"+kindValue;
						}else{
							dimensionValue=dimensionValue+"?ops_kind,"+kindValue;
						}
					}
				}
			}
