...
<script type="text/javascript">
	...
	var regMap=null;
	var feeMap=null;
	var dimensionNameArray =['人数','同比','环比','总费用','同比','环比','人均费用','同比','环比'];
...
//  var dateArray=new Array();//数组元素为年、季度、月、日的值
	...
	function queryList(dateArray,dateType,dimensionString,dimensionValue){
		...
		$('#table1').datagrid({
			...
			remoteSort:false,
 			url:'<%=basePath%>statistics/Outpatientcost/getOutpatientfeelist.action', 
	...
	*/
	function popWin(){
		...
		window.open ("<c:url value='/statistics/wdWin/WDWinToOutPatientWorkloadbi.action?randomId='/>"+timerStr,'newwindow','height=350,width=700,top=50,left=500,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
	}
