								...
								<tr>
									...
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>科室
									  ...
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="deptTdId" class="easyui-combotree" value="" name="exec_dept" style='width:200px;'></input>
								     </td>
									</tr>
									<tr>  
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>手术医生
									 </td>
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="opDoct" class="easyui-combobox" value="" name="ops_docd" style='width:200px;'></input>
								     </td>
									</tr>
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>手术分类
									 </td>
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="opKind" class="easyui-combobox" value="" name="ops_kind" style='width:200px;'></input>
								     </td>
									</tr>
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>手术名称
									 </td>
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="opName" class="easyui-combobox" value="" name="operation_id" style='width:200px;'></input>
								     </td>
									</tr>
							</table>
						</form>
	...
	var deptCodeMap = new Map();//20160817 hedong 用于存放后台获取的deptCode 000001=子部门 
		...
		$(function(){
		   ...
		   $('#monthTdId1').combobox({
				 ...
				 onChange:function(){
					 $("#msgTd").html("&nbsp;");
	                    ...
	                    if(deptCodes!=""){ 
	                        ...
	                        }else if(dimensionString.indexOf("exec_dept")==-1){
	                        	dimensionString=dimensionString+",exec_dept,科室";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("exec_dept")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("exec_dept");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",exec_dept,科室","");
	                    			 }else{
	                    				 var tmpStr = "exec_dept,科室";
			...
			//手术分类
			$('#opKind').combobox({
				url : "<c:url value='/baseinfo/pubCodeMaintain/queryDictionary.action'/>?type=operatetype",
                valueField:'encode',    
                textField:'name',
			    editable:false,
			    multiple:true,
			    onSelect:function(node){
			    	 $("#msgTd").html("&nbsp;");
	                    var sexCodes = $('#opKind').combobox('getValues');
	                    if(sexCodes!=""){
	                    	if(dimensionString==""){
	                        	dimensionString="ops_kind,手术分类";
	                        }else if(dimensionString.indexOf("ops_kind")==-1){
	                        	dimensionString=dimensionString+",ops_kind,手术分类";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("ops_kind")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("ops_kind");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",ops_kind,费别","");
	                    			 }else{
	                    				 dimensionString = dimensionString.replace("ops_kind,费别,","");
	                    			 }
	                    		 }
			...
			$('#opDoct').combobox({
			    ...
			    onSelect:function(node){
			    	 $("#msgTd").html("&nbsp;");
	                    var sexCodes = $('#opDoct').combobox('getValues');
	                    if(sexCodes!=""){
	                    	if(dimensionString==""){
	                        	dimensionString="ops_docd,手术医生";
	                        }else if(dimensionString.indexOf("ops_docd")==-1){
	                        	dimensionString=dimensionString+",ops_docd,手术医生";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("ops_docd")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("ops_docd");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",ops_docd,手术医生","");
	                    			 }else{
	                    				 dimensionString = dimensionString.replace("ops_docd,手术医生,","");
	                    			 }
	                    		 }
			...
			$('#opName').combobox({
			    ...
			    onSelect:function(node){
			    	 $("#msgTd").html("&nbsp;");
	                    var sexCodes = $('#opName').combobox('getValues');
	                    if(sexCodes!=""){
	                    	if(dimensionString==""){
	                        	dimensionString="operation_id,手术名称";
	                        }else if(dimensionString.indexOf("operation_id")==-1){ 
	                        	dimensionString=dimensionString+",operation_id,手术名称";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("operation_id")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("operation_id");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",operation_id,手术名称","");
	                    			 }else{
	                    				 dimensionString = dimensionString.replace("operation_id,手术名称,","");
	                    			 }
	                    		 }
	   ...
	   //20160817 hedong  将数组中数据进行','拼接 end
		function confirmWd(){
			...
			//手术分类
			  var opKindValue ="";
		      var opKindVal = $("#opKind").combobox("getValues");//用户选择全部 
		      if(opKindVal){
		    	  opKindValue = opKindVal;
		      } 
			//手术医生
			 var opDocValue ="";
		      var opDocVal = $("#opDoct").combobox("getValues");//用户选择全部 
		      if(opDocVal){
		    	  opDocValue = opDocVal;
		      }
			//手术名称
			 var opNameValue ="";
		      var opNameVal = $("#opName").combobox("getValues");//用户选择全部 
		      if(opNameVal){
		    	  opNameValue = opNameVal;
		      }
		      if(deptValue==""&&opKindValue==""&&opDocValue==""&&opNameValue==""){
					 $("#msgTd").html("&nbsp;");
			...
			if(dimensionString.indexOf(",")!=-1){
				...
				for(var i=0;i<diArr.length;i++){
					if(diArr[i]=="exec_dept"){
						if(dimensionValue==""){
							dimensionValue="exec_dept,"+deptValue
						}else{
							dimensionValue=dimensionValue+"?exec_dept,"+deptValue
						}
					}if(diArr[i]=="ops_kind"){
						if(dimensionValue==""){
							dimensionValue="ops_kind,"+opKindValue
						}else{
							dimensionValue=dimensionValue+"?ops_kind,"+opKindValue
						}
					}if(diArr[i]=="ops_docd"){
						if(dimensionValue==""){
							dimensionValue="ops_docd,"+opDocValue
						}else{
							dimensionValue=dimensionValue+"?ops_docd,"+opDocValue
						}
					}if(diArr[i]=="operation_id"){
						if(dimensionValue==""){
							dimensionValue="operation_id,"+opNameValue
						}else{
							dimensionValue=dimensionValue+"?operation_id,"+opNameValue
						}
					}
				}
			}
