...
<script type="text/javascript">
	...
	$(function(){
		...
		$.ajax({
			url:'<%=basePath%>statistics/outpatientFeeType/queryAddressMap.action',
			success:function(datavalue){
				addressMap=datavalue;
			}
		});
		$.ajax({
			url:'<%=basePath%>statistics/outpatientFeeType/queryAgeMap.action',
			success:function(datavalue){
				ageMap=datavalue;
			}
		});
		$.ajax({
			url:'<%=basePath%>statistics/outpatientFeeType/queryDegreeMap.action',
			success:function(datavalue){
				degreeMap=datavalue;
			}
		});
		$.ajax({
			url:'<%=basePath%>statistics/outpatientFeeType/queryPaywayMap.action',
			success:function(datavalue){
				paywayMap=datavalue;
			}
		});
	...
	function queryList(dateArray,dateType,dimensionString,dimensionValue){
		...
		var dimensionNameArray =['金额(元)','比例(%)'];
		...
		$('#table1').datagrid({
			columns:columnsArray,					
 			url:'<%=basePath%>statistics/outpatientFeeType/queryOutpatientFeeType.action', 
	...
	*/
	function popWin(){
		...
		window.open ("<c:url value='/statistics/wdWin/WDWinToOutFeeType.action?randomId='/>"+timerStr,'newwindow','height=370,width=700,top=50,left=500,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
	}
	...
	function doct_deptfunction(value,row,index){
		if(value!=null&&value!='' && value.indexOf(":")==-1){
			return deptMap[value];
		}else{
			return value;
	...
	function patient_nativeplacefunction(value,row,index){
		if(value!=null&&value!='' && value.indexOf(":")==-1){
	...
	function mode_codefunction(value,row,index){
		if(value!=null&&value!='' && value.indexOf(":")==-1){
			return paywayMap[value];
		}else{
			return value;
		}
	}
