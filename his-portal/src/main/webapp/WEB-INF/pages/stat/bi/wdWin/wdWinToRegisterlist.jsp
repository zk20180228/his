							...
							<table class="honry-table" cellpadding="1" cellspacing="1" border="1px solid black">
								...
								<tr>
									...
									<tr>
									 ...
									 <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									 &nbsp;<input id="feeTdId" class="easyui-combobox" value="" name="fee_code" style='width:200px;'></input>
									 </td>
									</tr>
							</table>
						</form>
	...
	var deptCodeMap = new Map();//20160817 hedong 用于存放后台获取的deptCode 000001=子部门 
		...
		$(function(){
		   ...
		   $('#monthTdId1').combobox({
			...
			//挂号级别下拉
			$('#feeTdId').combobox({
				url:"<%=basePath%>statistics/register/querygradename.action",
				valueField:'gradeCode',
				textField:'gradeName',
				multiple:true,
				onChange:function(){
					 $("#msgTd").html("&nbsp;");
					 var reglevlCode=$('#feeTdId').combotree('getValues');
					 if(reglevlCode!=""){
						 if(dimensionString==""){
	   ...
	   //20160817 hedong  将数组中数据进行','拼接 end
		...
		function confirmWd(){
			...
			//hedong 20160817 (若用户选择了科室分类如)处理科室end....
			 var feeValue="";
				var feeVal = $("#feeTdId").combobox("getValues");//用户选择全部时 inhos_deptcode='1'
			      if(feeVal){
			    	  feeValue = feeVal;
			      }
			...
			if(dimensionString.indexOf(",")!=-1){
				...
				for(var i=0;i<diArr.length;i++){
					...
					if(diArr[i]=="reglevl_code"){
						if(dimensionValue==""){
							dimensionValue="reglevl_code,"+feeValue
						}else{
							dimensionValue=dimensionValue+"?reglevl_code,"+feeValue
						}
					}
				}
			}
