...
$(function(){
	...
	$.ajax({
		url:'<%=basePath%>bi/project/systemTypeFunction.action',
		async:false,
		success:function(datavalue){
			feeMap=datavalue;
		}
...
function reg_dpcdfunction(value,row,index){
	if(value!=null&&value!=''){
		return deptMap[value];
	}
}
function class_codefunction(value,row,index){
	if(value!=null&&value!=''){
		return feeMap[value];
	}
}
...
function popWin(){
	...
	window.open ("<c:url value='/statistics/wdWin/WDWinToProject.action?randomId='/>"+timerStr,'newwindow','height=350,width=700,top=50,left=500,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
}
function queryList(datevo,dateType,dimensionString,dimensionValue){
	var dimensionNameArray =['费用（元）','平均费用'];
...
// 		remoteSort:false,
		url:'<%=basePath%>bi/project/queryProjectoadDatagrid.action', 
