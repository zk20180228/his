								...
								<tr>
									...
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>科室
									  ...
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="deptTdId" class="easyui-combotree" value="" name="inhos_deptcode" style='width:200px;'></input>
								     </td>
									</tr>
									<tr>
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>费用类别
									  ...
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="feeTdId" class="easyui-combobox" name="fee_code" style='width:200px;'></input>
								     </td>
									</tr>
									<tr style="display: none" id="setTr">
									 <td style='width: 320px;height: 40px;' class='honry-lable' style='width: 5%;' nowrap='nowrap'>性别
									 </td>
									  <td style='width: 320px;height: 40px;' class='honry-info' nowrap='nowrap'>
									  &nbsp;<input id="sexTdId" class="easyui-combobox" name="sex" style='width:200px;'></input>
								     </td>
									</tr>
							</table>
						</form>
	...
	<script type="text/javascript">
		...
		$(function(){
		   ...
		   $('#monthTdId1').combobox({
				 ...
				 onChange:function(){
					 $("#msgTd").html("&nbsp;");
	                    ...
	                    if(deptCodes!=""){ 
	                        ...
	                        }else if(dimensionString.indexOf("inhos_deptcode")==-1){
	                        	dimensionString=dimensionString+",inhos_deptcode,科室";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("inhos_deptcode")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("inhos_deptcode");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",inhos_deptcode,科室","");
	                    			 }else{
	                    				 var tmpStr = "inhos_deptcode,科室";
			...
			$('#feeTdId').combobox({ 
				url:"<%=basePath%>statistics/wdWin/casminfeeCom.action", 
				valueField:'encode',
				textField:'name',
				 multiple: true,
				 onChange:function(){
					 $("#msgTd").html("&nbsp;");
	                    var feeCodes = $('#feeTdId').combobox('getValues');
	                    if(feeCodes!=""){
	                        if(dimensionString==""){
	                        	dimensionString="fee_code,费用类别";
	                        }else if(dimensionString.indexOf("fee_code")==-1){
	                        	dimensionString=dimensionString+",fee_code,费用类别";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("fee_code")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("fee_code");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",fee_code,费用类别","");
	                    			 }else{
	                    				 var tmpStr = "fee_code,费用类别";
	                    				 if(dimensionString==tmpStr){
	                    					 dimensionString = dimensionString.replace("fee_code,费用类别","");
	                    				 }else{
	                    					 dimensionString = dimensionString.replace("fee_code,费用类别,","");
	                ...
	                onSelect:function(record){
	                	var rows = $('#feeTdId').combobox('getData');
	                	if(record.encode==00){
		                	for(var i=0;i<rows.length;i++){
		                		$('#feeTdId').combobox('select',rows[i].encode);
		                	}
	                	}
	                },
	                onUnselect:function(record){
	                	...
	                	if(record.encode==00){
		                	for(var i=0;i<rows.length;i++){ 
		                		$('#feeTdId').combobox('unselect',rows[i].encode);
		                	}
	                	}
	                }
			});
			...
			$('#sexTdId').combobox({
				...
				multiple : false,
				 onChange:function(){
					 $("#msgTd").html("&nbsp;");
	                    var feeCodes = $('#sexTdId').combobox('getValues');
	                    if(feeCodes!=""){
	                        if(dimensionString==""){
	                        	dimensionString="report_sex,性别";
	                        }else if(dimensionString.indexOf("report_sex")==-1){
	                        	dimensionString=dimensionString+",report_sex,性别";
	                        }
	                    }else{
	                    	if(dimensionString!=""){
	                    		 if(dimensionString.indexOf("report_sex")!=-1){
	                    			 var tmpIndex = dimensionString.indexOf("report_sex");
	                    			 if(tmpIndex!=0){
	                    				 dimensionString= dimensionString.replace(",report_sex,性别","");
	                    			 }else{
	                    				 var tmpStr = "report_sex,性别";
	                    				 if(dimensionString==tmpStr){
	                    					 dimensionString = dimensionString.replace("report_sex,性别","");
	                    				 }else{
	                    					 dimensionString = dimensionString.replace("report_sex,性别,","");
		...
		function confirmWd(){
		    ...
		    var feeValue="";
			var feeVal = $("#feeTdId").combobox("getValues");
			if(feeVal){
			    feeValue = feeVal;
			}
		    var sexValue="";
			var sexVal = $("#sexTdId").combobox("getValues");
			if(sexVal){
				sexValue = sexVal;
			}	
			...
			if(dimensionString.indexOf(",")!=-1){
				...
				for(var i=0;i<diArr.length;i++){
					if(diArr[i]=="inhos_deptcode"){
						if(dimensionValue==""){
							dimensionValue="inhos_deptcode,"+deptValue
						}else{
							dimensionValue=dimensionValue+"?inhos_deptcode,"+deptValue
						}
					}
					if(diArr[i]=="report_sex"){
						if(dimensionValue==""){
							dimensionValue="report_sex,"+sexValue
						}else{
							dimensionValue=dimensionValue+"?report_sex,"+sexValue
						}
					}
					if(diArr[i]=="fee_code"){
						for(var j=0;j<feeValue.length;j++){ 
							if(feeValue[j]=="00"){
								feeValue.remove(feeValue[j]);
							}
						}
						if(dimensionValue==""){
							dimensionValue="fee_code,"+feeValue
						}else{
							dimensionValue=dimensionValue+"?fee_code,"+feeValue
						}
					}
				}
			}
			var dateVo = [Number(yearStart),Number(yearEnd),Number(quarterStart),Number(quarterEnd),Number(monthStart),Number(monthEnd),Number(dayStart),Number(dayEnd)];
			window.opener.queryList(dateVo,dateType,dimensionString,dimensionValue,flag);
			window.close();
		}
