    ...
    grid.prototype = {
        create: function () {
            ...
            if ($this.options.title) {
                str.push('<div class="panel-heading clearfix"><span style="float:left;line-height: 34px;font-size: 20px;">' + $this.options.title+'</span>' );
                if($this.options.seach){
            		str.push('<div class="imggrid-srachBox" style="float:right">'+
									'<input type="text" style="    width: 200px;height: 34px;text-indent: 10px;" id="imggrid-srachinput" placeholder="请输入查询信息">'+
									'<button type="button" style="margin-left:50px" id="imggrid-srachBtn" class="btn btn-primary">查询</button>'+
            				'</div>');
      		    }
                str.push('</div>')
            }
            ...
            $(".imggrid-list").height($(".imggrid-list").height()-87)
            if ($this.options.width) { $this.$elem.css('width', $this.options.width); }
            $("#imggrid-srachBtn").on("click",function(){
        	  $this.showLoading();
                $.ajax({
                    type: $this.options.type, 
                    url: $this.options.url,
                    data:$.param($.extend($this.defaults.page, $this.options.params)) + "&queryName="+ $("#imggrid-srachinput").val(),
            ...
            page.find('.last_gridPager').on('click', function () {
                if ($this.isActivePageBtn(this)) { $this.gotopage(Math.ceil( $this.options.page.total/$this.options.page.rows)); }
            });
        ...
        gotopage: function (page) {
            ...
            if (page < 0 || ($this.options.page.total > 0 && page > Math.ceil( $this.options.page.total/$this.options.page.rows))) { return; }
        ...
        formatData: function (data) {
            ...
            $this.options.page.total = data.total; 
            //parseInt(data.records / $this.options.page.rows) + (data.records % $this.options.page.rows > 0 ? 1 : 0);
            ...
            if ($this.options.page.page >= Math.ceil( $this.options.page.total/$this.options.page.rows)) {
            ...
            $this.$elem.find('.imggrid-totalPage').html(Math.ceil( $this.options.page.total/$this.options.page.rows));
            ...
            $this.$elem.find('.imggrid-right').html('显示第 ' + startrecord + ' - ' + endrecord + ' 条记录　检索到 ' + $this.options.page.total + ' 条记录');
            $this.data = data.rows;
        ...
        loadData: function () {
            ...
            if ($this.options.data) {//本地存储
                ...
                var newdata = {
                    ...
                    queryName:$("#imggrid-srachinput").val(),
                    rows: []
                };
                ...
                $this.showLoading();
                $.ajax({
                    type: $this.options.type, url: $this.options.url, 
                    data: $.param($.extend($this.defaults.page, $this.options.params))+"&queryName="+$("#imggrid-srachinput").val(),
                    dataType: 'json',
                    success: function (data) {
                        $this.hideLoading();
