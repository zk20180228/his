...
My97DP.prototype = {
  init: function() {
    ...
    $c.currFocus = $dp.el;
    hideSel();
  	var $fontBox = document.getElementsByTagName("head")[0]
  	var $fontLink = document.createElement("style")
		var $fontSize = parent.document.getElementById("fontlink").getAttribute("href")
		if($fontSize.indexOf("fontSize0") != -1){
				$fontLink.innerHTML = ".WdateDiv .WdayTable td {font-size: 16px!important;}"
				$fontBox.appendChild($fontLink)
		}else if ($fontSize.indexOf("fontSize1") != -1){
				$fontLink.innerHTML = ".WdateDiv .WdayTable td {font-size: 18px!important;}"
				$fontBox.appendChild($fontLink)
		}else if ($fontSize.indexOf("fontSize2") != -1){
				$fontLink.innerHTML = ".WdateDiv .WdayTable td {font-size: 22px!important;}"
				$fontBox.appendChild($fontLink)
		}
  },
...
//  	  if (_[C].contentWindow == window) {
//  		    _[C].style = "width:256px;height:380px";
//	  		_[C].contentWindow.document.getElementsByTagName("div")[0].style.marginTop = "0px";
//	  		_[C].contentWindow.document.getElementsByTagName("div")[0].getElementsByTagName("div")[0].getElementsByTagName("div")[2].getElementsByTagName("div")[0].style.marginTop = "20px";
//  	  }
//    }
  },
  _fd: function() {
  ...
  _fM: function($) {
    this._fMyPos($, $d.MD);
   // $d.MD.innerHTML = this._f("M", 12, 1, 12, "i+j*6+1", $ == $d.rMI)
  ...
  _fy: function(_, B, A) {
    ...
    A = A || _ == $d.ryI;
    // B = rtn(B, $dt.y - 14);  原来的 
    B = rtn(B, new Date().getFullYear() - 14); // 上面改成当前年
    $.a(this._f("y", 9999,3, 5, B + "+i+j*5", A));//'y'代表'年',9999为允许的最大值,3代表显示3列,5代表每列显示5个
    $.a("<table  cellspacing=0 cellpadding=3 border=0 align=center><tr><td ");
    $.a(this.minDate.y < B ? "class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onmousedown='if(event.preventDefault)event.preventDefault();event.cancelBubble=true;$c._fy(0," + (B - 15) + "," + A + ")'": "class='invalidMenu'");
    ...
    $.a(this.maxDate.y >= B + 15 ? "class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onmousedown='if(event.preventDefault)event.preventDefault();event.cancelBubble=true;$c._fy(0," + (B + 15) + "," + A + ")'": "class='invalidMenu'");
    $.a(">\u2192</td></tr></table>");
    this._fMyPos(_, $d.yD);
    $d.yD.innerHTML = $.j()
  },
  ...
  _fillQS: function(C, A) {
    ...
    _.a("<tr  class=MTitle><td colspan = 2><div style=\"float:left\">" + $ + "</div>");
    if (!C) _.a("<div style=\"float:right;cursor:pointer\" onclick=\"hide($d.qsDivSel);\">X&nbsp;</div>");
    _.a("</td></tr>");
    for (var D = 0; D < B.length/2; D++) if (B[D] && B[D+1]) {
    // 年快速选择
      _.a("<tr><td style='text-align:centen' nowrap='nowrap' class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onclick=\"");
      _.a("day_Click(" + B[D].y + ", " + B[D].M + ", " + B[D].d + "," + B[D].H + "," + B[D].m + "," + B[D].s + ");\">");
      _.a("&nbsp;" + this.getDateStr(null, B[D]));
      if(B[D+5]){
    	  _.a("</td><td style='text-align:centen' nowrap='nowrap' class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onclick=\"");
    	  _.a("day_Click(" + B[D+5].y + ", " + B[D+5].M + ", " + B[D+5].d + "," + B[D+5].H + "," + B[D+5].m + "," + B[D+5].s + ");\">");
    	  _.a("&nbsp;" + this.getDateStr(null, B[D+5]));
    	  _.a("</td></tr>")
      }
      
    } else _.a("<tr><td class='menu'>&nbsp;</td><td class='menu'>&nbsp;</td></tr>");
    _.a("</table>");
  ...
  initQS: function() {
    ...
    _ = $dp.has.minUnit;
    if (E > $) E = $;
    else if (_ == "m" || _ == "s") C = [ - 60, -30, 0, 30, 60, -15, 15, -45, 45,-60];
    else{
    	var date=new Date;
    	...
    	for (H = 0; H < $ + 9; H++){
        	...
        	if(_ == "y"){
        		if($dt[_]< year - 5){
        			C[H] = $dt[_] - 5 + H; // 快速选择年月显示数量
        		}else{
        			C[H] = year - 9 + H;
        		}
        	}else if(_ == "M"){
        		//判断月 
        		//$dt["y"]*12+$dt[_] - year*12+month
        		if(Math.abs(($dt["y"]*12+$dt[_]) - (year*12+month)) > 5){
        			C[H] = $dt[_] - 5 + H;
        		}else{
        			C[H] = month - 9 + H;
        		}
...
function _focus($) {
  ...
  showB(_);
  if ("Hms".indexOf(A) >= 0) {
    _.style.marginLeft = Math.min(this.offsetLeft+30, $d.sI.offsetLeft + 60 - _.offsetWidth+30);
