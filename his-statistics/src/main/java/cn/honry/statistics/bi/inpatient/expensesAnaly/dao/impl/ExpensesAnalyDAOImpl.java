package cn.honry.statistics.bi.inpatient.expensesAnaly.dao.impl;
...
@Repository("expensesAnalyDAO")
@SuppressWarnings({"all"})
public class ExpensesAnalyDAOImpl extends HibernateEntityDao<BiInpatientFeeinfo> implements ExpensesAnalyDAO{
	...
	@Override
	public List<ExpensesAnalyVO> querytExpensesAnalyDatagrid(
		...
		if(dateType==1){
			sql.append(", trunc((sum(t.tot_cost)/(select sum(ss.TOT_COST) from view_inpatient_feeinfo ss "
					...
					+ "and ss.trans_type = 1 and mm.report_code = 'ZY01' ");
			int a = 0;
		...
		sql.append("  left join view_inpatient_info aa on aa.inpatient_no=t.inpatient_no  ");
		sql.append("	 where t.trans_type = 1 and m.report_code='ZY01'  ");
		if(dateType==1){
		...
		List<ExpensesAnalyVO> bdl= queryObject1.setResultTransformer(Transformers.aliasToBean(ExpensesAnalyVO.class)).list();
		if(bdl!=null){
			return bdl;
		}
