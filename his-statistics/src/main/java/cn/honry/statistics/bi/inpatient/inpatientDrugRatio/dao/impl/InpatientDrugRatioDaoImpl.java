package cn.honry.statistics.bi.inpatient.inpatientDrugRatio.dao.impl;
...
@Repository("inpatientDrugRatioDao")
@SuppressWarnings({ "all" })
public class InpatientDrugRatioDaoImpl extends HibernateEntityDao<BiInpatientInfo> implements InpatientDrugRatioDao{
	...
	@Override
	public List<InpatientDrugRatioVo> queryInpatientDrugRatio(String[] diArrayKey,List<Map<String,List<String>>> list,int dateType,DateVo datevo ) {
				//天数
				int num=365;
				String sqldate=null;
				//通过判断时间维度种类确定天数和
				...
				sql.append("	         count(c.INPATIENT_NO) as inpatientSum, ");
				sql.append("	       TRUNC(sum(t.TOT_COST),2) as inpatientFee, ");
				sql.append("	      TRUNC( sum(e.TOT_COST),2) as drugFee, ");	
				sql.append("	       TRUNC(sum(t.TOT_COST)*1./count(c.INPATIENT_NO),2) as perFee, ");
				sql.append("	       TRUNC(sum(t.TOT_COST)*1./"+num+",2) as dayFee, ");
				if(dateType==1){
				...
				sql.append("	 left join BI_INPATIENT_INFO c on c.INPATIENT_NO = t.INPATIENT_NO ");
...
//						sql.append(" and d.dept_code  in ('8751','8131','8030')");
				...
				queryObject1.addScalar("inpatientSum",Hibernate.DOUBLE).addScalar("inpatientFee",Hibernate.DOUBLE)
						.addScalar("drugFee",Hibernate.DOUBLE).addScalar("perFee",Hibernate.DOUBLE).addScalar("dayFee",Hibernate.DOUBLE).addScalar("timeChose");
				List<InpatientDrugRatioVo> bdl = queryObject1.setResultTransformer(Transformers.aliasToBean(InpatientDrugRatioVo.class)).list();
				if(bdl!=null){
					return bdl;
				}
				return new ArrayList<InpatientDrugRatioVo>();
	}
