package cn.honry.statistics.bi.inpatient.drugFeeTolFeeRatio.dao.impl;
...
import cn.honry.statistics.bi.inpatient.drugFeeTolFeeRatio.dao.DrugFeeTolFeeRatioDAO;
import cn.honry.statistics.bi.inpatient.drugFeeTolFeeRatio.vo.DrugFeeTolFeeRatioVo;
import cn.honry.statistics.bi.inpatient.inpatientDrugRatio.vo.InpatientDrugRatioVo;
import cn.honry.statistics.util.dateVo.DateVo;
@Repository("drugFeeTolFeeRatioDAO")
@SuppressWarnings({ "all" })
public class DrugFeeTolFeeRatioDAOImpl extends HibernateEntityDao<BiInpatientInfo> implements DrugFeeTolFeeRatioDAO{
	...
	@Override
	public List<DrugFeeTolFeeRatioVo> queryDrugFeeTolFeeRatio(String[] diArrayKey, List<Map<String, List<String>>> list,int dateType, DateVo datevo) {
		//天数
		int num=365;
		String sqldate=null;	
		//通过判断时间维度种类确定天数和
		...
		sql.append("	         count(1) as inpatientTolSum, ");
		sql.append("	       TRUNC(sum(t.TOT_COST),2) as inpatientFee, ");
		sql.append("	       TRUNC(sum(e.TOT_COST),2) as drugFee, ");	
		sql.append("	      TRUNC( (sum(t.TOT_COST)-sum(e.TOT_COST)),2) as otherFee, ");
		sql.append("	      round((sum(e.TOT_COST) / sum(t.TOT_COST))*100,2) as drugFeeRatio, ");
		if(dateType==1){
		...
		sql.append("	  from VIEW_INPATIENT_FEEINFO t ");
		sql.append("	 left join BI_MINFEETOSTAT b on b.MINFEE_CODE = t.FEE_CODE");
		sql.append("	 left join VIEW_INPATIENT_FEEINFO e on e.RECIPE_NO = t.RECIPE_NO and b.MINFEE_CODE = e.FEE_CODE and b.FEE_STAT_CODE in('01','02','03') where 1=1  ");
		if(dateType==1){
...
//				sql.append(" and d.dept_code  in ('8751','8131','8030')");
		...
		queryObject1.addScalar("inpatientTolSum",Hibernate.DOUBLE).addScalar("inpatientFee",Hibernate.DOUBLE)
				.addScalar("drugFee",Hibernate.DOUBLE).addScalar("otherFee",Hibernate.DOUBLE).addScalar("drugFeeRatio").addScalar("timeChose");
		List<DrugFeeTolFeeRatioVo> bdl=bdl = queryObject1.setResultTransformer(Transformers.aliasToBean(DrugFeeTolFeeRatioVo.class)).list();
		if(bdl!=null){
			return bdl;
		}
		return new ArrayList<DrugFeeTolFeeRatioVo>();
	}  
	@Override
